...
#define TEST_ROOT "handle_to_wire_latency"
#define TEST_FILE "handle_to_wire_latency_file"
...
namespace {
  ...
  class HandleToWireEmptyLatencyTest : public gtest::GaneshaBaseTest {
  protected:
...
TEST_F(HandleToWireEmptyLatencyTest, SIMPLE)
{
  fsal_status_t status;
...
TEST_F(HandleToWireEmptyLatencyTest, LOOP)
{
  fsal_status_t status;
  struct gsh_buffdesc fh_desc;
  ...
  fprintf(stderr, "Average time per handle_to_wire: %" PRIu64 " ns\n",
