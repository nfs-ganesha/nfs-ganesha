...
namespace {
  ...
  class ReaddirEmptyLatencyTest : public ::testing::Test {
  protected:
    ...
    virtual void SetUp() {
      ...
      status = fsal_create(root_entry, TEST_DIR, DIRECTORY, &attrs, NULL,
		      &test_dir, &attrs_out);
      ASSERT_EQ(status.major, 0);
    ...
    virtual void TearDown() {
      ...
      status = test_dir->obj_ops.unlink(root_entry, test_dir, TEST_DIR);
