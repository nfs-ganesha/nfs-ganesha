		...
		int *currlen, int maxlen)
{
	...
	if (*currlen + strlen(this_node->name) > maxlen) {
		...
		strncpy(fullpseudopath + (*currlen), this_node->name, maxlen - *currlen - 1);	// leave room for \0
	      ...
	      pseudofs_entry_t * this_node, int maxlen)
{
	...
	if (currlen + strlen(new_node->name) <= maxlen) {
		...
		strncpy(fullpseudopath + currlen, new_node->name, maxlen - currlen - 1);	// leave room for \0
...
static bool pseudo_node(char *token, void *arg)
{
	...
	if (gPseudoFs.last_pseudo_id == (MAX_PSEUDO_ENTRY - 1)) {
		LogMajor(COMPONENT_NFS_V4_PSEUDO,
			 ...
			 state->entry->id, state->entry->fullpath,
	...
	key =
	    create_pseudo_handle_key(fullpseudopath,
	...
	if (isMidDebug(COMPONENT_NFS_V4_PSEUDO)) {
		...
		LogMidDebug(COMPONENT_NFS_V4_PSEUDO,
			    ...
			    gPseudoFs.last_pseudo_id, new_node->name,
			    fullpseudopath, str);
	}
...
static bool export_to_pseudo(struct gsh_export *exp, void *arg)
{
	...
	if (entry->pseudopath[0] != '/') {
		LogCrit(COMPONENT_NFS_V4_PSEUDO,
			"Pseudo Path '%s' is badly formed", entry->pseudopath);
		state->retval = EINVAL;
		return false;
	}
	if (strlen(entry->pseudopath) > MAXPATHLEN) {
		LogCrit(COMPONENT_NFS_V4_PSEUDO,
			"Bad Pseudo=\"%s\", path too long", entry->pseudopath);
		state->retval = EINVAL;
		return false;
	}
	/* Parsing the path
...
int nfs4_ExportToPseudoFS(void)
{
	...
	key =
	    create_pseudo_handle_key(gPseudoFs.root.name,
	...
	for (i = 0; i < gPseudoFs.last_pseudo_id; i++) {
		if (isFullDebug(COMPONENT_NFS_V4_PSEUDO)) {
			...
			LogFullDebug(COMPONENT_NFS_V4_PSEUDO,
				     ...
				     gPseudoFs.last_pseudo_id, str);
		}
		if (memcmp
		    (gPseudoFs.reverse_tab[i]->fsopaque, pfhandle4->fsopaque,
		     V4_FH_OPAQUE_SIZE) == 0) {
...
void nfs4_PseudoToFhandle(nfs_fh4 * fh4p, pseudofs_entry_t * psfsentry)
{
	...
	if (isFullDebug(COMPONENT_NFS_V4_PSEUDO)) {
		...
		LogFullDebug(COMPONENT_NFS_V4_PSEUDO,
			     "PSEUDO_TO_FH: name:%s handle:%s", psfsentry->name,
			     str);
	}
			   ...
			   struct nfs_resop4 *resp)
{
	...
	if (nfs4_PseudoToFattr
	    (psfsentry, &(res_GETATTR4->GETATTR4res_u.resok4.obj_attributes),
	...
	if (iter->junction_export == NULL) {
       ...
       */
		...
		if (data->currentFH.nfs_fh4_val == NULL) {
			if ((error =
			     nfs4_AllocateFH(&(data->currentFH))) != NFS4_OK) {
				LogMajor(COMPONENT_NFS_V4_PSEUDO,
			   ...
			   struct nfs_resop4 *resp)
{
	...
	LogMidDebug(COMPONENT_NFS_V4_PSEUDO,
		    ...
		    ", sizeof(entry4)=%lu num_entries=%lu", dircount, maxcount,
	...
	if (NFS_SpecificConfig.UseCookieVerf == 1) {
		memcpy(cookie_verifier, &ServerBootTime.tv_sec,
	...
	for (; iter != NULL; iter = iter->next) {
		...
		if ((entry_nfs_array[i].name.utf8string_val =
		     gsh_malloc(namelen + 1)) == NULL) {
			LogError(COMPONENT_NFS_V4_PSEUDO, ERR_SYS, ERR_MALLOC,
				 errno);
			res_READDIR4->status = NFS4ERR_RESOURCE;
			return res_READDIR4->status;
		}
		memcpy(entry_nfs_array[i].name.utf8string_val, iter->name,
		       namelen);
		...
		if (iter->junction_export == NULL) {
			...
			if (nfs4_PseudoToFattr
			    (iter, &(entry_nfs_array[i].attrs), data, &entryFH,
			...
			if (res_READDIR4->status == NFS4ERR_WRONGSEC) {
				...
				if (check_for_wrongsec_ok_attr
					 ...
					 */
					if (nfs4_PseudoToFattr(iter, &(entry_nfs_array[i].attrs), data, NULL,	/* don't need the file handle */
							       &(arg_READDIR4->
								 attr_request))
					    != 0) {
						LogFatal
				...
				cache_status =
				    nfs_export_get_root_entry(iter->
							      junction_export,
							      &entry);
