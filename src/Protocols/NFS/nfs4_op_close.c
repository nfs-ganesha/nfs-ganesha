		  ...
		  struct nfs_resop4 *resp)
{
	...
	nfs_status = nfs4_Check_Stateid(&arg_CLOSE4->open_stateid, data->current_entry, &state_found, data, data->minorversion == 0 ? STATEID_SPECIAL_FOR_CLOSE_40 : STATEID_SPECIAL_FOR_CLOSE_41, 0, FALSE,	/* do not check owner seqid */
					close_tag);
	...
	if (data->minorversion == 0) {
		if (!Check_nfs4_seqid
		    (open_owner, arg_CLOSE4->seqid, op, data->current_entry,
		     resp, close_tag)) {
	...
	update_stateid(state_found, &res_CLOSE4->CLOSE4res_u.open_stateid, data,
		       close_tag);
	...
	if (state_found->state_type == STATE_TYPE_SHARE) {
		state_status =
		    state_share_remove(state_found->state_entry, open_owner,
				       state_found);
