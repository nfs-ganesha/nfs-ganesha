			      ...
			      bool * new_state, fsal_openflags_t openflags)
{
	...
	*new_state = true;
	...
	if (cache_status != CACHE_INODE_SUCCESS) {
		 ...
		 */
		cache_status =
		    cache_inode_access(data->current_entry,
				       FSAL_MODE_MASK_SET(FSAL_X_OK) |
				       FSAL_ACE4_MASK_SET
				       (FSAL_ACE_PERM_EXECUTE), data->req_ctx);
	...
	if (*new_state) {
		state_status =
		    state_add_impl(data->current_entry, candidate_type,
				   &candidate_data, owner, &file_state,
	...
	if (*new_state) {
		...
		if ((candidate_type == STATE_TYPE_SHARE)
			...
			state_status =
			    state_share_upgrade(data->current_entry,
		      ...
		      state_owner_t ** owner)
{
	...
	*owner =
	    create_nfs4_owner(&owner_name, clientid, STATE_OPEN_OWNER_NFSV4,
			      NULL, 0, &isnew, CARE_ALWAYS);
	...
	if (!isnew) {
		...
		if (data->minorversion == 0) {
			if (arg_OPEN4->seqid == 0) {
				...
				/* Check for replay */
				if (!Check_nfs4_seqid
				    (*owner, arg_OPEN4->seqid, op,
				     data->current_entry, res, open_tag)) {
			     ...
			     cache_entry_t ** entry, const char *filename)
{
	...
	*entry = NULL;
	...
	if ((arg->openhow.openflag4_u.how.mode == EXCLUSIVE4_1)
		...
		char *verf =
		    (char *)((arg->openhow.openflag4_u.how.mode == EXCLUSIVE4_1)
				 ...
				 ch_createboth.cva_verf) : &(arg->openhow.
							     openflag4_u.how.
							     createhow4_u.
							     createverf));
		verf_provided = true;
	...
	cache_status =
	    cache_inode_create(parent, filename, REGULAR_FILE, mode, NULL,
			       data->req_ctx, &entry_newfile);
	...
	if (sattr_provided) {
		...
		if ((sattr.
		     mask & (ATTR_ACL | ATTR_ATIME | ATTR_MTIME | ATTR_CTIME |
			     ATTR_SIZE)) || ((sattr.mask & ATTR_OWNER)
					     && (data->req_ctx->creds->
						 caller_uid != sattr.owner))
		 ...
		 struct nfs_resop4 *resp)
{
	...
	LogFullDebug(COMPONENT_STATE,
		     ...
		     "Share Access = %d ", arg_OPEN4->claim.claim,
		     arg_OPEN4->openhow.opentype, arg_OPEN4->share_deny,
	...
	if ((arg_OPEN4->openhow.opentype == OPEN4_CREATE)
	    && ((data->export_perms.options & EXPORT_OPTION_MD_WRITE_ACCESS) ==
		0)) {
	...
	if (((arg_OPEN4->share_access & OPEN4_SHARE_ACCESS_WRITE) != 0)
	    && ((data->export_perms.options & EXPORT_OPTION_WRITE_ACCESS) ==
		0)) {
		res_OPEN4->status = NFS4ERR_ROFS;
	...
	case CLAIM_DELEGATE_CUR:
		...
		/* Check if filename is correct */
		res_OPEN4->status =
		    nfs4_utf8string2dynamic(&arg_OPEN4->claim.open_claim4_u.
		...
		cache_status =
		    cache_inode_lookup(entry_parent, filename, data->req_ctx,
				       &entry_lookup);
		...
		if (cache_status != CACHE_INODE_NOT_FOUND) {
			...
			LogDebug(COMPONENT_NFS_CB, "did not find entry %p",
				 entry_lookup);
	...
	res_OPEN4->status =
	    open4_do_open(op, data, owner, &file_state, &new_state, openflags);
 ...
 out:
	 ...
	 */
	if (data->minorversion == 0) {
		Copy_nfs4_state_req(owner, arg_OPEN4->seqid, op, entry_parent,
				    resp, open_tag);
	}
 ...
 out3:
	...
	if ((file_state != NULL) && new_state && (res_OPEN4->status != NFS4_OK)) {
