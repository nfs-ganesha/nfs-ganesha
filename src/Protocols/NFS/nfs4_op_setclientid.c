					...
					struct nfs_resop4 *resp)
{
	...
	if (data->req->rq_xprt->xp_tls.tls_established) {
		...
		if (data->req->rq_xprt->xp_tls.mtls)
			server_exflags |= EXCHGID4_FLAG_MTLS_ESTABLISHED;
	}
#endif
	...
	if (conf != NULL) {
		...
		if (!credmatch && clientid_has_state(conf)) {
			...
			res_SETCLIENTID4_INUSE->r_netid =
				(char *)netid_nc_table[conf->cid_cb.v40.cb_addr
							       .nc]
					.netid;
		...
		if (credmatch && memcmp(arg_SETCLIENTID4->client.verifier,
			...
			LogFullDebug(COMPONENT_CLIENTID,
				     "Update ClientId %s->%s", str_clientid4,
				     str_client);
			 ...
			 */
			LogFullDebug(COMPONENT_CLIENTID,
				     "Replace ClientId %s->%s", str_clientid4,
				     str_client);
		...
		clientid = new_clientid();
		display_clientid(&dspbuf_clientid4, clientid);
		LogFullDebug(COMPONENT_CLIENTID, "New client %s",
			     str_clientid4);
		new_clientid_verifier(verifier);
	}
