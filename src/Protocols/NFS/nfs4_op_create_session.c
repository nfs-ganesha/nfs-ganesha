...
#define log_channel_attributes(component, chan_attrs, name)                   \
	LogFullDebug(component,                                               \
		     ...
		     " ca_maxoperations %" PRIu32 " ca_maxrequests %" PRIu32, \
		     name, (chan_attrs)->ca_headerpadsize,                    \
					   ...
					   struct nfs_resop4 *resp)
{
	...
	if (rc == CLIENT_ID_SUCCESS) {
		...
		if (rc != CLIENT_ID_SUCCESS) {
			...
			LogDebug(component, "%s clientid=%s",
	...
	if (arg_CREATE_SESSION4->csa_flags &
		...
		LogDebug(component, "Invalid create session flags %" PRIu32,
			 arg_CREATE_SESSION4->csa_flags);
		res_CREATE_SESSION4->csr_status = NFS4ERR_INVAL;
		goto out;
	}
	...
	if (arg_CREATE_SESSION4->csa_fore_chan_attrs.ca_maxrequestsize <
		...
		LogWarnLimited(component, "Invalid channel attributes for %s",
			       data->tagname);
	...
	memcpy(res_CREATE_SESSION4ok->csr_sessionid, nfs41_session->session_id,
	       NFS4_SESSIONID_SIZE);

	GSH_AUTO_TRACEPOINT(nfs4, session_create, TRACE_INFO,
			    "Create session. Session: {}, refcount: 2",
			    nfs41_session);
	...
	if (conf != NULL) {
		...
		LogDebug(component, "Updating clientid %s->%s cb_program=%u",
			 str_clientid4, str_client,
			 arg_CREATE_SESSION4->csa_cb_program);
	...
	if (isDebug(component)) {
		...
		LogDebug(component, "success %s csa_flags 0x%X csr_flags 0x%X",
