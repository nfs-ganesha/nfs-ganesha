...
int test_mnt_Export()
{
	...
	for (i = 0; i < NB_EXPORT_ENTRIES; i++) {
		...
		/* setting paths */
		snprintf(export_entries[i].dirname,
			 sizeof(export_entries[i].dirname), "/dirname-%d", i);
		snprintf(export_entries[i].fsname,
			 sizeof(export_entries[i].fsname), "/fsname-%d", i);
		snprintf(export_entries[i].pseudopath, MAXPATHLEN,
			 "/pseudopath-%d", i);
		snprintf(export_entries[i].fullpath, MAXPATHLEN, "/fullpath-%d",
			 i);
		...
		case 1:
			...
			p_cli[0].client.hostif.clientaddr =
		...
		case 2:
			...
			strcpy(p_cli[1].client.netgroup.netgroupname,
			       "netgroup");
		...
		case 3:
			...
			strcpy(p_cli[3].client.gssprinc.princname,
			       "gssprincipal");
