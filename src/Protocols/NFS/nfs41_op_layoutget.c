                       ...
                       struct nfs_resop4 *resp)
{
  ...
  /*
   * Blank out argument structures and get the filehandle proper.
   */
...
void nfs41_op_layoutget_Free(LAYOUTGET4res * resp)
{
  ...
  if (resp->logr_status == NFS4_OK)
    {
      for (i = 0;
           i < resp->LAYOUTGET4res_u.logr_resok4.logr_layout.logr_layout_len;
           i++)
        {
...
/**
 *
 * acquire_layout_state: Get or make a layout state
 *
                                     ...
                                     const char *tag)
{
  ...
  /* If the state supplied is a layout state, we can simply use it */
  if (supplied_state->state_type == STATE_TYPE_LAYOUT)
    {
      *layout_state = supplied_state;
    }
  /* For share, delegation, and lock states, create a new layout
     state. */
           ...
           (supplied_state->state_type == STATE_TYPE_LOCK))
    {
      ...
      state_status = state_lookup_layout_state(data->current_entry,
                                               ...
                                               layout_type,
                                               &condemned_state);
      /* If it does, we assume that the client is using the forgetful
         model and has forgotten it had any layouts.  Free all
         layouts associated with the state and delete it. */
      if (state_status == STATE_SUCCESS)
        {
          /* Flag indicating whether all layouts were returned and
             the state was deleted */
          fsal_boolean_t deleted = FALSE;
          if ((nfs_status
...
/**
 *
 * one_segment: Grant and add one layout segment
 *
                            ...
                            layout4 *current)
{
  ...
  nfs_status = FSAL_layoutget(handle,
                              context,
                              &loc_body,
                              arg,
                              res);
