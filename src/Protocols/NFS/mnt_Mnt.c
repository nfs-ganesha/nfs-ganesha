	    ...
	    struct svc_req *preq, nfs_res_t * pres)
{
	...
	LogDebug(COMPONENT_NFSPROTO,
		 "REQUEST PROCESSING: Calling mnt_Mnt path=%s", parg->arg_mnt);
	...
	if (exp == NULL) {
		...
		LogEvent(COMPONENT_NFSPROTO,
			 "MOUNT: Export entry for %s not found", parg->arg_mnt);
	 ...
	 */
	if (parg->arg_mnt[0] != '/'
	    || !strcmp(parg->arg_mnt, p_current_item->fullpath)) {
		...
		if (FSAL_IS_ERROR
		    (exp_hdl->ops->
		     lookup_path(exp_hdl, req_ctx, parg->arg_mnt,
				 &pfsal_handle))) {
			switch (preq->rq_vers) {
			case MOUNT_V1:
				pres->res_mnt1.status = NFSERR_ACCES;
	...
	case MOUNT_V1:
		if (!nfs2_FSALToFhandle
		    (&(pres->res_mnt1.fhstatus2_u.directory), pfsal_handle)) {
...
void mnt3_Mnt_Free(nfs_res_t * pres)
{
	if (pres->res_mnt3.fhs_status == MNT3_OK) {
		gsh_free(pres->res_mnt3.mountres3_u.mountinfo.auth_flavors.
			 auth_flavors_val);
		gsh_free(pres->res_mnt3.mountres3_u.mountinfo.fhandle.
			 fhandle3_val);
	}
	return;
}				/* mnt_Mnt_Free */
