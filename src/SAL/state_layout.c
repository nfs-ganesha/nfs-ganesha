...
/**
 * add_layout_segment: Add a segment to an existing layout state
 *
 ...
 *
 * @param pstate  [IN] Pointer to the layout state.
 * @param segment [IN] The layout segment itself (as a layout4,
 *                     specified by RFC5661) returned by the FSAL.
                                 ...
                                 fsal_boolean_t  return_on_close)
{
  ...
  new_segment->sls_state     = pstate;
  new_segment->sls_segment   = *psegment;

  glist_add_tail(&pstate->state_data.layout.state_segments,
  ...
  if (return_on_close)
    {
      pstate->state_data.layout.state_return_on_close = TRUE;
    }
...
/**
 * state_lookup_layout_state: Find pre-existing layouts
 *
 * This function finds a state corresponding to a given file,
 * clientid, and layout type if one exists.
 *
 * @param pentry [IN]  Cache_inode entry for the file.
 * @param powner [IN]  Pointer to the state owner.  This must be
 *                     a clientid owner.
 * @param type   [IN]  The layout type specified by the client.
 * @param pstate [OUT] The found state, NULL if not found.
 *
