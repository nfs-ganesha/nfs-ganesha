...
{
	gsh_free(client->ssc_nlm_caller_name, MEM_COMP_CLIENTID);
	...
	gsh_free(client, MEM_COMP_CLIENTID);
}
...
state_nsm_client_t *get_nsm_client(care_t care, char *caller_name)
{
	...
	pclient = gsh_malloc(sizeof(*pclient), MEM_COMP_CLIENTID);
	...
	pclient->ssc_nlm_caller_name =
		gsh_strdup(key.ssc_nlm_caller_name, MEM_COMP_CLIENTID);
...
void free_nlm_client(state_nlm_client_t *client)
{
	...
	gsh_free(client->slc_nlm_caller_name, MEM_COMP_CLIENTID);
...
not_found:
	...
	pclient->slc_nlm_caller_name =
		gsh_strdup(key.slc_nlm_caller_name, MEM_COMP_CLIENTID);
