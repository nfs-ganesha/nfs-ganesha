...
static fsal_fs_locations_t *nfs4_fs_locations_alloc(const unsigned int count)
{
	...
	fs_locations =
		gsh_calloc(1, sizeof(fsal_fs_locations_t), MEM_COMP_PROTOCOL);
	if (count)
...
void nfs4_fs_locations_free(fsal_fs_locations_t *fs_locations)
{
	...
	gsh_free(fs_locations->fs_root, MEM_COMP_PROTOCOL);
	gsh_free(fs_locations->rootpath, MEM_COMP_PROTOCOL);

	for (i = 0; i < fs_locations->nservers; ++i)
		free_utf8string(&fs_locations->server[i]);

	PTHREAD_RWLOCK_destroy(&(fs_locations->fsloc_lock));
	gsh_free(fs_locations->server, MEM_COMP_PROTOCOL);
	gsh_free(fs_locations, MEM_COMP_PROTOCOL);
}
					   ...
					   const unsigned int count)
{
	...
	fs_locations->fs_root = gsh_strdup(fs_root, MEM_COMP_PROTOCOL);
	fs_locations->rootpath = gsh_strdup(rootpath, MEM_COMP_PROTOCOL);
	fs_locations->ref = 1;
