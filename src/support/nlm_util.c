                                                   ...
                                                   int exclusive)
{
  ...
  glist_for_each(glist, &nlm_lock_list)
  {
    ...
    if((nlm_entry_end > nlm_lock->l_offset) && (nlm_lock_end > nlm_entry->start))
      {
...
nlm_lock_entry_t *nlm_find_lock_entry(struct nlm4_lock *nlm_lock, int exclusive,
                                      int state)
{
  nlm_lock_entry_t *nlm_entry;
  struct glist_head *glist;
                                    ...
                                    struct glist_head *split_list)
{
  ...
  if(nlm_entry_end < nlm_lock->l_offset)
    /* nothing to split */
    goto done;
  if((nlm_lock->l_offset <= nlm_entry->start) && nlm_lock_end >= nlm_entry_end)
    /* Fully overlap */
...
static void nlm4_send_grant_msg(void *arg)
{
  ...
  retval = nlm_send_reply(NLMPROC4_GRANTED_MSG, nlm_entry->caller_name, &inarg, NULL);
