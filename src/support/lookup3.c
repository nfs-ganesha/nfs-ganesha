...
--------------------------------------------------------------------
*/
static void hashword2(const uint32_t * k,	/* the key, an array of uint32_t values */
		      size_t length,	/* the length of the key, in uint32_ts */
 ...
 */
static void hashlittle2(const void *key,	/* the key to hash */
...
#ifdef LITTLEEND
	ret = hashlittle((const uint32_t *)str, len, 13);
#else
	ret = hashbig((const uint32_t *)str, len, 13);
#endif

	return ret;
}				/* HashTable_hash_buff */

void Lookup3_hash_buff_dual(char *str, uint32_t len, uint32_t * pval1,
			    uint32_t * pval2)
{
#ifdef LITTLEEND
