...
#ifdef _USE_PNFS
static int nfs_read_conf_pnfs_ds_conf( config_item_t subblock, pnfs_ds_parameter_t * pds_conf )
{
	 ...
	 /* Get key's name */
	  ...
	  case CONFIG_ITEM_BLOCK:
            ...
            if( !strcasecmp(block_name, "DataServer"))
             if( nfs_read_conf_pnfs_ds_conf( item, &pparam->layoutfile.ds_param[ds_count] ) != 0 )
              {
                fprintf(stderr,
                    "Unknown or unsettable key: %s (item %s)\n", key_name, CONF_LABEL_PNFS);
                return -1 ;
