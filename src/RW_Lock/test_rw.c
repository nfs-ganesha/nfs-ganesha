...
int main( int argc, char * argv[] )
{
  ...
  for( i = 0 ; i < MAX_WRITTERS ; i++ ) 
    {
      if( ( rc = pthread_create( &ThrWritters[i], &attr_thr, thread_writter, (void *)NULL ) ) != 0 )
        {
  ...
  for( i = 0 ; i < MAX_READERS ; i++ ) 
    {
      if( ( rc = pthread_create( &ThrReaders[i], &attr_thr, thread_reader, (void *)NULL ) ) != 0 )
        {
          fprintf( stderr, "pthread_create: Erreur %d %d \n", rc, errno ) ;
          printf( "Test RW_Lock ECHOUE: Mauvaise allocation des thread\n" ) ;
          exit( 1 ) ;
  ...
  if( OkWrite == 1 & OkRead == 1 )
    ...
    {
      if( OkWrite == 0 )  printf( "Test RW_Lock ECHOUE: deadlock dans les redacteurs\n" ) ;
      if( OkRead == 0 ) printf( "Test RW_Lock ECHOUE: deadlock dans les lecteur\n" ) ;
      exit( 1 ) ;
