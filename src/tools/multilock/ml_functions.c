...
int compare_responses(response_t * expected, response_t * received)
{
  ...
  if(expected->r_client != received->r_client &&
     strcmp(expected->r_client->c_name, received->r_client->c_name) != 0)
    {
      sprintf(errdetail, "Unexpected response from %s", received->r_client->c_name);
      return FALSE;
    }
...
response_t * check_expected_responses(response_t *expected_responses, response_t * client_resp)
{
  ...
  while(expected_resp != NULL && !compare_responses(expected_resp, client_resp))
