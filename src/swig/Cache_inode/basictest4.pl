...
BEGIN {
  ...
  import Cache_inode_tools qw(:DEFAULT cacheinode_get cacheinode_lookup cacheinode_setattr cacheinode_getattr);
  require FSAL_tools;
...
for (my $i=0; $i<$b->{count}; $i++) {
  for (my $fi=0; $fi<$b->{files}; $fi++) {
    ...
    $ret = cacheinode_lookup($curentry, $lookupname, \$entry);
    if ($ret != 0) { die; }
    # CHMOD NONE (SETATTR)
    ...
    # CHMOD RW
    $sattr->{asked_attributes} = FSAL::get_mask_attr_mode(); # Oo ! $sattr->{asked_attributes} has been changed ...
