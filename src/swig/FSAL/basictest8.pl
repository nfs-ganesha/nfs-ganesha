...
BEGIN {
  require FSAL_tools;
  import FSAL_tools qw(:DEFAULT fsal_str2path fsal_lookupPath fsal_str2name fsal_symlink fsal_getattrs fsal_readlink fsal_path2str fsal_unlink);
}
...
for (my $ci=0; $ci<$b->{count}; $ci++) {
  for (my $fi=0; $fi<$b->{files}; $fi++) {
    ...
    $ret = fsal_symlink($curhdl, $linkname, $linkcontent, $mode, \$hdl);
    ...
    # READLINK
    $ret = fsal_readlink($hdl, \$linkcontent);
    ...
    # REMOVE
    $ret = fsal_unlink($curhdl, $linkname);
