                                                ...
                                                cache_inode_client_t * pclient)
{
      ...
      if( ( pfsal_handle_remove =  cache_inode_get_fsal_handle( to_remove_entry, &status ) ) == NULL )
        {
          return status ;
        }
      ...
      if (rc) DisplayLogJdLevel( pclient->log_outputs, NIV_CRIT, "HashTable_Del error %d in cache_inode_clean_internal", rc );
  ...
  if( to_remove_numlinks == 0 )
	...
	if( to_remove_entry->object.file.pentry_content != NULL )
      ...
      while( pentry_iter )
      {
          ...
          if ( (*pstatus = cache_inode_clean_internal(pentry_iter, ht, pclient)) != CACHE_INODE_SUCCESS )
          {
              if( use_mutex )
              {
                V_w( &pentry->lock ) ;
              ...
              DisplayLogJdLevel(pclient->log_outputs, NIV_CRIT, "cache_inode_clean_internal ERROR %d", *pstatus );
              return *pstatus;
          }
