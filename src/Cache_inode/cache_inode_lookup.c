      ...
      if(pentry == NULL)
        {
          ...
          if((pentry = cache_inode_new_entry(&new_entry_fsdata, /* This is a population and not a creation */
	  				     &object_attributes,
	  				     type, &create_arg, NULL, 
	  				     ht, pclient, pcontext, FALSE, pstatus)) == NULL)
            {
              if(use_mutex == TRUE)
                V_r(&pentry_parent->lock);
                                           ...
                                           cache_inode_status_t * pstatus)
{
  return cache_inode_lookup_sw(pentry_parent,
                               pname, pattr, ht, pclient, pcontext, pstatus,
                               FALSE);
