...
static inline bool gsh_xprt_ref(SVCXPRT * xprt, uint32_t flags, const char *tag,
				const int line)
{
	gsh_xprt_private_t *xu = (gsh_xprt_private_t *) xprt->xp_u1;
	uint32_t req_cnt;
...
extern int cmp_sockaddr(sockaddr_t * addr_1, sockaddr_t * addr_2,
			ignore_port_t ignore_port);
extern int sockaddr_cmpf(sockaddr_t * addr1, sockaddr_t * addr2,
			 ignore_port_t ignore_port);
