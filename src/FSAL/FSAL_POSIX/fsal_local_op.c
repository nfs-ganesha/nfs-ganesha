 ...
 * for example.
 *
 * \param cred (in posixfsal_cred_t *) user's identifier.
 ...
 *
 * \param pcontext (in posixfsal_cred_t *) user's context.
                                      ...
                                      fsal_attrib_list_t * pattrdest)   /* IN */
{
  ...
  fsal_status = POSIXFSAL_test_access(pcontext, FSAL_W_OK, pattrsrc);
  if(FSAL_IS_ERROR(fsal_status))
    Return(fsal_status.major, fsal_status.minor, INDEX_FSAL_rename_access);

  fsal_status = POSIXFSAL_test_access(pcontext, FSAL_W_OK, pattrdest);
  if(FSAL_IS_ERROR(fsal_status))
    Return(fsal_status.major, fsal_status.minor, INDEX_FSAL_rename_access);
                                      ...
                                      fsal_attrib_list_t * pattr)       /* IN */
{
  ...
  fsal_status = POSIXFSAL_test_access(pcontext, FSAL_W_OK, pattr);
  if(FSAL_IS_ERROR(fsal_status))
