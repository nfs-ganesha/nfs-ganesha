...
int  fsal2posix_testperm(fsal_accessflags_t testperm){
  ...
  if (testperm & FSAL_R_OK) posix_testperm |= R_OK;
  if (testperm & FSAL_W_OK) posix_testperm |= W_OK;
  if (testperm & FSAL_X_OK) posix_testperm |= X_OK;
  if (testperm & FSAL_F_OK) posix_testperm |= F_OK;
...
{ 
  ...
  if (fsal_flags & FSAL_O_RDONLY)                       strcpy(p_posix_flags, "r");
  else if (fsal_flags & FSAL_O_WRONLY & FSAL_O_APPEND)  strcpy(p_posix_flags, "a");
  else if (fsal_flags & FSAL_O_WRONLY & FSAL_O_TRUNC)   strcpy(p_posix_flags, "w");
  else if (fsal_flags & FSAL_O_APPEND)                  strcpy(p_posix_flags, "a+");
  else if (fsal_flags & FSAL_O_TRUNC)                   strcpy(p_posix_flags, "w+");
  else                                                  strcpy(p_posix_flags, "r+");
...
mode_t fsal2unix_mode( fsal_accessmode_t fsal_mode ){
  ...
  if ( (fsal_mode & FSAL_MODE_SVTX) ) out_mode |= S_ISVTX ;
...
fsal_accessmode_t unix2fsal_mode(  mode_t unix_mode ){
    ...
    if ( unix_mode & S_ISVTX ) fsalmode|= FSAL_MODE_SVTX;
  ...
  switch( posix_type_in & S_IFMT){
    ...
    default:
      DisplayLogJdLevel( fsal_log, NIV_EVENT,
          "Unknown object type: %d",posix_type_in );
      return -1;
  }    
...
fsal_status_t posix2fsal_attributes(  struct stat	* p_buffstat,
                                     fsal_attrib_list_t * p_fsalattr_out )
 {
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_TYPE)){
      p_fsalattr_out->type = 
          posix2fsal_type( p_buffstat->st_mode );
  }
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_SIZE)){
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_SPACEUSED )){
        p_fsalattr_out->spaceused
            =  p_buffstat->st_blocks * S_BLKSIZE ;
  }

  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_RAWDEV )){
        p_fsalattr_out->rawdev
            = posix2fsal_devt( p_buffstat->st_rdev ); /* XXX: convert ? */
  }
  /* mounted_on_fileid :
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
