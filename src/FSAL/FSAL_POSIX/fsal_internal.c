...
void fsal_increment_nbcall( int function_index , fsal_status_t  status )
{
  ...
  if (function_index >= FSAL_NB_FUNC) return;
                                         ...
                                         fs_specific_initinfo_t * fs_specific_info)
{
  ...
  /* sanity check */
  if (!fsal_info || !fs_common_info || !fs_specific_info) ReturnCode(ERR_FSAL_FAULT,0);
  ...
  if ( fsal_info->max_fs_calls > 0 )
  {
    ...
    if (rc != 0) ReturnCode(ERR_FSAL_SERVERFAULT,rc);
...
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  maxlink  = %lu   ",default_posix_info.maxlink);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  maxnamelen  = %lu  ",default_posix_info.maxnamelen);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  maxpathlen  = %lu  ",default_posix_info.maxpathlen);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  no_trunc  = %d ",default_posix_info.no_trunc);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  chown_restricted  = %d ",default_posix_info.chown_restricted);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  case_insensitive  = %d ",default_posix_info.case_insensitive);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  case_preserving  = %d ",default_posix_info.case_preserving);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  fh_expire_type  = %hu ",default_posix_info.fh_expire_type);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  link_support  = %d  ",default_posix_info.link_support);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  symlink_support  = %d  ",default_posix_info.symlink_support);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  lock_support  = %d  ",default_posix_info.lock_support);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  named_attr  = %d  ",default_posix_info.named_attr);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  unique_handles  = %d  ",default_posix_info.unique_handles);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  acl_support  = %hu  ",default_posix_info.acl_support);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  cansettime  = %d  ",default_posix_info.cansettime);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  homogenous  = %d  ",default_posix_info.homogenous);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  supported_attrs  = %llX  ",default_posix_info.supported_attrs);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  maxread  = %llX     ",default_posix_info.maxread);
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  maxwrite  = %llX     ",default_posix_info.maxwrite);
...
DisplayLogJdLevel( fsal_log, NIV_DEBUG,"}");
  ...
  if (
      (fs_common_info->behaviors.maxfilesize != FSAL_INIT_FS_DEFAULT ) ||
...
{
  /* sanity check */
  if (!info || !buffstat) ReturnCode(ERR_FSAL_FAULT,0);
...
add:  
  ...
  if (stdb.major == ERR_FSAL_POSIXDB_CONSISTENCY) { /* there is already an entry with this path, but it's an inconsistent one */
                                              ...
                                              struct stat             * p_buffstat /* OUT */)
{
  ...
  statusdb = fsal_posixdb_getInfoFromHandle( p_context->p_conn,
                                             ...
                                             &count);
  if (FSAL_POSIXDB_IS_ERROR(statusdb) && FSAL_IS_ERROR(status = posixdb2fsal_error(statusdb))) return status;
  ...
  if (p_buffstat) {
    for (i=0; i<count; i++) {
      ...
      /* error : delete the bad path from the database */
        ...
        if (!FSAL_IS_ERROR(status)) {
          statusdb = fsal_posixdb_delete(p_context->p_conn, &parenthdl, &filename, NULL);
    ...
    if (i==count) ReturnCode( ERR_FSAL_STALE, 0);
  ...
  switch (stdb.major) {
    case ERR_FSAL_POSIXDB_NOERR:
      ...
      if ( fsal_posixdb_consistency_check(&(p_object_handle->info), p_infofs) ) {
        ...
        if (FSAL_POSIXDB_IS_ERROR(stdb) && FSAL_IS_ERROR( st = posixdb2fsal_error(stdb) )) return st;
    ...
    case ERR_FSAL_POSIXDB_NOENT:
      ...
      st = fsal_internal_posixdb_add_entry( p_context->p_conn,
                                            ...
                                            p_parent_dir_handle,
                                            p_object_handle);
      if (FSAL_IS_ERROR(st)) return st;
      break;
    default:
      /* error */
      if (FSAL_IS_ERROR(st = posixdb2fsal_error(stdb))) return st;
  }
                                                     ...
                                                     fsal_handle_t           * p_object_handle)   /* OUT */
{
  ...
  /* sanity check */
  if ( !p_context || !p_parent_dir_handle || !p_fsalname || ( !p_children && children_count ) || !p_object_handle )
    ReturnCode(ERR_FSAL_FAULT, 0);
  ...
  switch (cmp) {
    case 0:
      ...
      if ( fsal_posixdb_consistency_check(&(p_children[count].handle.info), p_infofs) )
      {
        /* Entry not consistent */
        /* Delete the Handle entry, then add a new one (with a Parent entry) */
        stdb = fsal_posixdb_deleteHandle( p_context->p_conn, &(p_children[count].handle) );
...
# endif
  ...
  if ( !is_grp )
  {
    for ( i = 0; i < p_context->credential.nbgroups; i++ )
    {
      ...
      #ifdef _DEBUG_FSAL
        if (is_grp)
          DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "File belongs to user's alt group %d",
