...
fsal_status_t FSAL_DigestHandle(
  ...
  /* sanity checks */
  if (!p_in_fsal_handle || !out_buff || !p_expcontext) ReturnCode( ERR_FSAL_FAULT ,0 );
...
fsal_status_t FSAL_ExpandHandle(
  ...
  /* sanity checks */
  if (!p_out_fsal_handle || !in_buff || !p_expcontext) ReturnCode( ERR_FSAL_FAULT ,0 );
    ...
    if ( !STRCMP( key_name, "link_support" ) )
      {
        ...
        if ( bool == -1 )
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: 0 or 1 expected.",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
        }
    ...
    )
{
  ...
  if (out_parameter->fs_specific_info.dbparams.host[0] == '\0'
      || out_parameter->fs_specific_info.dbparams.dbname[0] == '\0') {
    DisplayLog("FSAL LOAD PARAMETER: DB_Host and DB_Name MUST be specified in the configuration file",
        CONF_LABEL_FS_SPECIFIC );
    ReturnCode( ERR_FSAL_NOENT, 0 );
  }
