...
)
{
  ...
  status = fsal_internal_getPathFromHandle(p_context, p_filehandle, 0, &fsalpath, &buffstat);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_getattrs);
...
fsal_status_t FSAL_setattrs(
  ...
  if ( FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_MODE ) )
  {
    ...
    if ( ! S_ISLNK( buffstat.st_mode ) )
    {
      ...
      if (rc) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_setattrs);
  ...
  if ( FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_GROUP ) )
  {
    ...
    if (p_context->credential.user != 0 && p_context->credential.user != buffstat.st_uid) Return( ERR_FSAL_PERM, 0, INDEX_FSAL_setattrs );
    int in_grp = 0;
    /* set in_grp */
    if (p_context->credential.group == attrs.group) in_grp = 1;
    else for (i=0; i < p_context->credential.nbgroups; i++) {
      if (in_grp = (attrs.group == p_context->credential.alt_groups[i])) break; 
    }
    if (p_context->credential.user != 0 && !in_grp) Return( ERR_FSAL_PERM, 0, INDEX_FSAL_setattrs );
  }
  
  if ( FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_OWNER | FSAL_ATTR_GROUP ) ) {
    TakeTokenFSCall();
    rc = lchown( fsalpath.path,
                 FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_OWNER ) ? (int) attrs.owner : -1,
                 FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_GROUP ) ? (int) attrs.group : -1  );
    ReleaseTokenFSCall();
    if (rc) Return(posix2fsal_error(errno), errno, INDEX_FSAL_setattrs);
  }
