...
void display_directory(fsal_posixdb_conn * p_conn, fsal_handle_t * p_handle_parent, char * basedir);
...
void add_dir(fsal_posixdb_conn *p_conn, char *path, fsal_handle_t *p_dir_handle) {
  ...
  if ( (dirp = opendir(path)) ) {
    while (!readdir_r(dirp, &dpe, &dp) && dp)
    {
      ...
      st = fsal_internal_posixdb_add_entry( p_conn, &fsalname, &info, p_dir_handle, &new_handle);
      if (FSAL_IS_ERROR(st)) {
...
void find(fsal_posixdb_conn *p_conn)
{
  ...
  st = fsal_posixdb_getInfoFromName(  p_conn,
                                      NULL, /* parent handle */
                                      NULL, /* filename */
                                      NULL,  /* path */
                                      &handle_root);
...
void display_directory(fsal_posixdb_conn * p_conn, fsal_handle_t * p_handle_parent, char * basedir)
{
    ...
    if (FSAL_POSIXDB_IS_ERROR(st)) {
      fprintf(stderr, "Error (%i/%i) while getting children of %s\n", st.major, st.minor, basedir);
      return;
    }
    for (i=0; i<count; i++) {
      printf("%llu %s/%s\n",p_children[i].handle.info.inode,basedir, p_children[i].name.name);
