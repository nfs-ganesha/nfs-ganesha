                             ...
                             fsal_path_t * p_path /* IN */  )
{
    ...
    if ( cache_array[i].is_set
         && cache_array[i].handle.id == p_handle->id && cache_array[i].handle.ts == p_handle->ts )
    {
...
fsal_posixdb_status_t fsal_posixdb_buildOnePath( fsal_posixdb_conn * p_conn,
                                                 fsal_handle_t * p_handle, fsal_path_t * p_path )
{
    unsigned int   shift;
    char          *new_pos;
    int            toomanypaths = 0;
                                                    ...
                                                    fsal_nodetype_t ftype )
{
    ...
    snprintf( query, 2048, "DELETE FROM Parent WHERE (handleid=%llu AND handlets=%u)", id, ts );
                                                 ...
                                                 int nlink )    /* IN */
{
    ...
    if ( nlink == 1 )
    {
        ...
        snprintf( query, 1024, "DELETE FROM Handle WHERE handleid=%llu AND handlets=%u", id, ts );
        st = db_exec_sql( p_conn, query, NULL );
        if ( FSAL_POSIXDB_IS_ERROR( st ) )
            return st;
        ...
        snprintf( query, 1024, "DELETE FROM Parent WHERE handleid=%llu AND handlets=%u", id, ts );
        st = db_exec_sql( p_conn, query, NULL );
        if ( FSAL_POSIXDB_IS_ERROR( st ) )
            return st;
#endif
    ...
    else
    {
        ...
        snprintf( query, 1024, "UPDATE Handle SET nlink=%u WHERE handleid=%llu AND handlets=%u",
                  nlink - 1, id, ts );
        st = db_exec_sql( p_conn, query, NULL );
        if ( FSAL_POSIXDB_IS_ERROR( st ) )
            return st;
                                                    ...
                                                    p_object_info /* IN */  )
{
    ...
    posixdb_internal_fillFileinfoFromStrValues( &infodb, row[2], row[3], row[4], row[5], row[6] );
    mysql_free_result( res );
