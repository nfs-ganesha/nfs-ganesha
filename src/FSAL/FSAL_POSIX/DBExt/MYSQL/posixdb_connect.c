                                            ...
                                            fsal_posixdb_conn ** p_conn )
{
    ...
    if ( dbparams->port[0] != '\0' )
    {
        ...
        {
            DisplayLog( "Numerical value expected for database port number (invalid value: %s)",
                        dbparams->port );
...
fsal_posixdb_status_t fsal_posixdb_initPreparedQueries( fsal_posixdb_conn * p_conn )
{
    ...
    do
    {
        ...
        {
            DisplayLog( "Connection to database lost in %s()... Retrying in %u sec.", __FUNCTION__,
                        retry );
            sleep( retry );
            retry *= 2;
