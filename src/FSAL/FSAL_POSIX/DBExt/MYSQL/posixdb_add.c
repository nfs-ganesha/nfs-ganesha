                                        ...
                                        fsal_handle_t * p_object_handle /* OUT */  )
{
   ...
   *******************/
    ...
    printf( "adding entry with parentid=%llu, id=%llu, name=%s\n",
            ...
            p_object_info ? p_object_info->inode : 0, p_filename ? p_filename->name : "NULL" );
#endif
   ...
   ***************************************/
    ...
    if ( p_parent_directory_handle )    /* the root has no parent */
    {
        snprintf( query, 4096,
                  ...
                  "FROM Handle WHERE handleid=%llu AND handlets=%u", p_parent_directory_handle->id,
   ...
   **********************************************************/
    ...
    if ( found )
    {
        ...
        p_object_handle->id = atoll( row[0] );
        p_object_handle->ts = atoi( row[1] );
        mysql_free_result( res );
    ...
    else                        /* no handle found */
    {
        ...
        snprintf( query, 4096, "INSERT INTO Handle(deviceid, inode, handlets, nlink, ctime, ftype) "
                  "VALUES ( %u, %llu, %u, %u, %u, %u)",
                  ( unsigned int ) p_object_info->devid,
                  ( unsigned long long ) p_object_info->inode,
                  ( unsigned int ) p_object_handle->ts,
                  ( unsigned int ) p_object_info->nlink,
   ...
   ************************************************/
    snprintf( query, 4096, "SELECT handleid, handlets "
              ...
              p_parent_directory_handle ? p_parent_directory_handle->id : p_object_handle->id,
              p_parent_directory_handle ? p_parent_directory_handle->ts : p_object_handle->ts,
              p_filename ? p_filename->name : "" );
    ...
    if ( add_parent_entry )
    {
        ...
        snprintf( query, 4096,
                  ...
                  p_parent_directory_handle ? p_parent_directory_handle->id : p_object_handle->id,
                  p_parent_directory_handle ? p_parent_directory_handle->ts : p_object_handle->ts,
                  p_filename ? p_filename->name : "", p_object_handle->id, p_object_handle->ts );
