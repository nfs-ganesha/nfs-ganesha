                                            ...
                                            fsal_name_t               * p_filename_new /* IN */)
{
  ...
  if ( !p_conn || !p_object_info || !p_parent_directory_handle_old || !p_filename_old || !p_parent_directory_handle_new || !p_filename_new )
  ...
  */
  ...
  snprintf(handleidparentold_str, MAX_HANDLEIDSTR_SIZE, "%lli", p_parent_directory_handle_old->id);
  snprintf(handletsparentold_str, MAX_HANDLETSSTR_SIZE, "%i", p_parent_directory_handle_old->ts);
  ...
  if ( !fsal_posixdb_GetInodeCache( p_parent_directory_handle_old )
    || fsal_posixdb_consistency_check(&(p_parent_directory_handle_old->info), p_object_info) )
  {
      ...
      if ( fsal_posixdb_consistency_check(&(p_parent_directory_handle_old->info), p_object_info) ) { 
        DisplayLog( "Consistency check failed while renaming a file : Handle deleted");
        st = fsal_posixdb_recursiveDelete(p_conn, PQgetvalue(p_res, 0, 0), PQgetvalue(p_res, 0, 1), FSAL_TYPE_DIR);
        PQclear( p_res );
  ...
  */
  ...
  snprintf(handleidparentnew_str, MAX_HANDLEIDSTR_SIZE, "%lli", p_parent_directory_handle_new->id);
  snprintf(handletsparentnew_str, MAX_HANDLETSSTR_SIZE, "%i", p_parent_directory_handle_new->ts);
  ...
  if ( PQntuples(p_res) > 0 )
  {
    ...
    st = fsal_posixdb_deleteParent( p_conn, PQgetvalue(p_res, 0, 0), PQgetvalue(p_res, 0, 1), 
