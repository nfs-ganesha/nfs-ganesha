...
#ifdef _DEBUG_FSAL      
    DisplayLog( "fsal_posixdb_CachePath: %u, %u = %s", (unsigned int)(p_handle->id), (unsigned int)(p_handle->ts),
             p_path->path );
#endif            
...
#ifdef _DEBUG_FSAL      
        DisplayLog( "fsal_posixdb_UpdateInodeCache: %u, %u (existing entry)", (unsigned int)(p_handle->id), (unsigned int)(p_handle->ts) );
#endif                    
...
#ifdef _DEBUG_FSAL      
    DisplayLog( "fsal_posixdb_UpdateInodeCache: %u, %u (new entry)", (unsigned int)(p_handle->id), (unsigned int)(p_handle->ts) );
#endif
...
#ifdef _DEBUG_FSAL      
            DisplayLog( "fsal_posixdb_GetInodeCache(%u, %u)", (unsigned int)(p_handle->id), (unsigned int)(p_handle->ts) );
#endif            
         ...
         && cache_array[i].handle.ts == p_handle->ts )
    {
        if ( cache_array[i].path_is_set )
        {
            ...
            DisplayLog( "fsal_posixdb_GetPathCache(%u, %u)=%s",
                    (unsigned int)p_handle->id, (unsigned int)p_handle->ts, p_path->path );
#endif                
            return TRUE;
        }
    }
...
fsal_posixdb_status_t fsal_posixdb_buildOnePath(fsal_posixdb_conn* p_conn, fsal_handle_t *p_handle, fsal_path_t *p_path)
{
  PGresult * p_res;
  ...
  while (handleid_str[0] != '\0') { 
    ...
    if (PQntuples(p_res) == 0) ReturnCode(ERR_FSAL_POSIXDB_NOENT, 0); /* not found */
    ...
    if (p_path->len + shift >= FSAL_MAX_PATH_LEN) ReturnCode(ERR_FSAL_POSIXDB_PATHTOOLONG, 0);
  ...
  if (ftype == FSAL_TYPE_DIR) {
    ...
    for(i=0; i<i_max; i++) {
      ...
      if (ftype_tmp == FSAL_TYPE_DIR) {
        st = fsal_posixdb_recursiveDelete( p_conn, PQgetvalue( p_res, i, 0) , PQgetvalue( p_res, i, 1), ftype_tmp);
      } else {
        st = fsal_posixdb_deleteParent( p_conn, 
                                        PQgetvalue( p_res, i, 0), /* handleidparent */
                                        PQgetvalue( p_res, i, 1), /* handletsparent */
                                        handleid_str, 
                                        handlets_str, 
                                        PQgetvalue( p_res, i, 3), /* filename */
                                                    ...
                                                    fsal_posixdb_fileinfo_t * p_object_info /* IN */) 
{
  ...
  posixdb_internal_fillFileinfoFromStrValues( &infodb, 
                                              PQgetvalue(p_res, 0, 2), /* no need to compare inode & devid, they are the same */
                                              PQgetvalue(p_res, 0, 3),
  ...
  switch (infodb.ftype) {
    case FSAL_TYPE_DIR:
      /* directory */
      st = fsal_posixdb_recursiveDelete( p_conn, 
                                         handleid_str, 
                                         handlets_str, 
                                         infodb.ftype );
      break;
