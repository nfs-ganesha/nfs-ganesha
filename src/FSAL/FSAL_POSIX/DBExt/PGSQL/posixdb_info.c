                                                    ...
                                                    fsal_handle_t           * p_handle /* OUT */ )
{
  ...
  posixdb_internal_fillFileinfoFromStrValues( &(p_handle->info), 
                                              PQgetvalue(p_res, 0, 2), 
  ...
  if (p_paths) {
    ...
    *p_count = PQntuples(p_res);
    ...
    for (i_path=0; i_path < *p_count; i_path++)
    {
      ...
      {      
          /* then concatenate the name of the file */
          if ( (tmp_len + 1 + strlen(PQgetvalue(p_res, i_path, 0)) >= FSAL_MAX_PATH_LEN) )
          {
            RollbackTransaction( p_conn, p_res );
            ReturnCode(ERR_FSAL_POSIXDB_PATHTOOLONG, 0);
          }
                                                       ...
                                                       fsal_handle_t      * p_parent_directory_handle /* OUT */)
{
  ...
  posixdb_internal_fillFileinfoFromStrValues( &(p_parent_directory_handle->info),
                                              PQgetvalue(p_res, 0, 3), 
                                              PQgetvalue(p_res, 0, 4),
                                              PQgetvalue(p_res, 0, 5), /* nlink */
