                                           ...
                                           fsal_posixdb_fileinfo_t     * p_object_info /* IN */)
{
  ...
  snprintf(handleidparent_str, MAX_HANDLEIDSTR_SIZE, "%lli", p_parent_directory_handle->id);
  ...
  st = fsal_posixdb_internal_delete(p_conn, handleidparent_str, handletsparent_str, p_filename->name, p_object_info);
                                                 ...
                                                 fsal_handle_t            * p_parent_directory_handle /* IN */)
{
  ...
  if (found) {
    /* entry found */
    st = fsal_posixdb_recursiveDelete(p_conn, handleid_str, handlets_str, FSAL_TYPE_DIR);
    if (FSAL_POSIXDB_IS_ERROR(st)) {
      RollbackTransaction( p_conn, p_res );
      return st; 
    }    
