                                        ...
                                        fsal_handle_t           * p_object_handle /* OUT */)
{
  ...
  if ( !p_conn || !p_object_info || !p_object_handle || (p_filename && !p_parent_directory_handle) || (!p_filename && p_parent_directory_handle) )
  ...
  if (p_parent_directory_handle) { /* the root has no parent */
    snprintf(handleidparent_str, MAX_HANDLEIDSTR_SIZE, "%llu", p_parent_directory_handle->id);
  ...
  if (found) { /* a Handle (that matches devid & inode) already exists */
    ...
    p_object_handle->id = atoll(handleid_str);
    ...
    strncpy(handleid_str, PQgetvalue(p_res, 0, 0), MAX_HANDLEIDSTR_SIZE);
    strncpy(handlets_str, PQgetvalue(p_res, 0, 1), MAX_HANDLETSSTR_SIZE);
    p_object_handle->id = atoll(PQgetvalue(p_res, 0, 0));
    PQclear( p_res );
  ...
  if (found) {
    ...
    if (   (fsal_u64_t) atoll(PQgetvalue(p_res, 0, 0)) != p_object_handle->id 
      ...
      if (found) { /* we have retrieved the handle information of the bad entry */
        ...
        st = fsal_posixdb_deleteParent( p_conn, bad_handleid_str, bad_handlets_str, 
                                        p_parent_directory_handle ? handleidparent_str:handleid_str,
                                        p_parent_directory_handle ? handletsparent_str:handlets_str,
      ...
      } else { /* the Handle line has been deleted */
        PQclear( p_res ); /* clear old res before a new query */
      }
