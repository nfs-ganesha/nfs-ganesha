...
fsal_posixdb_status_t fsal_posixdb_connect(fsal_posixdb_conn_params_t *dbparams, fsal_posixdb_conn **p_conn)
{
  ...
  *p_conn = PQsetdbLogin(
  ...
  {
    DisplayLogLevel( NIV_EVENT, "ERROR: could not connect to database : %s", PQerrorMessage(*p_conn)) ;
    PQfinish(*p_conn);
...
fsal_posixdb_status_t fsal_posixdb_initPreparedQueries( fsal_posixdb_conn *p_conn )
{
  ...
  p_res = PQprepare( p_conn,
                     "buildOnePathPL",
                     "SELECT buildOnePath($1::bigint, $2::int)", /* handleid, handlets */
                     2,
                     NULL);
  CheckCommand( p_res );
  PQclear(p_res);
  ...
  p_res = PQprepare( p_conn,
                     ...
                     "INSERT INTO Handle(deviceid, inode, handleTs, nlink, ctime, ftype) \
                      VALUES ($1::int, $2::bigint, $3::bigint, $4::smallint, $5::int, $6::int)",
                     6,
                     NULL);
  CheckCommand( p_res );
  PQclear(p_res);
  ...
  p_res = PQexec( p_conn,
                     "PREPARE \"countChildren\"(bigint, int) AS "\
                     "SELECT count(*) \
                      FROM Parent INNER JOIN Handle ON Handle.handleid=Parent.handleid AND Handle.handlets=Parent.handlets\
                      WHERE Parent.handleidparent=$1 AND Parent.handletsparent=$2 \
                        AND NOT (Parent.handleidparent = Parent.handleid AND Parent.handletsparent = Parent.handlets)");
