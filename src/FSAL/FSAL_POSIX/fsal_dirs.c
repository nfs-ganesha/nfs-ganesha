...
fsal_status_t FSAL_opendir(
  ...
  status = fsal_internal_getPathFromHandle(p_context, p_dir_handle, 1, &fsalpath, &buffstat);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_opendir);
...
attr_err:
      ...
      FSAL_SET_MASK( p_dir_attributes->asked_attributes,
...
fsal_status_t FSAL_readdir(
    fsal_dir_t            * p_dir_descriptor,     /* IN */
  ...
  if ( !p_dir_descriptor || !p_pdirent || !p_end_position
...
*p_nb_entries = 0;
  while (*p_nb_entries < max_dir_entries)
  {
    ...
    if (FSAL_IS_ERROR(st = FSAL_str2name(dp->d_name, FSAL_MAX_NAME_LEN, &(p_pdirent[*p_nb_entries].name)))) 
      goto readdir_error;
    memcpy(&fsalpath, &(p_dir_descriptor->path), sizeof(fsal_path_t));
    st = fsal_internal_appendFSALNameToFSALPath(&fsalpath, &(p_pdirent[*p_nb_entries].name));
    if (FSAL_IS_ERROR(st))
      goto readdir_error;
    ...
    /* check for "." and ".." */
    if ( !strcmp(dp->d_name, ".") ) {
      memcpy( &(p_pdirent[*p_nb_entries].handle), &(p_dir_descriptor->handle), sizeof(fsal_handle_t) );
...
#ifdef _USE_POSIXDB_READDIR_BLOCK
      if ( p_dir_descriptor->dbentries_count > -1 ) { /* look for the entry in dbentries */
...
fsal_status_t FSAL_closedir(
  ...
  #ifdef _USE_POSIXDB_READDIR_BLOCK
    if (p_dir_descriptor->p_dbentries) Mem_Free(p_dir_descriptor->p_dbentries); 
  #endif
