...
fsal_status_t FSAL_create(
  ...
  status = fsal_internal_getPathFromHandle(p_context, p_parent_directory_handle, 1, &fsalpath, &buffstat);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_create);

  /* Check the user can write in the directory, and check the setgid bit on the directory */
  if (buffstat.st_mode & S_ISGID) setgid_bit = 1;
  ...
  errsv = errno;
  if (fd == -1) goto releaseToken;
  /* close the file descriptor */
  rc = close(fd);
  errsv = errno;
  if (rc) goto releaseToken;
  /* stat the new file */
...
releaseToken:
  ...
  if (fd == -1 || rc) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_create);
  ...
  if (FSAL_IS_ERROR (status = fsal_internal_posixdb_add_entry( p_context->p_conn, p_filename, &info, p_parent_directory_handle, p_object_handle)))
    Return( status.major, status.minor, INDEX_FSAL_create);
  ...
  if (p_context->credential.user != geteuid()) {
    ...
    rc = lchown( fsalpath.path, p_context->credential.user, setgid_bit ? -1 : (int) p_context->credential.group );
    errsv = errno;
    ReleaseTokenFSCall();
    if ( rc ) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_create);
...
fsal_status_t FSAL_mkdir(
  ...
  /* build the destination path */
  status = fsal_internal_getPathFromHandle(p_context, p_parent_directory_handle, 1, &fsalpath, &buffstat);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_mkdir);
  ...
  if (!rc) rc = lstat(fsalpath.path, &buffstat);
  errsv = errno;
  
  ReleaseTokenFSCall();
  
  if ( rc ) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_mkdir);
    

  /* add the directory to the database */
  if (FSAL_IS_ERROR( status = fsal_internal_posix2posixdb_fileinfo(&buffstat, &info) ) )
    Return( status.major, status.minor, INDEX_FSAL_mkdir);
  if (FSAL_IS_ERROR (status = fsal_internal_posixdb_add_entry( p_context->p_conn, p_dirname, &info, p_parent_directory_handle, p_object_handle)))
    Return( status.major, status.minor, INDEX_FSAL_mkdir);
...
fsal_status_t FSAL_link(
    fsal_handle_t         * p_target_handle,        /* IN */
...
#ifdef _DEBUG_FSAL    
  fprintf(stderr, "linking %llu/%i to %llu.%i/%s \n", p_target_handle->id, p_target_handle->ts, p_dir_handle->id, p_dir_handle->ts, p_link_name->name);
#endif
    
  /* get the old path */
  status = fsal_internal_getPathFromHandle(p_context, p_target_handle, 0, &fsalpath_old, &buffstat);
  if (FSAL_IS_ERROR(status)) {
    Return(status.major, status.minor, INDEX_FSAL_link);
  }
  
  /* build the destination path and check permissions on the directory */
  status = fsal_internal_getPathFromHandle(p_context, p_dir_handle, 1, &fsalpath_new, &buffstat_dir);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_link);
  
  status = fsal_internal_testAccess( p_context, FSAL_W_OK | FSAL_X_OK, &buffstat_dir, NULL);
  if (FSAL_IS_ERROR(status)) Return(status.major,status.minor,INDEX_FSAL_link);
  ...
  errsv = errno;
  ReleaseTokenFSCall();
  if (rc) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_link);
  ...
  if (FSAL_IS_ERROR (status = fsal_internal_posixdb_add_entry( p_context->p_conn, p_link_name, &info, p_dir_handle, &newhandle)))
    Return(status.major, status.minor, INDEX_FSAL_link);
  ...
  if ( p_attributes ) {
    ...
    if ( FSAL_IS_ERROR( status ) )
    {
      ...
      FSAL_SET_MASK( p_attributes->asked_attributes,
          FSAL_ATTR_RDATTR_ERR );
    }
...
fsal_status_t FSAL_mknode(
  ...
  {
    case FSAL_TYPE_BLK:
        if ( !dev ) Return(ERR_FSAL_FAULT ,0 , INDEX_FSAL_mknode);
        unix_mode |= S_IFBLK;
    ...
    default:
      DisplayLogJdLevel( fsal_log, NIV_MAJOR, "Invalid node type in FSAL_mknode: %d", nodetype );
      Return(ERR_FSAL_INVAL, 0, INDEX_FSAL_mknode);        
  }
  
  
  /* build the destination path */
  status = fsal_internal_getPathFromHandle(p_context, parentdir_handle, 1, &fsalpath, &buffstat);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_mknode);
  ...
  if ( rc ) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_mknode);
  ...
  if (FSAL_IS_ERROR (status = fsal_internal_posixdb_add_entry( p_context->p_conn, p_node_name, &info, parentdir_handle, p_object_handle)))
    Return( status.major, status.minor, INDEX_FSAL_mknode);
  ...
  if ( node_attributes )
  {
      ...
      if ( FSAL_IS_ERROR( status ) )
      {
        ...
        FSAL_SET_MASK( node_attributes->asked_attributes,
            FSAL_ATTR_RDATTR_ERR );
      }
