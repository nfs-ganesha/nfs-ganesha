...
#define RCP_BUFFER_SIZE 10485760
  ...
  st = POSIXFSAL_open(filehandle, p_context, fs_flags, &fs_fd, NULL);
  ...
  while(!eof)
    {
      ...
      if(to_fs)                 /* from local filesystem */
        ...
        {
          fs_size = 0;
          st = POSIXFSAL_read(&fs_fd, NULL, RCP_BUFFER_SIZE, IObuffer, &fs_size, &eof);

          if(FSAL_IS_ERROR(st))
            break;              /* exit loop */

#ifdef  _DEBUG_FSAL
          DisplayLogJdLevel(fsal_log, NIV_FULL_DEBUG, "Size read from source: %llu",
          ...
          if(to_fs)             /* to FSAL filesystem */
            {
              ...
              st = POSIXFSAL_write(&fs_fd, NULL, local_size, IObuffer, &fs_size);
...
#ifdef  _DEBUG_FSAL
              DisplayLogJdLevel(fsal_log, NIV_FULL_DEBUG, "Size written to target: %llu",
