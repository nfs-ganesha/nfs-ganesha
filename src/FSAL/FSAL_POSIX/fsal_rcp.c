...
#define RCP_BUFFER_SIZE 10485760
...
#ifdef  _DEBUG_FSAL
  {
    ...
    DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,
        "Openning FSAL file with flags: %s", msg);
...
#ifdef  _DEBUG_FSAL
    DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,
        "Allocating IO buffer of size %llu", (unsigned long long) RCP_BUFFER_SIZE);
#endif
  ...
  while ( !eof )
  {
    ...
    else /* from FSAL filesystem */
    {
      ...
      if (FSAL_IS_ERROR( st )) break; /* exit loop */
    
#ifdef  _DEBUG_FSAL
      DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Size read from source: %llu", (unsigned long long)fs_size);
#endif
...
#ifdef  _DEBUG_FSAL
      DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Size written to target: %llu", (unsigned long long)local_size);
#endif
