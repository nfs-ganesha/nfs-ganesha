...
fsal_status_t FSAL_unlink(
  ...
  status = fsal_internal_getPathFromHandle(p_context, p_parent_directory_handle, 1, &fsalpath, &buffstat_parent);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_unlink);
  ...
  errsv = errno;
  ReleaseTokenFSCall();
  if ( rc ) Return(posix2fsal_error(errsv), errsv, INDEX_FSAL_unlink);
  ...
  if ( FSAL_IS_ERROR( status = fsal_internal_testAccess( p_context, FSAL_W_OK | FSAL_X_OK, &buffstat_parent, NULL ) ) ) {
    fsal_posixdb_cancelHandleLock( p_context->p_conn );
    Return(status.major, status.minor, INDEX_FSAL_unlink);
  }
  ...
  statusdb = fsal_posixdb_delete( p_context->p_conn, p_parent_directory_handle, p_object_name, &info );
  ...
  switch (statusdb.major) {
    ...
    case ERR_FSAL_POSIXDB_NOENT:
      ...
      break;
    default:
      if ( FSAL_IS_ERROR(status = posixdb2fsal_error(statusdb)) ) Return(status.major, status.minor, INDEX_FSAL_unlink);
  }
