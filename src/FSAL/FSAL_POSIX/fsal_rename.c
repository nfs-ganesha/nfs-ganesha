...
fsal_status_t FSAL_rename(
    fsal_handle_t         * p_old_parentdir_handle,      /* IN */
   ...
   **************************************/
  status = fsal_internal_getPathFromHandle(p_context, p_old_parentdir_handle, 1, &old_fsalpath, &old_parent_buffstat);
  if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_rename);
  ...
  if (! FSAL_handlecmp( p_old_parentdir_handle, p_new_parentdir_handle, &status )) {
    ...
    status = fsal_internal_getPathFromHandle(p_context, p_new_parentdir_handle, 1, &new_fsalpath, &new_parent_buffstat);
    if (FSAL_IS_ERROR(status)) Return(status.major, status.minor, INDEX_FSAL_rename);
  }
  ...
  errsv = errno;
  ReleaseTokenFSCall();
  if (rc) Return( posix2fsal_error(errsv), errsv, INDEX_FSAL_rename );
  ...
  if ( FSAL_IS_ERROR( status = fsal_internal_testAccess( p_context, FSAL_W_OK | FSAL_X_OK, &old_parent_buffstat, NULL) ) )
    Return(status.major, status.minor, INDEX_FSAL_rename);

  if ( FSAL_IS_ERROR( status = fsal_internal_testAccess( p_context, FSAL_W_OK | FSAL_X_OK, &new_parent_buffstat, NULL) ) )
    Return(status.major, status.minor, INDEX_FSAL_rename);
  ...
  if ( new_parent_buffstat.st_mode & S_ISVTX) { /* Sticky bit on the directory => the user who wants to delete the file must own it or its parent dir */
    TakeTokenFSCall();
  ...
  switch (statusdb.major) {
    ...
    case ERR_FSAL_POSIXDB_NOERR:
      break;
    default:
      if ( FSAL_IS_ERROR(status = posixdb2fsal_error(statusdb)) ) Return( status.major, status.minor, INDEX_FSAL_rename);
  }
