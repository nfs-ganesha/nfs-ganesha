  ...
  for(index = 0; index < XATTR_COUNT; index++)
    {
      if(do_match_type(xattr_list[index].flags, obj_hdl->attributes.type)
        ...
        {
          return tank_getextattr_value_by_id( obj_hdl, opctx, index, buffer_addr, buffer_size, p_output_size ) ;
        }
    }
					  ...
					  size_t buffer_size)
{
  ...
  return tank_setextattr_value( obj_hdl, opctx, name, buffer_addr, buffer_size, FALSE);
}
