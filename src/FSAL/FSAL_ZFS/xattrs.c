...
static int xattr_id_to_name( libzfswrap_vfs_t *p_vfs, 
                             creden_t * pcred,
                             inogen_t object,
                             ...
                             char *name)
{
  ...
  retval = libzfswrap_listxattr( p_vfs, pcred, object, (char **)&names,  &namesize ) ;
  if( retval )
 ...
 */
static int xattr_name_to_id( libzfswrap_vfs_t *p_vfs, 
                             creden_t * pcred,
                             inogen_t object,
                             char *name)
{
  ...
  retval = libzfswrap_listxattr( p_vfs, pcred, object, (char **)&name, &namesize ) ;
  if( retval )
      return -posix2fsal_error(retval) ;
				  ...
				  int *end_of_list)
{
  ...
  /* Workaround : no opctx is provided, using root for wanting of a better solution */
  cred.uid = 0 ;
  cred.gid = 0 ;
					...
					unsigned int *pxattr_id)
{
  ...
  if(!found)
    {
      rc = xattr_name_to_id( ZFSFSAL_GetVFS( obj_handle->handle ),
                             &cred, 
                             obj_handle->handle->zfs_handle,
                             (char *)xattr_name);
      if(rc)
					 ...
					 size_t *p_output_size)
{
  ...
  if((xattr_id < XATTR_COUNT)
    ...
    {
      /* get the value */
      retval = xattr_list[xattr_id].get_func(obj_hdl,
  ...
  for(index = 0; index < XATTR_COUNT; index++)
    {
      if(do_match_type(xattr_list[index].flags, obj_hdl->attributes.type)
        ...
        {
          return tank_getextattr_value_by_id( obj_hdl, index, buffer_addr, buffer_size, p_output_size ) ;
        }
    }
				   ...
				   int create)
{
  ...
  rc = libzfswrap_setxattr( ZFSFSAL_GetVFS( obj_handle->handle ),
                             &cred, 
                             obj_handle->handle->zfs_handle,
                             xattr_name,
                             buffer_addr);
					 ...
					 size_t buffer_size)
{
  ...
  return tank_setextattr_value( obj_hdl, name, buffer_addr, buffer_size, FALSE);
}
