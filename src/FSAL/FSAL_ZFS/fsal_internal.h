...
unsigned int ZFSFSAL_Handle_to_RBTIndex(fsal_handle_t * p_handle,
					unsigned int cookie);
...
fsal_status_t ZFSFSAL_SetDefault_FS_specific_parameter(fsal_parameter_t *
						       out_parameter);

fsal_status_t ZFSFSAL_load_FS_specific_parameter_from_conf(config_file_t
							   in_config,
							   fsal_parameter_t *
							   out_parameter);
...
fsal_status_t ZFSFSAL_getextattrs(fsal_handle_t * p_filehandle,	/* IN */
				  ...
				  p_object_attributes /* OUT */ );

fsal_status_t ZFSFSAL_commit(fsal_file_t * p_file_descriptor, fsal_off_t offset,
			     fsal_size_t size);
