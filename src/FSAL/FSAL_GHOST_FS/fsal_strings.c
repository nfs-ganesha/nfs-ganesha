...
fsal_status_t FSAL_str2name(
  ...
  /* sanity checks */
  if (!name || !string ) ReturnCode( ERR_FSAL_FAULT ,0 );
   ...
   */
  if (i >= FSAL_MAX_NAME_LEN ) ReturnCode( ERR_FSAL_NAMETOOLONG ,0 );
  
  (*name).len=i;

  /* copies the string */
  for (i=0;i<(*name).len;i++){ output[i] = string[i]; }
...
fsal_status_t FSAL_str2path(
  ...
  /* sanity checks */
  if (!p_path || !string ) ReturnCode( ERR_FSAL_FAULT ,0);
   ...
   */
  if (i >= FSAL_MAX_PATH_LEN ) ReturnCode( ERR_FSAL_NAMETOOLONG ,0);
  
  p_path->len=i;
  
  /* copies the string */
  for (i=0;i<p_path->len;i++){ output[i] = string[i]; }
  /* sets the terminating \0 */
  output[i]='\0';
...
fsal_status_t FSAL_name2str(
    ...
    /* sanity checks */
    if (!string || !p_name ) ReturnCode( ERR_FSAL_FAULT ,0);
...
fsal_status_t FSAL_path2str(
    ...
    /* sanity checks */
    if (!string || !p_path) ReturnCode( ERR_FSAL_FAULT ,0);
...
fsal_status_t FSAL_namecpy( fsal_name_t * p_tgt_name, fsal_name_t * p_src_name ){
    ...
    /* sanity check */
    if (!p_tgt_name || !p_src_name) ReturnCode( ERR_FSAL_FAULT ,0);
...
fsal_status_t FSAL_pathcpy( fsal_path_t * p_tgt_path, fsal_path_t * p_src_path ){
    ...
    /* sanity check */
    if (!p_tgt_path || !p_src_path) ReturnCode( ERR_FSAL_FAULT ,0);
