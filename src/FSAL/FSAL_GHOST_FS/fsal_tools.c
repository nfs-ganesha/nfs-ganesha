...
{
   unsigned int h = ~( cookie * alphabet_len + ( (unsigned int)p_handle->inode ^ (unsigned int)p_handle->magic ) );
   return h % index_size;
}
...
fsal_status_t FSAL_DigestHandle(
  ...
  /* sanity checks */
  if (!in_fsal_handle || !out_buff) ReturnCode( ERR_FSAL_FAULT ,0 );
  ...
  switch(output_type){   
    ...
    case  FSAL_DIGEST_FILEID2:      
         ...
         */
      {
        ...
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Source=%p Target=%#X",in_fsal_handle->inode,target32);
...
fsal_status_t FSAL_ExpandHandle(
  ...
  /* sanity checks */
  if (!out_fsal_handle || !in_buff) ReturnCode( ERR_FSAL_FAULT ,0 );
...
static int ParseGhostFsDirDef( char * string, ghostfs_dir_def_t * p_dir )
{
  ...
  modestr = strtok( NULL, ":" );
  if (modestr==NULL) return -1;
  
  ownstr = strtok( NULL, ":" );
  if (ownstr==NULL) return -1;
  
  grpstr = strtok( NULL, ":" );
  if (grpstr==NULL) return -1;
    ...
    )
{
  ...
  for ( var_index = 0; var_index < var_max ; var_index ++ )
  {
    ...
    if ( !STRCMP( key_name, "fs_root_mode" ) )
      ...
      {
        ghostfs_dir_def_t * p_dir = (ghostfs_dir_def_t*)Mem_Alloc( sizeof( ghostfs_dir_def_t ) );
        ghostfs_dir_def_t * p_cur;
            
        if ( p_dir == NULL )
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR ALLOCATING MEMORY");
        ...
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: 'path:mode:owner:group' expected",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
        }
