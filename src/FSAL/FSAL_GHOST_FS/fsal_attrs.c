...
)
{
  ...
  if ( rc ) Return( ghost2fsal_error(rc), rc, INDEX_FSAL_getattrs );
  ...
  if ( unsupp_attr ) {
    ...
    object_attributes->asked_attributes = object_attributes->asked_attributes & ( ~ unsupp_attr ) ;
  }
...
#define SETTABLE_ATTRIBUTES ( FSAL_ATTR_SIZE |\
  ...
  if ( FSAL_TEST_MASK(attrib_set->asked_attributes,
      FSAL_ATTR_SIZE))
  {
  ...
  if ( rc ) Return( ghost2fsal_error(rc), rc, INDEX_FSAL_setattrs );
