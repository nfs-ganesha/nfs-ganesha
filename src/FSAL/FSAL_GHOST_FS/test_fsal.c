...
void usage(){
  ...
  fprintf(stderr,"\t\t8 - mkdir/rmdir [optional base dir argument]\n");
...
if (test[0]=='3') {
  ...
  /* lookup path */
  if (FSAL_IS_ERROR(
        st = FSAL_str2path("/cea/prot/lama",15,&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
...
if (test[0]=='8') {
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2path(fixpath,strlen(fixpath)+1,&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
