...
static int CreateInitDir( ghostfs_dir_def_t * p_dir )
{
  ...
  if (GHOSTFS_GetRoot( &handle )) return -1;
    
  cur_str = buff+1;
  
  cur_str = strtok( cur_str, "/" );
  if ( cur_str == NULL ) return -1;
  ...
  do
  {
    ...
    {
      DisplayLogJdLevel( fsal_log, NIV_EVENT,"FSAL: Creating predefined directory '%s'", cur_str );
      ...
      rc = GHOSTFS_MkDir( handle, cur_str,
                          p_dir->dir_owner,
                          p_dir->dir_group,
                          fsal2ghost_mode(p_dir->dir_mode),
                          &next_handle,
                          NULL );
    }
...
fsal_status_t  FSAL_Init(
  ...
  if (rc)  Return( ghost2fsal_error(rc), rc, INDEX_FSAL_Init);
  
  /* proceeds FSAL initialization */
  switch( (status = fsal_internal_init_global(
            &(init_info->fsal_info), &(init_info->fs_common_info))).major          
        )
  {
  ...
  for ( p_cur = init_info->fs_specific_info.dir_list ;
        p_cur != NULL ;
        p_cur = p_cur->next )
  {
    rc = CreateInitDir( p_cur );
    if (rc) DisplayLogJdLevel( fsal_log, NIV_CRIT,"FSAL: /!\\ WARNING /!\\ Could not create init dir '%s'",
        p_cur->path );
  }
