...
)
{
  ...
  if ( rc ) Return( ghost2fsal_error(rc) ,rc, INDEX_FSAL_opendir);
...
fsal_status_t FSAL_readdir(
  ...
  rc = GHOSTFS_Seekdir( &(dir_descriptor->dir_descriptor),
                                 start_position.cookie );
  
  if ( rc ) Return( ghost2fsal_error(rc) ,rc, INDEX_FSAL_readdir);
  ...
  *end_of_dir = FALSE;
  ...
  while ( *nb_entries < max_entries ){
    ...
    if ( rc == ERR_GHOSTFS_ENDOFDIR )
    {
        ...
        if (last_ent) last_ent->nextentry = NULL;
        (*end_of_dir) = TRUE;
    ...
    if (last_ent) last_ent->nextentry = curr_ent;
