...
{
  { "hello_world", get_void_attr, NULL, XATTR_FOR_ALL | XATTR_RO }
};
...
)
{
  ...
  fsal_status_t st;
  fsal_attrib_list_t file_attrs;
  GHOSTFS_Attrs_t attrs;
  fsal_nodetype_t objtype;
  ...
  /* get object type */
  rc = GHOSTFS_GetAttrs( (GHOSTFS_handle_t)(*p_objecthandle) , &attrs);  
  if ( rc ) Return( ghost2fsal_error(rc), rc, INDEX_FSAL_ListXAttrs );
    
  objtype = ghost2fsal_type( attrs.type );
  ...
  for ( index = cookie, out_index = 0 ;
        ...
        index ++ )
  {
    if ( do_match_type( xattr_list[index].flags, objtype ) )
    {
...
)
{
  ...
  /* get object type */
  rc = GHOSTFS_GetAttrs( (GHOSTFS_handle_t)(*p_objecthandle) , &attrs);  
  if ( rc ) Return( ghost2fsal_error(rc), rc, INDEX_FSAL_GetXAttrValue );
    
  objtype = ghost2fsal_type( attrs.type ); 
  
  /* check that this index match the type of entry */
  if ( xattr_id >= XATTR_COUNT
      || !do_match_type( xattr_list[xattr_id].flags, objtype ) )
  {
