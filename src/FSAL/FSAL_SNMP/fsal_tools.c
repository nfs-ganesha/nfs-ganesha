...
int FSAL_handlecmp( fsal_handle_t * handle1, fsal_handle_t * handle2,
  ...
  *status =  FSAL_STATUS_NO_ERROR;
  ...
  if ( handle1->oid_len != handle2->oid_len ) return ( handle1->oid_len - handle2->oid_len ) ;          
    ...
    case  FSAL_DIGEST_NFSV4:
      ...
      for ( i = 0; i < nb_oids; i++ )
      {
          ...
          if ( in_fsal_handle->oid_tab[i+root_len] > MAX_SHORT_VAL )
          {
                ...
                curr_addr[3] = (unsigned char)( in_fsal_handle->oid_tab[i+root_len]        & 0xFF); 
                curr_addr[2] = (unsigned char)((in_fsal_handle->oid_tab[i+root_len] >> 8)  & 0xFF); 
                curr_addr[1] = (unsigned char)((in_fsal_handle->oid_tab[i+root_len] >> 16) & 0xFF); 
                curr_addr[0] = (unsigned char)((in_fsal_handle->oid_tab[i+root_len] >> 24) & 0xFF); 
          ...
          else if ( in_fsal_handle->oid_tab[i+root_len] > MAX_CHAR_VAL )
          {
                ...
                curr_addr[1] = (unsigned char)( in_fsal_handle->oid_tab[i+root_len]        & 0xFF);
                curr_addr[0] = (unsigned char)((in_fsal_handle->oid_tab[i+root_len] >> 8)  & 0xFF);
          ...
          else /* write 1 byte */
          {
                ...
                curr_addr[0] = (unsigned char)( in_fsal_handle->oid_tab[i+root_len] & 0xFF );
                curr_addr ++;
          }
...
fsal_status_t FSAL_ExpandHandle(
  ...
  switch(in_type){
    ...
    case  FSAL_DIGEST_NFSV4:
      ...
      root_len = p_expcontext->root_handle.oid_len; 
      memcpy( out_fsal_handle->oid_tab, p_expcontext->root_handle.oid_tab, root_len * sizeof(oid) ); 
      ...
      for ( i = 0; i < p_digest->relative_oid_len ; i++ )
      {
          ...
          {
             /* read 4 bytes */
             out_fsal_handle->oid_tab[i+root_len] = ( (unsigned long)curr_addr[3] +  ((unsigned long)curr_addr[2] << 8 )
                                                    + ( (unsigned long)curr_addr[1] << 16 ) + ( (unsigned long)curr_addr[0] << 24 ));
             curr_addr += 4;
             curr_int_idx ++;
          }
          else if ( (curr_short_idx < short_tab_indexes + nb_short) && ( i == *curr_short_idx ) )
          {
             /* read 2 bytes */
             out_fsal_handle->oid_tab[i+root_len] = ( (unsigned long)curr_addr[1] +  ((unsigned long)curr_addr[0] << 8 ));
             curr_addr += 2;
             curr_short_idx ++;
          }
          else
    ...
    )
{
  ...
  for ( var_index = 0; var_index < var_max ; var_index ++ )
  {
    ...
    if ( !STRCMP( key_name, "snmp_version" ) )
    {
       ...
       if ( version == -1 )
       {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: 1, 2c or 3 expected.",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
       }
    ...
    {
       strncpy( out_parameter->fs_specific_info.community , key_value, COMMUNITY_MAX_LEN );  
    }
    ...
    {
	   strncpy( out_parameter->fs_specific_info.auth_proto, key_value, FSAL_MAX_PROTO_LEN );  
    }
    else if ( !STRCMP( key_name, "enc_proto") )
    {
	   strncpy( out_parameter->fs_specific_info.enc_proto, key_value, FSAL_MAX_PROTO_LEN );  
    }
    else if ( !STRCMP( key_name, "username") )
    {
	   strncpy( out_parameter->fs_specific_info.username, key_value, FSAL_MAX_USERNAME_LEN );  
    }
    else if ( !STRCMP( key_name, "auth_phrase") )
    {
	   strncpy( out_parameter->fs_specific_info.auth_phrase, key_value, FSAL_MAX_PHRASE_LEN );  
    }
    else if ( !STRCMP( key_name, "enc_phrase") )
    {
	   strncpy( out_parameter->fs_specific_info.enc_phrase, key_value, FSAL_MAX_PHRASE_LEN );  
    }
    else
    {
      DisplayLog("FSAL LOAD PARAMETER: ERROR: Unknown or unsettable key: %s (item %s)",
