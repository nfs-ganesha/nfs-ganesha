...
#define TEST_SNMP_OID "/" 
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_lookup(NULL,NULL,&op_ctx,&root_handle,NULL))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_getattrs(&root_handle,&op_ctx,&attribs))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
...
if (test[0]=='3') {
  ...
  /* lookup root */
  if (FSAL_IS_ERROR(
        st = FSAL_str2path("/",2,&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
        st = FSAL_lookupPath(&path,&op_ctx,&handle,&attribs))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }  
...
#define MY_SNMP_VAR "/iso/org/dod/internet/mgmt/mib-2/system/sysUpTime/0"
  /* lookup path */
  if (FSAL_IS_ERROR(
        st = FSAL_str2path(MY_SNMP_VAR,strlen(MY_SNMP_VAR)+1,&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
  ...
  while (!error && !eod){
    ...
    for (i=0;(!error) && (i<number);i++){
      ...
      if (st1.major != st2.major){
        printf("Error : different access permissions given by FSAL_access and FSAL_test_access : %d <>%d\n",
            st1.major,st2.major 
            );
      }
...
if (test[0]=='7') {
  ...
  if (size1 <= 0 ) printf("***** ERROR: source size=0 !!!\n");
  
  if (size1 != size3) printf("***** ERROR: source size <> target size\n");
  else printf("OK: source size = target size\n");
  
  if ((size1 * 2) != size2) printf("***** ERROR: hexa size <> 2 * source size\n");
  else printf("OK: hexa size = 2 * source size\n");
  
  if (strcmp(test_string,string)) printf("***** ERROR: source string <> target string\n");
  else printf("OK: source string = target string\n");  
...
#define TEST8_PATH "/iso/org/dod/internet/mgmt/mib-2/system"
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2path(TEST8_PATH,sizeof(TEST8_PATH),&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2name("tests_GANESHA",30,&name))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  ...
  if (FSAL_IS_ERROR(
       st = FSAL_mkdir( &handle, &name, &op_ctx,
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2name("subdir_GANESHA",30,&subdir_name))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  
  if (FSAL_IS_ERROR(
       st = FSAL_mkdir( &dir_hdl, &subdir_name, &op_ctx,
                        FSAL_MODE_RUSR|FSAL_MODE_WUSR
                        |FSAL_MODE_XUSR|FSAL_MODE_RGRP
                        |FSAL_MODE_WGRP,
                        &subdir_hdl,
  ...
  if (FSAL_IS_ERROR(
       st = FSAL_unlink( &handle, &name, &op_ctx,
                         &attribs ) )){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  } else {
  ...
  if (FSAL_IS_ERROR(
       st = FSAL_unlink( &dir_hdl, &subdir_name, &op_ctx,
                         &attribs ) )){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  } else {
...
if (test[0]=='9') {
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2path("/cea/prot/S/lama/s8/leibovic",40,&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2name("tests_GANESHA_setattrs",30,&name))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  ...
  if (FSAL_IS_ERROR(
       st = FSAL_create( &handle, &name, &op_ctx,
                        FSAL_MODE_RUSR|FSAL_MODE_WUSR
                        |FSAL_MODE_XUSR|FSAL_MODE_RGRP
                        |FSAL_MODE_WGRP,
                        &dir_hdl,
...
if (test[0]=='A') {
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_str2path(MY_SNMP_VAR, sizeof(MY_SNMP_VAR),&path))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
  }
  attribs.asked_attributes = mask;
  if (FSAL_IS_ERROR(
...
if (test[0]=='B') {
  ...
  if (FSAL_IS_ERROR(
        st = FSAL_dynamic_fsinfo(&root_handle,&op_ctx,&dyninfo))){
    DisplayErrorJd(log_desc,ERR_FSAL,st.major,st.minor);
    exit( st.major );
  }
  ...
  printf("time_delta = %u.%u\n", dyninfo.time_delta.seconds, dyninfo.time_delta.nseconds);
