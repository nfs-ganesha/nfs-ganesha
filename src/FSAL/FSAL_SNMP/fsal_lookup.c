...
fsal_status_t FSAL_lookup (
  ...
  memset( object_handle, 0, sizeof( fsal_handle_t ) );
  ...
  else /* this is a real lookup(parent, name)  */
  {
    ...
    if ( !FSAL_namecmp( p_filename, &FSAL_DOT )
        || ( !FSAL_namecmp( p_filename, &FSAL_DOT_DOT ) && parent_directory_handle->oid_len == 0 ))
    {
        FSAL_OID_DUP( object_handle, parent_directory_handle->oid_tab, parent_directory_handle->oid_len );
        object_handle->object_type_reminder = parent_directory_handle->object_type_reminder;

        if ( object_attributes )
        {
                rc = snmp2fsal_attributes( object_handle, NULL, GetMIBNode( p_context, object_handle, TRUE ), object_attributes );  
    ...
    if ( !FSAL_namecmp( p_filename, &FSAL_DOT_DOT ) )
    {
        ...
        FSAL_OID_DUP( object_handle, parent_directory_handle->oid_tab, parent_directory_handle->oid_len - 1 );
    ...
    for ( curr_child = GetMIBChildList( p_context, parent_directory_handle );
          curr_child != NULL;
          curr_child = curr_child->next_peer )
    {
#ifdef _DEBUG_FSAL
        printf("CHILD = %s (%lu)\n", curr_child->label, curr_child->subid );
#endif

        if ( curr_child->label && !strncmp( p_filename->name, curr_child->label, FSAL_MAX_NAME_LEN ) )
        {
#ifdef _DEBUG_FSAL
                printf("MATCHES !!!!\n");
#endif
    ...
    if ( !type_ok )
    {
        ...
        * and for finding object type */
        ...
        /* call to snmpget */
        rc = IssueSNMPQuery( p_context, object_handle->oid_tab, object_handle->oid_len, &query_desc );
             ...
             || p_context->snmp_response->variables->type == SNMP_ENDOFMIBVIEW )
        {
          ...
          {
            case -1 : Return( ERR_FSAL_IO, snmp_errno ,INDEX_FSAL_lookup);
            case 0: Return( ERR_FSAL_NOENT, 0 ,INDEX_FSAL_lookup);
            case 1 :
                object_handle->object_type_reminder = FSAL_NODETYPE_NODE ;
                break;
            default :
                DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: unexpected return value from HasSNMPChild" ); 
                Return( ERR_FSAL_SERVERFAULT, 0 ,INDEX_FSAL_lookup);
          }
        }
        else
     ...
     */
    if ( !found ) curr_child = GetMIBNode( p_context, object_handle, TRUE );
        
    if (object_attributes)
    {
       rc = snmp2fsal_attributes( object_handle,
                (object_handle->object_type_reminder == FSAL_NODETYPE_LEAF ? p_context->snmp_response->variables : NULL ), 
                curr_child, object_attributes );    
...
fsal_status_t FSAL_lookupPath (
  ...
  if ( strncmp( p_context->export_context->root_path.path, p_path->path,  p_context->export_context->root_path.len ) )
  {
        DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: FSAL_lookupPath was called on a path that doesn't match export info (path=%s, export=%s)",
  ...
  {
     rc = snmp2fsal_attributes( &out_hdl, NULL, p_context->export_context->root_mib_tree, object_attributes );
