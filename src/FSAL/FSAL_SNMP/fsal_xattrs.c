...
int get_oid_text( fsal_handle_t     * p_objecthandle,  /* IN */
                   ...
                   size_t            * p_output_size )  /* OUT */
{
  ...
  snprint_objid( (char*)buffer_addr, buffer_size, p_objecthandle->oid_tab, p_objecthandle->oid_len );
...
int get_oid_numeric( fsal_handle_t     * p_objecthandle,  /* IN */
                     fsal_op_context_t * p_context,       /* IN */
                     caddr_t             buffer_addr,     /* IN/OUT */
                     size_t              buffer_size,     /* IN */
                     size_t            * p_output_size )  /* OUT */
...
)
{
  ...
  for ( index = cookie, out_index = 0 ;
        ...
        index ++ )
  {
    if ( do_match_type( xattr_list[index].flags, intern2extern_type( p_objecthandle->object_type_reminder) ) )
    {
...
)
{
  ...
  /* check that this index match the type of entry */
  if ( xattr_id >= XATTR_COUNT
      || !do_match_type( xattr_list[xattr_id].flags, intern2extern_type( p_objecthandle->object_type_reminder) ) )
  {
