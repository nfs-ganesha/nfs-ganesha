...
int IssueSNMPQuery( fsal_op_context_t * p_context, oid * oid_tab, int oid_len, fsal_request_desc_t * p_req_desc )
{
   int rc;
   ...
   {
        case SNMP_MSG_GET:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing SNMP GET request on %s", oid_str);
                /* for a get request, the value field is empty */
                snmp_add_null_var(p_context->snmp_request, oid_tab, oid_len);
                break;

        case SNMP_MSG_GETNEXT:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing SNMP GETNEXT request on %s", oid_str);
                /* for a get request, the value field is empty */
                snmp_add_null_var(p_context->snmp_request, oid_tab, oid_len);
                break;

        case SNMP_MSG_SET:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing SNMP SET request on %s (type '%c', value='%s')", oid_str,
                                        p_req_desc->SET_REQUEST_INFO.type, p_req_desc->SET_REQUEST_INFO.value );
                /* for a set request, we set the value and type */
                snmp_add_var(p_context->snmp_request, oid_tab, oid_len, p_req_desc->SET_REQUEST_INFO.type,
                                p_req_desc->SET_REQUEST_INFO.value);
                break;

        case SNMP_MSG_TRAP:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing a SNMP TRAP (v1) on %s", oid_str);
                break;

        case SNMP_MSG_GETBULK:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing SNMP GETBULK request on %s (max-repetitions=%ld)", oid_str,
                                         p_req_desc->GETBULK_REQUEST_INFO.max_repetitions);
                /* in case of a GETBULK request, we set the request options */
                p_context->snmp_request->non_repeaters = p_req_desc->GETBULK_REQUEST_INFO.non_repeaters ;
                p_context->snmp_request->max_repetitions = p_req_desc->GETBULK_REQUEST_INFO.max_repetitions ;
        ...
        case SNMP_MSG_INFORM:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing a SNMP INFORM message on %s", oid_str);
                break;

        case SNMP_MSG_TRAP2:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing a SNMP TRAP (v2,3) on %s", oid_str);
                break;

        case SNMP_MSG_REPORT:
                DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Issuing a SNMP REPORT message on %s", oid_str);
                break;

        default:
                DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: Unknown request %#X on %s", p_req_desc->request_type , oid_str);
                return SNMPERR_MAX - 1 ; 

   }

   /* issue the message and wait for server response  */
   rc = snmp_synch_response(p_context->snmp_session,p_context->snmp_request, &(p_context->snmp_response));
...
struct tree * GetMIBNode( fsal_op_context_t * p_context, fsal_handle_t * p_handle, int return_nearest_parent )
{
    if ( !p_context || !p_handle ) return NULL;
    ...
    return FSAL_GetTree( p_handle->oid_tab, p_handle->oid_len, p_context->export_context->root_mib_tree, return_nearest_parent );
}
...
struct tree * GetMIBChildList( fsal_op_context_t * p_context, fsal_handle_t * p_handle )
{
    ...
    obj_tree = FSAL_GetTree( p_handle->oid_tab, p_handle->oid_len, p_context->export_context->root_mib_tree, FALSE ); 
    
    if ( obj_tree == NULL ) return NULL;
...
{
   return ( ( child_oid_len > parent_oid_len ) && ( !memcmp( parent_oid, child_oid, parent_oid_len * sizeof(oid)) ) );
}
...
int HasSNMPChilds( fsal_op_context_t * p_context, fsal_handle_t * p_handle )
{
        ...
        return IsSNMPChild( p_handle->oid_tab, p_handle->oid_len,
                            p_context->snmp_response->variables->name, p_context->snmp_response->variables->name_length ) ;
...
int fsal_oid_cmp( oid * oid_tab1, oid * oid_tab2, unsigned int count )
{
    ...
    for ( i = 0; i < count; i++ )
    {
        if ( oid_tab1[i] < oid_tab2[i] ) return -1;
        if ( oid_tab1[i] > oid_tab2[i] ) return  1;
    }
    return 0;
}
...
long StrToSNMPVersion( char * str )
{
  ...
  if ( ! strcmp( str , "1" ) ) return SNMP_VERSION_1 ;
  if ( ! strcasecmp( str , "2c" ) ) return SNMP_VERSION_2c ;
  if ( ! strcmp( str , "3" ) ) return SNMP_VERSION_3 ;
