...
int snmp2fsal_error(int snmp_error)
{
   switch( snmp_error )
   {
        ...
        case SNMP_ERR_NOERROR:  return ERR_FSAL_NO_ERROR;
        case SNMP_ERR_TOOBIG:   return ERR_FSAL_TOOSMALL;
        case SNMP_ERR_NOSUCHNAME: return ERR_FSAL_NOENT;
        case SNMP_ERR_BADVALUE: return ERR_FSAL_INVAL;
        case SNMP_ERR_READONLY: return ERR_FSAL_ACCESS;
        case SNMP_ERR_GENERR:   return ERR_FSAL_IO;
        case SNMP_ERR_NOACCESS: return ERR_FSAL_PERM;
        case SNMP_ERR_WRONGTYPE: return ERR_FSAL_INVAL;
        case SNMP_ERR_WRONGLENGTH: return ERR_FSAL_INVAL;
        case SNMP_ERR_WRONGENCODING: return ERR_FSAL_INVAL;

        /* for bad range values, we choose returning DQUOT  */
        case SNMP_ERR_WRONGVALUE: return ERR_FSAL_DQUOT;

        case SNMP_ERR_NOCREATION: return ERR_FSAL_NOENT;
        case SNMP_ERR_INCONSISTENTVALUE: return ERR_FSAL_INVAL; 
        case SNMP_ERR_RESOURCEUNAVAILABLE: return ERR_FSAL_PERM;
        case SNMP_ERR_COMMITFAILED: return ERR_FSAL_IO;
        case SNMP_ERR_UNDOFAILED:   return ERR_FSAL_IO;
        case SNMP_ERR_AUTHORIZATIONERROR: return ERR_FSAL_ACCESS;
        case SNMP_ERR_NOTWRITABLE: return ERR_FSAL_ACCESS;
        case SNMP_ERR_INCONSISTENTNAME: return ERR_FSAL_NOENT; 

        /* snmp_errno (negative codes)  */

        case SNMPERR_TOO_LONG : return ERR_FSAL_TOOSMALL;
        case SNMPERR_BAD_ASN1_BUILD : return ERR_FSAL_NOENT;
        case SNMPERR_BAD_PARSE : return ERR_FSAL_NOENT;

        case SNMPERR_BAD_COMMUNITY : return ERR_FSAL_PERM;
        
        case SNMPERR_NOAUTH_DESPRIV : return ERR_FSAL_SEC;
        
        case SNMPERR_UNKNOWN_USER_NAME : return ERR_FSAL_PERM;

        case SNMPERR_BAD_SEC_NAME : return ERR_FSAL_SEC;
        case SNMPERR_BAD_SEC_LEVEL : return ERR_FSAL_SEC;
        case SNMPERR_UNKNOWN_SEC_MODEL : return ERR_FSAL_SEC;
        case SNMPERR_UNSUPPORTED_SEC_LEVEL : return ERR_FSAL_SEC;
        case SNMPERR_AUTHENTICATION_FAILURE : return ERR_FSAL_SEC;
        case SNMPERR_DECRYPTION_ERR : return ERR_FSAL_SEC;
        case SNMPERR_KRB5 : return ERR_FSAL_SEC;
        
        /* for bad range values, we choose returning DQUOT  */
        case SNMPERR_RANGE : return ERR_FSAL_DQUOT;
        ...
        case SNMPERR_UNKNOWN_OBJID : return ERR_FSAL_NOENT;
        case SNMPERR_MALLOC : return ERR_FSAL_NOMEM;
...
{
        /* sanity check */
        if ( !p_out_handle || !p_in_var ) return ERR_FSAL_FAULT ;
 ...
 */
int snmp_object2name( netsnmp_variable_list * p_in_var, struct tree * p_in_node, fsal_handle_t * p_handle, fsal_name_t * p_out_name )
{
       fsal_status_t st;

       char tmp_name[FSAL_MAX_NAME_LEN] = "";
       /* sanity check */
       if ( !p_out_name ) return ERR_FSAL_FAULT ;
       ...
       if ( p_in_node && p_in_node->label && p_in_node->label[0] != '\0' )
          ...
          snprintf( tmp_name, FSAL_MAX_NAME_LEN, "%lu", p_in_var->name[ p_in_var->name_length - 1 ] );
       else if ( p_handle && p_handle->oid_len > 0 )
          snprintf( tmp_name, FSAL_MAX_NAME_LEN, "%lu", p_handle->oid_tab[p_handle->oid_len-1] );
       else
          return ERR_FSAL_SERVERFAULT ; 
...
static size_t Timeticks2Str( long timetick , char * out_str, size_t buflen )
{
    ...
    return snprintf( out_str, buflen, "%lu days, %02lu:%02lu:%02lu.%02lu", days, hours, minutes, seconds, hseconds );
}


/* print the object value to a string, and return its length (the buffer ends with '\n\0', like /proc files)  */
int snmp_object2str( netsnmp_variable_list * p_in_var, char * p_out_string, size_t * in_out_len )
{
        size_t written = 0;
        ...
        /* sanity check  */ 
        if ( !p_in_var ) return ERR_FSAL_FAULT;
        ...
        switch( p_in_var->type )
        {
           ...
           case ASN_OBJECT_ID:
                if ( p_in_var->val.objid )
                        snprint_objid( tmp_buf, FSALSNMP_MAX_FILESIZE, p_in_var->val.objid, p_in_var->val_len / sizeof(oid) );
                else
           ...
           case ASN_TIMETICKS:
                ...
                written = snprintf( p_out_string, *in_out_len, "%ld (%s)\n", *p_in_var->val.integer, tmp_buf ); 
                break;

           case ASN_OPAQUE:
                if (  p_in_var->val.string )
                        snprintmem(  tmp_buf, FSALSNMP_MAX_FILESIZE, p_in_var->val.string, p_in_var->val_len );
                else
           ...
           case ASN_COUNTER64:
                ...
                {
                        int64 = (((unsigned long long) p_in_var->val.counter64->high) << 32) | (unsigned long long) p_in_var->val.counter64->low ; 
           ...
           default:
                written = snprintf(  p_out_string, *in_out_len, "(unsupported object type %#X)\n", p_in_var->type );
        ...
        {
                DisplayLogJdLevel( fsal_log, NIV_MAJOR, "Warning: actual datasize is over client buffer limit (%llu > %llu)", written, *in_out_len );
                written = *in_out_len;
        }
...
fsal_accessmode_t snmp_object2access_mode( nodetype_t obj_type, struct tree * p_in_node )
{
   ...
   else if ( p_in_node )
   {
        switch( p_in_node->access )
        {
          ...
          default :
                mode = 0;
                DisplayLogJdLevel( fsal_log, NIV_MAJOR, "Warning: unsupported access mode %#X", p_in_node->access );  
        }
   }
   else
   {
...
fsal_nodetype_t intern2extern_type( nodetype_t internal_type )
{
        switch( internal_type )
        {
          ...
          default :
                DisplayLogJdLevel( fsal_log, NIV_CRIT, "Error: unexpected internal type %d", internal_type );  
                return 0;
 ...
 * /!\ the p_in_node is the PARENT NODE (access right are stored in the parent node)
 */
int snmp2fsal_attributes( fsal_handle_t * p_handle, netsnmp_variable_list * p_var, struct tree * p_in_node, fsal_attrib_list_t * p_fsalattr_out )
{
  fsal_attrib_mask_t supp_attr, unsupp_attr ;
  int rc;

  /* sanity checks */
  if ( ! p_handle || !p_fsalattr_out ) return ERR_FSAL_FAULT; 

  if( p_fsalattr_out->asked_attributes == 0 )
    {
       p_fsalattr_out->asked_attributes =  global_fs_info.supported_attrs ;

       DisplayLogJdLevel( fsal_log, NIV_MAJOR, "Error: p_fsalattr_out->asked_attributes was set to 0 in snmp2fsal_attributes line %d, file %s: retrieving all supported attributes", __LINE__, __FILE__ ) ;
    }
  ...
  if( unsupp_attr )
  {
    DisplayLogJdLevel( fsal_log, NIV_MAJOR,
                  "Unsupported attributes: %#llX removing it from asked attributes ",unsupp_attr);
    p_fsalattr_out->asked_attributes = p_fsalattr_out->asked_attributes & ( ~ unsupp_attr ) ;
  }
       ...
       FSAL_TEST_MASK(p_fsalattr_out->asked_attributes, FSAL_ATTR_SPACEUSED ) )
  {
        if ( p_handle->object_type_reminder == FSAL_NODETYPE_LEAF )
        {
                ...
                if ( rc != 0 )
                {
                   DisplayLogJdLevel( fsal_log, NIV_CRIT, "Error %d converting object data to string", rc );
                   return rc ;
                }
  ...
  {
        p_fsalattr_out->fsid.major = 222;/* @todo: compute fsid from server address and port number  */;
        p_fsalattr_out->fsid.minor = 111;/* @todo: compute fsid from server address and port number  */;
  }
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes, FSAL_ATTR_MODE ))
        p_fsalattr_out->mode = snmp_object2access_mode( p_handle->object_type_reminder, p_in_node );
       ...
       FSAL_TEST_MASK(p_fsalattr_out->asked_attributes, FSAL_ATTR_CHGTIME )) 
  {
    ...
    p_fsalattr_out->mtime =
        ...
        p_fsalattr_out->chgtime = curr_time;
...
char ASN2add_var( u_char asn_type )
{
        switch( asn_type )
        {
           ...
           case ASN_COUNTER:   return 'c';
           case ASN_GAUGE:     return 'u';
           ...
           case ASN_OPAQUE:    return 'x';
           case ASN_COUNTER64: return 'U';
           case ASN_OPAQUE_FLOAT: return 'F';
           case ASN_NULL:      return 'n';
           default:
...
mode_t fsal2unix_mode( fsal_accessmode_t fsal_mode ){
  ...
  if ( (fsal_mode & FSAL_MODE_SUID) ) out_mode |= S_ISUID ;
  if ( (fsal_mode & FSAL_MODE_SGID) ) out_mode |= S_ISGID ;
  
  if ( (fsal_mode & FSAL_MODE_RUSR) ) out_mode |= S_IRUSR ;
  if ( (fsal_mode & FSAL_MODE_WUSR) ) out_mode |= S_IWUSR ;
  if ( (fsal_mode & FSAL_MODE_XUSR) ) out_mode |= S_IXUSR ;
  if ( (fsal_mode & FSAL_MODE_RGRP) ) out_mode |= S_IRGRP ;
  if ( (fsal_mode & FSAL_MODE_WGRP) ) out_mode |= S_IWGRP ;
  if ( (fsal_mode & FSAL_MODE_XGRP) ) out_mode |= S_IXGRP ;
  if ( (fsal_mode & FSAL_MODE_ROTH) ) out_mode |= S_IROTH ;
  if ( (fsal_mode & FSAL_MODE_WOTH) ) out_mode |= S_IWOTH ;
  if ( (fsal_mode & FSAL_MODE_XOTH) ) out_mode |= S_IXOTH ;
...
fsal_accessmode_t unix2fsal_mode(  mode_t unix_mode ){
    ...
    if ( unix_mode & S_ISUID ) fsalmode|= FSAL_MODE_SUID;
    if ( unix_mode & S_ISGID ) fsalmode|= FSAL_MODE_SGID;
    
    if ( unix_mode & S_IRUSR ) fsalmode|= FSAL_MODE_RUSR;
    if ( unix_mode & S_IWUSR ) fsalmode|= FSAL_MODE_WUSR;
    if ( unix_mode & S_IXUSR ) fsalmode|= FSAL_MODE_XUSR;

    if ( unix_mode & S_IRGRP ) fsalmode|= FSAL_MODE_RGRP;
    if ( unix_mode & S_IWGRP ) fsalmode|= FSAL_MODE_WGRP;
    if ( unix_mode & S_IXGRP ) fsalmode|= FSAL_MODE_XGRP;

    if ( unix_mode & S_IROTH ) fsalmode|= FSAL_MODE_ROTH;
    if ( unix_mode & S_IWOTH ) fsalmode|= FSAL_MODE_WOTH;
    if ( unix_mode & S_IXOTH ) fsalmode|= FSAL_MODE_XOTH;
