...
fsal_status_t FSAL_readdir(
  ...
  {
        FSAL_OID_DUP( &last_listed, dir_descriptor->node_handle.oid_tab, dir_descriptor->node_handle.oid_len );
  ...
  while ( !bool_eod && cur_nb_entries < max_dir_entries )
  {
        ...
        rc = IssueSNMPQuery( dir_descriptor->p_context, last_listed.oid_tab, last_listed.oid_len, &req_opt );

        ReleaseTokenFSCall();

        if ( rc != SNMP_ERR_NOERROR  &&  snmp2fsal_error( rc ) != ERR_FSAL_NOENT )
        {
                DisplayLogJdLevel( fsal_log, NIV_DEBUG,"SNMP GET request failed: error=%d, snmp_errno=%d, errno=%d, msg=%s",
        ...
        else if ( snmp2fsal_error( rc ) != ERR_FSAL_NOENT ) 
        {
            ...
            {
                FSAL_OID_DUP( &pdirent[cur_nb_entries].handle, p_curr_var->name, p_curr_var->name_length );
                ...
                FSAL_OID_DUP( &pdirent[cur_nb_entries].cookie, p_curr_var->name, p_curr_var->name_length );
                FSAL_OID_INC( &pdirent[cur_nb_entries].cookie );

                cur_node =  GetMIBNode( dir_descriptor->p_context, &pdirent[cur_nb_entries].handle, FALSE );
                
                /* build the label */
                rc = snmp_object2name( p_curr_var, cur_node, &pdirent[cur_nb_entries].handle, &pdirent[cur_nb_entries].name );

                if ( rc )  Return( rc , 0, INDEX_FSAL_readdir );
                
                /* when a node does not exist, we take its nearest parent's rights */
                if ( cur_node == NULL )
                    nearest_node = GetMIBNode( dir_descriptor->p_context, &pdirent[cur_nb_entries].handle, TRUE );
                else
                    nearest_node = cur_node;

    #ifdef _DEBUG_FSAL
                printf( "FOUND A DIRECT CHILD (LEAF) = %s, parent_oid_len=%d, oid_len=%d, index=%d\n",
                             pdirent[cur_nb_entries].name.name, dir_descriptor->node_handle.oid_len,
                             p_curr_var->name_length, p_curr_var->index );
                if ( nearest_node ) printf("type = %#X, last oid=%ld\n", nearest_node->type, p_curr_var->name[p_curr_var->name_length-1] );
    #endif
                ...
                if ( get_attr_mask )
                {
                   ...
                   rc = snmp2fsal_attributes( &pdirent[cur_nb_entries].handle, p_curr_var, nearest_node, &pdirent[cur_nb_entries].attributes );

                   if ( rc != ERR_FSAL_NO_ERROR )
                   {
                       FSAL_CLEAR_MASK( pdirent[cur_nb_entries].attributes.asked_attributes );
                       FSAL_SET_MASK( pdirent[cur_nb_entries].attributes.asked_attributes, FSAL_ATTR_RDATTR_ERR );
                   }
                }
                ...
                FSAL_OID_DUP( &last_listed, pdirent[cur_nb_entries].cookie.oid_tab, pdirent[cur_nb_entries].cookie.oid_len );
                cur_nb_entries++;
        ...
        if ( rc != SNMP_ERR_NOERROR  && snmp2fsal_error( rc ) != ERR_FSAL_NOENT )
        {
                DisplayLogJdLevel( fsal_log, NIV_DEBUG,"SNMP GETNEXT request failed: error=%d, snmp_errno=%d, errno=%d, msg=%s",
                                    rc, snmp_errno, errno, snmp_api_errstring(rc) ); 
                Return( snmp2fsal_error( rc ), rc, INDEX_FSAL_readdir );
        }
        else if (  snmp2fsal_error( rc ) == ERR_FSAL_NOENT )
        ...
        if ( IsSNMPChild( dir_descriptor->node_handle.oid_tab, dir_descriptor->node_handle.oid_len,
                        p_curr_var->name, p_curr_var->name_length ) )
        {
                ...
                else /* directory item */
                {
                     ...
                     */
                    if ( fsal_oid_cmp( p_curr_var->name, last_listed.oid_tab, last_listed.oid_len ) < 0 )
                    {
                    ...
                    FSAL_OID_DUP( &pdirent[cur_nb_entries].handle, p_curr_var->name, dir_descriptor->node_handle.oid_len + 1 );
                    ...
                    FSAL_OID_DUP( &pdirent[cur_nb_entries].cookie, pdirent[cur_nb_entries].handle.oid_tab,
                    ...
                    /* build the label */
                    rc = snmp_object2name( NULL, cur_node, &pdirent[cur_nb_entries].handle, &pdirent[cur_nb_entries].name );
                    ...
                    printf( "FOUND A NEW SUBDIR = %s (%ld) (cookie->%ld)\n", pdirent[cur_nb_entries].name.name,
                            pdirent[cur_nb_entries].handle.oid_tab[dir_descriptor->node_handle.oid_len],
                            pdirent[cur_nb_entries].cookie.oid_tab[pdirent[cur_nb_entries].cookie.oid_len-1] );
#endif
                    ...
                    {
                       pdirent[cur_nb_entries].attributes.asked_attributes = get_attr_mask ;
                       rc = snmp2fsal_attributes( &pdirent[cur_nb_entries].handle, NULL, nearest_node, &pdirent[cur_nb_entries].attributes ); 
