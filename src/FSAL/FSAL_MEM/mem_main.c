...
static struct config_item mem_items[] = {
	CONF_ITEM_UI32("Inode_Size", 0, 0x200000, 0, mem_fsal_module,
		       inode_size),
	CONF_ITEM_UI32("Up_Test_Interval", 0, UINT32_MAX, 0, mem_fsal_module,
		       up_interval),
	CONF_ITEM_UI32("Async_Threads", 0, 100, 0, mem_fsal_module,
		       async_threads),
	CONF_ITEM_BOOL("Whence_is_name", false, mem_fsal_module,
		       whence_is_name),
	CONFIG_EOL
};
 ...
 */
static fsal_status_t mem_async_pkginit(void)
{
	...
	LogEvent(COMPONENT_FSAL,
		 "Initialized FSAL_MEM async thread pool with %" PRIu32
		 " threads.",
		 MEM.async_threads);
 ...
 * @return FSAL status
 */
static fsal_status_t mem_async_pkgshutdown(void)
{
	if (!mem_async_fridge) {
	...
	int rc = fridgethr_sync_command(mem_async_fridge, fridgethr_comm_stop,
					120);
				     ...
				     struct config_error_type *err_type)
{
	 ...
	 * params.
	 */
	(void)load_config_from_parse(config_struct, &mem_block, mem_me, true,
				     err_type);
...
MODULE_INIT void init(void)
{
	...
	if (retval != 0) {
		LogCrit(COMPONENT_FSAL, "MEM module failed to register.");
	}
...
MODULE_FINI void finish(void)
{
	...
	LogDebug(COMPONENT_FSAL, "MEM module finishing.");
