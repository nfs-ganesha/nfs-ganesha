    ...
    )
{
  ...
  TakeTokenFSCall();
  status = fsal_internal_handle2fd( p_context, p_old_parentdir_handle, &old_parent_fd, O_DIRECTORY ) ;
  ReleaseTokenFSCall();
   ...
   *************************************/
  TakeTokenFSCall();
  rc = renameat( old_parent_fd, p_old_name->name,
                 new_parent_fd, p_new_name->name );
  errsv = errno;
  ReleaseTokenFSCall();
  close( old_parent_fd ) ;
  close( new_parent_fd ) ;
