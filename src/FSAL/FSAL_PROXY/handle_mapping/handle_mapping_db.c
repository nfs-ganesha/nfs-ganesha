...
static int init_db_thread_info( db_thread_info_t * p_thr_info )
{
    ...
    if (!p_thr_info) return HANDLEMAP_INTERNAL_ERROR;
...
static int init_database_access( db_thread_info_t * p_thr_info )
{
    ...
    snprintf( db_file, MAXPATHLEN, "%s/%s.%u", dbmap_dir, DB_FILE_PREFIX, p_thr_info->thr_index );
    ...
    if ( rc != 0 )
    {
        ...
        {
            DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: could not connect to SQLite3 database (file %s): %s",
                    db_file, sqlite3_errmsg(p_thr_info->db_conn) );
            sqlite3_close(p_thr_info->db_conn);
        }
        else
        {
            DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: could not connect to SQLite3 database (file %s): status=%d",
                    db_file, rc );
        }
        return HANDLEMAP_DB_ERROR;
    }
    ...
    rc = sqlite3_get_table(  p_thr_info->db_conn, "SELECT name FROM sqlite_master WHERE type = 'table' AND name = '"MAP_TABLE"'",
    ...
    rc = sqlite3_prepare_v2( p_thr_info->db_conn,
                            "SELECT "OBJID_FIELD","HASH_FIELD","HANDLE_FIELD" FROM "MAP_TABLE,
                            -1, &(p_thr_info->prep_stmt[LOAD_ALL_STATEMENT]), &unparsed );
                            
    CheckPrepare( p_thr_info->db_conn, rc );
   
    rc = sqlite3_prepare_v2( p_thr_info->db_conn,
                            "INSERT INTO "MAP_TABLE"("OBJID_FIELD","HASH_FIELD","HANDLE_FIELD") "
                            "VALUES (?1, ?2, ?3 )",
                            -1, &(p_thr_info->prep_stmt[INSERT_STATEMENT]), &unparsed );
                            
    CheckPrepare( p_thr_info->db_conn, rc );
    
    rc = sqlite3_prepare_v2( p_thr_info->db_conn,
                            "DELETE FROM "MAP_TABLE" WHERE "OBJID_FIELD"=?1 AND "HASH_FIELD"=?2",
                            -1, &(p_thr_info->prep_stmt[DELETE_STATEMENT]), &unparsed );
...
static int db_load_operation(db_thread_info_t * p_info, hash_table_t * p_hash )
{
    ...
    while ( rc == SQLITE_ROW )
    {
        ...
        if ( rc == 0 )
            ...
            DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR %d adding entry to hash table <object_id=%llu, FH_hash=%u, FSAL_Handle=%s>",
...
static int db_insert_operation(db_thread_info_t * p_info, nfs23_map_handle_t * p_nfs23_digest, fsal_handle_t * p_handle )
{
     int rc;
     char handle_str[2*sizeof(fsal_handle_t)+1];
            
     rc = sqlite3_bind_int64(p_info->prep_stmt[INSERT_STATEMENT], 1, p_nfs23_digest->object_id );
     CheckBind( p_info->db_conn, rc, p_info->prep_stmt[INSERT_STATEMENT] );
             
     rc = sqlite3_bind_int(p_info->prep_stmt[INSERT_STATEMENT], 2, p_nfs23_digest->handle_hash );
     CheckBind( p_info->db_conn, rc, p_info->prep_stmt[INSERT_STATEMENT] );
    
     snprintHandle(handle_str, 2*sizeof(fsal_handle_t)+1, p_handle) ;
     
     rc = sqlite3_bind_text( p_info->prep_stmt[INSERT_STATEMENT], 3, handle_str, -1, SQLITE_STATIC );
     CheckBind( p_info->db_conn, rc, p_info->prep_stmt[INSERT_STATEMENT] );
...
static int db_delete_operation( db_thread_info_t * p_info, nfs23_map_handle_t * p_nfs23_digest )
{
     int rc;

     rc = sqlite3_bind_int64(p_info->prep_stmt[DELETE_STATEMENT], 1, p_nfs23_digest->object_id );
     CheckBind( p_info->db_conn, rc, p_info->prep_stmt[DELETE_STATEMENT] );
             
     rc = sqlite3_bind_int(p_info->prep_stmt[DELETE_STATEMENT], 2, p_nfs23_digest->handle_hash );
     CheckBind( p_info->db_conn, rc, p_info->prep_stmt[DELETE_STATEMENT] );
...
static int dbop_push( flusher_queue_t * p_queue, db_op_item_t * p_op )
{
    ...
    switch ( p_op->op_type )
    {
        ...
        default:
            DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR in dbop_push: Invalid operation type %d", p_op->op_type );
    }
...
static void * database_worker_thread( void * arg )
{
    ...
    if ( rc != HANDLEMAP_SUCCESS )
      {
          /* Failed init */
          DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: Database initialization error %d", rc );
          exit( rc );
      }
    ...
    while ( 1 )
    {
        ...
        /* nothing to be done ? */
        while ( p_info->work_queue.highprio_first == NULL && p_info->work_queue.lowprio_first == NULL )
        {
                to_be_done = NULL;
                ...
                pthread_cond_wait( &p_info->work_queue.work_avail_condition, &p_info->work_queue.queues_mutex );
        ...
        switch ( to_be_done->op_type )
        {
            ...
            case INSERT:
                db_insert_operation( p_info, &to_be_done->op_arg.fh_info.nfs23_digest, &to_be_done->op_arg.fh_info.fsal_handle );
                break;
            ...
            default:
                  DisplayLogJdLevel( fsal_log, NIV_CRIT, "ERROR: Invalid operation type %d", to_be_done->op_type );
        }
                       ...
                       int synchronous_insert )
{
    ...
    for ( i = 0; i < nb_db_threads; i++ )
    {
        ...
        rc = pthread_create( &db_thread[i].thr_id, NULL, database_worker_thread, &db_thread[i] );
        if ( rc ) return HANDLEMAP_SYSTEM_ERROR;                        
    }
...
static void wait_thread_jobs_finished( db_thread_info_t * p_thr_info )
{
    ...
    while ( p_thr_info->work_queue.highprio_first != NULL
        ...
        pthread_cond_wait( &p_thr_info->work_queue.work_done_condition, &p_thr_info->work_queue.queues_mutex );
...
int handlemap_db_reaload_all( hash_table_t * target_hash )
{
    ...
    for ( i = 0; i < nb_db_threads; i++ )
    {
        ...
        GET_PREALLOC( new_task,
                      db_thread[i].dbop_pool,
                      nb_prealloc,
                      db_op_item_t,
                      p_next );
        ...
        if ( !new_task ) return HANDLEMAP_SYSTEM_ERROR;
...
int handlemap_db_flush()
{
    ...
    DisplayLogJdLevel( fsal_log, NIV_EVENT, "Waiting for database synchronization (%u operations pending)", to_sync );
