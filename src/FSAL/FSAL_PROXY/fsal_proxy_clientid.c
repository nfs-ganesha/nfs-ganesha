  ...
  if( done == 0 )
   {
     ...
     if( resnfs4.status != NFS4_OK )
       {
         V( fsal_clientid_mutex ) ;
         return fsal_internal_proxy_error_convert( resnfs4.status, INDEX_FSAL_InitClientContext ) ;
       }
     ...
     argnfs4.argarray.argarray_val[0].nfs_argop4_u.opsetclientid_confirm.clientid = 
           resnfs4.resarray.resarray_val[0].nfs_resop4_u.opsetclientid.SETCLIENTID4res_u.resok4.clientid ;                                          
     memcpy( (char *)argnfs4.argarray.argarray_val[0].nfs_argop4_u.opsetclientid_confirm.setclientid_confirm,
             (char *)resnfs4.resarray.resarray_val[0].nfs_resop4_u.opsetclientid.SETCLIENTID4res_u.resok4.setclientid_confirm, 
              NFS4_VERIFIER_SIZE ) ; 
     ...
     fsal_clientid = argnfs4.argarray.argarray_val[0].nfs_argop4_u.opsetclientid_confirm.clientid ;
...
void * FSAL_proxy_clientid_renewer_thread( void * Arg )
{
  ...
  if ( ( rc = BuddyInit( &buddy_param )) != BUDDY_SUCCESS )
    {
      /* Failed init */
      DisplayLog( "FSAL_proxy_clientid_renewer_thread: Memory manager could not be initialized, exiting..." ) ;
      exit( 1 ) ;
    }
#endif
  ...
  if( FSAL_IS_ERROR( fsal_status ) ) 
   {
      DisplayLog( "FSAL_proxy_clientid_refresher_thread: FSAL error(%u,%u) during init... exiting", fsal_status.major, fsal_status.minor ) ;
      exit( 1 ) ;
   }
  ...
  while( 1 )
   {
    ...
    if( resnfs4.status != NFS4_OK )
         DisplayLog( "FSAL_PROXY: /!\\ NFSv4 error %u occured when trying to new clienitf %llu", resnfs4.status, fsal_clientid ) ;
