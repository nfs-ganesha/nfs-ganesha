...
static int load_pxy_config(const char *name, config_file_t config,
			   struct pxy_fsal_module *pxy)
{
	int err;
	int cnt, i;
	...
	LogWarn(COMPONENT_CONFIG,
		...
		"consider switching to PROXY block inside FSAL", name);

	if (config_ItemType(block) != CONFIG_ITEM_BLOCK) {
		LogCrit(COMPONENT_CONFIG, "\"%s\" is expected to be a block",
			name);
		return 1;
	}
	...
	for (i = 0; i < cnt; i++) {
		...
		errcnt +=
		    pxy_key_to_param(key_name, key_value, &pxy->init, name);
	}
				     ...
				     config_file_t config_struct)
{
	...
	st = fsal_load_config("PROXY", config_struct, &pxy->init, &pxy->fsinfo,
			      pxy_key_to_param);
	if (FSAL_IS_ERROR(st))
		return st;
