...
int get_svr_addr( fsal_handle_t     * p_objecthandle,  /* IN */
...
int get_svr_port( fsal_handle_t     * p_objecthandle,  /* IN */
                  fsal_op_context_t * p_context,       /* IN */
                  caddr_t             buffer_addr,     /* IN/OUT */
                  size_t              buffer_size,     /* IN */
                  size_t            * p_output_size )  /* OUT */
...
int get_prognum( fsal_handle_t     * p_objecthandle,  /* IN */
                  fsal_op_context_t * p_context,       /* IN */
                  caddr_t             buffer_addr,     /* IN/OUT */
                  size_t              buffer_size,     /* IN */
                  size_t            * p_output_size )  /* OUT */
...
int get_proto( fsal_handle_t     * p_objecthandle,  /* IN */
               fsal_op_context_t * p_context,       /* IN */
               caddr_t             buffer_addr,     /* IN/OUT */
               size_t              buffer_size,     /* IN */
               size_t            * p_output_size )  /* OUT */
...
int get_clientid( fsal_handle_t     * p_objecthandle,  /* IN */
                  fsal_op_context_t * p_context,       /* IN */
                  caddr_t             buffer_addr,     /* IN/OUT */
                  size_t              buffer_size,     /* IN */
                  size_t            * p_output_size )  /* OUT */
{
  ...
  snprintf( (char*)buffer_addr, buffer_size, "%llX", p_context->clientid );
...
int get_type( fsal_handle_t     * p_objecthandle,  /* IN */
              fsal_op_context_t * p_context,       /* IN */
              caddr_t             buffer_addr,     /* IN/OUT */
              size_t              buffer_size,     /* IN */
              size_t            * p_output_size )  /* OUT */
{
  if (!p_objecthandle || !p_context || !p_output_size)
    ...
    switch ( p_objecthandle->object_type_reminder )
    {
      case FSAL_TYPE_DIR :
...
int get_ts( fsal_handle_t     * p_objecthandle,  /* IN */
                   fsal_op_context_t * p_context,       /* IN */
                   caddr_t             buffer_addr,     /* IN/OUT */
                   size_t              buffer_size,     /* IN */
                   size_t            * p_output_size )  /* OUT */
{
  ...
  memcpy( buffer_addr, &p_objecthandle->timestamp, sizeof(p_objecthandle->timestamp) );
  *p_output_size = sizeof(p_objecthandle->timestamp);
  
  return 0;
  
}

int print_ts( caddr_t    InBuff, 
              size_t     InSize,
              caddr_t    OutBuff, 
             size_t   * pOutSize )
{
...
int get_svr_handle( fsal_handle_t     * p_objecthandle,  /* IN */
                   fsal_op_context_t * p_context,       /* IN */
                   caddr_t             buffer_addr,     /* IN/OUT */
                   size_t              buffer_size,     /* IN */
                   size_t            * p_output_size )  /* OUT */
{
  ...
  if ( p_objecthandle->srv_handle_len > buffer_size )
  {
                      ...
                      size_t   * pOutSize )
{
   ...
   for( i = 0 ; i < MaxSize ; i++ )
     *pOutSize += sprintf( &(((char *)OutBuff)[i*2]), "%02x", (unsigned char)(((char *)InBuff)[i]) ) ;
...
static int file_attributes_to_xattr_attrs( fsal_attrib_list_t * file_attrs, fsal_attrib_list_t * p_xattr_attrs, unsigned int attr_index )
{
   ...
   */
   fsal_attrib_mask_t supported = FSAL_ATTR_SUPPATTR | FSAL_ATTR_MODE | FSAL_ATTR_FILEID
                                  ...
                                  | FSAL_ATTR_SPACEUSED | FSAL_ATTR_NUMLINKS | FSAL_ATTR_RAWDEV
                                  | FSAL_ATTR_FSID ;
   ...
   if( p_xattr_attrs->asked_attributes == 0 )
    {
      ...
      DisplayLogJdLevel( fsal_log, NIV_CRIT, "Error: p_xattr_attrs->asked_attributes was 0 in %s() line %d, file %s",
   ...
   if ( unsupp )
   {
        DisplayLogJdLevel( fsal_log, NIV_DEBUG,
                  "Asking for unsupported attributes in %s(): %#llX removing it from asked attributes", __FUNCTION__, unsupp );
   ...
   if ( (p_xattr_attrs->asked_attributes & FSAL_ATTR_OWNER)
	&& (p_xattr_attrs->asked_attributes & FSAL_ATTR_MODE)
	&& (p_xattr_attrs->mode == 0) )
   {
...
)
{
      ...
      || !do_match_type( xattr_list[xattr_id].flags, p_objecthandle->object_type_reminder ) )
  {
   ...
   file_attrs.asked_attributes = FSAL_ATTR_MODE | FSAL_ATTR_FILEID | FSAL_ATTR_OWNER
                                ...
                                | FSAL_ATTR_CTIME | FSAL_ATTR_CREATION | FSAL_ATTR_CHGTIME
                                | FSAL_ATTR_FSID ;
   ...
   if ( FSAL_IS_ERROR( st ) ) Return( st.major, st.minor, INDEX_FSAL_GetXAttrAttrs );
...
)
{
  ...
  if(  xattr_list[xattr_id].print_func == NULL )
  ...
  else
   {
      ...
      xattr_list[xattr_id].print_func( buff, 
                                       MAXPATHLEN, 
                                       buffer_addr, 
                                       p_output_size ) ;
   }
