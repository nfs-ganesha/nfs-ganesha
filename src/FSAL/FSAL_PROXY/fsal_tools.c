...
fsal_status_t FSAL_DigestHandle(
  ...
  /* sanity checks */
  if (!in_fsal_handle || !out_buff || !p_expcontext) ReturnCode( ERR_FSAL_FAULT ,0 );
  
  if( in_fsal_handle->srv_handle_len + sizeof( fsal_u64_t ) + 2*sizeof( unsigned int ) > NFSV4_FH_OPAQUE_SIZE )
  ...
  switch(output_type)
  {
    ...
    case  FSAL_DIGEST_NFSV4:
      ...
      memcpy( (char *)(out_buff + sizeof( fsal_u64_t ) ), (char *)&(in_fsal_handle->object_type_reminder), sizeof( unsigned int ) ) ;

      /* Then the len of the file handle */
      memcpy( (char *)(out_buff + sizeof( fsal_u64_t ) + sizeof( unsigned int ) ), &(in_fsal_handle->srv_handle_len), sizeof( unsigned int ) ) ;
      
      /* Then keep the value of the buff */
      memcpy( (char *)(out_buff + sizeof( fsal_u64_t ) + 2*sizeof( unsigned int ) ), in_fsal_handle->srv_handle_val, in_fsal_handle->srv_handle_len) ;
      break;
    ...
    case  FSAL_DIGEST_FILEID2:        
      ...
      memcpy( out_buff, (char *)( &(in_fsal_handle->fileid4) + sizeof( u_int32_t ) ), sizeof( u_int32_t ) ) ;
      break;
...
fsal_status_t FSAL_ExpandHandle(
  ...
  /* sanity checks */
  if (!out_fsal_handle || !in_buff || !p_expcontext) ReturnCode( ERR_FSAL_FAULT ,0 );
  ...
  switch(in_type){
    ...
    case  FSAL_DIGEST_NFSV3:
      ...
      if ( rc == HANDLEMAP_STALE )
      	printf("File id=%llu : HandleMap_GetFH returns HANDLEMAP_STALE\n", map_hdl.object_id );
      else if ( rc == 0 )
	...
	printf("File id=%llu : HandleMap_GetFH returns error %d\n", map_hdl.object_id, rc );
      ...
      if( fsal_internal_proxy_create_fh( &nfs4fh, tmp_hdl.object_type_reminder, tmp_hdl.fileid4, out_fsal_handle ) != TRUE )
    ...
    case  FSAL_DIGEST_NFSV4:
      ...
      /* Keep  the type of then object at the beginning */
      memcpy( (char *)&nodetype, (char *)(in_buff + sizeof( fsal_u64_t ) ), sizeof( unsigned int ) ) ;

      /* Then the len of the file handle */
      memcpy( (char *)&(nfs4fh.nfs_fh4_len), (char *)(in_buff + sizeof( fsal_u64_t ) + sizeof( unsigned int) ), sizeof( unsigned int ) ) ;
     
      /* Then keep the value of the buff */
      nfs4fh.nfs_fh4_val = (char *)(in_buff  + sizeof( fsal_u64_t )+ 2*sizeof( unsigned int ) )  ;

      if( fsal_internal_proxy_create_fh( &nfs4fh, nodetype, fileid, out_fsal_handle ) != TRUE )
         ReturnCode( ERR_FSAL_FAULT ,0 );
    ...
    )
{
  ...
  {    
    ...
    if ( err ){
      DisplayLog("FSAL LOAD PARAMETER: ERROR reading key[%d] from section \"%s\" of configuration file.",
    ...
    else if ( !STRCMP( key_name, "Max_FS_calls" ) )
    {
        ...
        if ( maxcalls < 0 )
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: null or positive integer expected.",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
        }
    ...
    {
      DisplayLog("FSAL LOAD PARAMETER: ERROR: Unknown or unsettable key: %s (item %s)",
    ...
    if ( !STRCMP( key_name, "link_support" ) )
      {
        ...
        if ( bool == -1 )
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: boolean expected.",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
        }
    ...
    else if ( !STRCMP( key_name, "maxread" ) )
      {
        ...
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: positive integer expected.",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
        }
    ...
    else if ( !STRCMP( key_name, "umask" ) )
      {
        ...
        if ( mode < 0 )
        {
          DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: octal expected.",key_name);
          ReturnCode( ERR_FSAL_INVAL, 0 );
        }
    ...
    )
{
  ...
  for ( var_index = 0; var_index < var_max ; var_index ++ )
  {
       ...
       if( isdigit( key_value[0] ) )
       ...
       else
        {
          ...
          if( ( hp = gethostbyname( key_value ) )  == NULL )
           {
                DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s", key_name);
                ReturnCode( ERR_FSAL_INVAL, 0 );
           }
    ...
    {
       out_parameter->fs_specific_info.srv_port = htons( (unsigned short)atoi( key_value ) );
    }    
    ...
    else if( !STRCMP( key_name, "NFS_Proto" ) ) 
    {
       ...
       if( strncasecmp( key_value, "udp", MAXNAMLEN ) && strncasecmp( key_value, "tcp", MAXNAMLEN ) ) 
         {
             DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s --> %s", key_name, key_value);
             ReturnCode( ERR_FSAL_INVAL, 0 );
         }
    ...
    else if( !STRCMP( key_name, "Sec_Type" ) ) 
    {
      ...
      if( !STRCMP( key_value, "krb5" ) )
       ...
       {
         DisplayLog("FSAL LOAD PARAMETER: bad value %s for parameter %s", key_value, key_name ) ;
         ReturnCode( ERR_FSAL_INVAL, 0 );
       }
#endif
    }
    ...
    else if( !STRCMP( key_name, "Enable_Handle_Mapping" ) )
    {
       ...
       {
             DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s --> %s (boolean expected)", key_name, key_value);
             ReturnCode( ERR_FSAL_INVAL, 0 );
       }
    }
    ...
    {
        out_parameter->fs_specific_info.hdlmap_nb_entry_prealloc = (unsigned int)atoi( key_value );
    }
    else if( !STRCMP( key_name, "HandleMap_Nb_DB_Operations_Prealloc" ) )
    {
        out_parameter->fs_specific_info.hdlmap_nb_db_op_prealloc = (unsigned int)atoi( key_value );
    }
    else if( !STRCMP( key_name, "Open_by_FH_Working_Dir" ) )
    {
       strncpy( out_parameter->fs_specific_info.openfh_wd, key_value, MAXPATHLEN ) ; 
