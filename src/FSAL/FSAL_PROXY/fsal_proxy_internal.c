...
void fsal_interval_proxy_fsalattr2bitmap4( fsal_attrib_list_t * pfsalattr, bitmap4 * pbitmap ) 
{
  ...
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_SIZE ) )  tmpattrlist[attrlen++] = FATTR4_SIZE ;
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_MODE ) )  tmpattrlist[attrlen++] = FATTR4_MODE ;
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_OWNER ) ) tmpattrlist[attrlen++] = FATTR4_OWNER ;
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_GROUP ) ) tmpattrlist[attrlen++] = FATTR4_OWNER_GROUP ;
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_ATIME ) ) tmpattrlist[attrlen++] = FATTR4_TIME_ACCESS_SET ;
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_MTIME ) ) tmpattrlist[attrlen++] = FATTR4_TIME_MODIFY_SET ;
  if ( FSAL_TEST_MASK( pfsalattr->asked_attributes, FSAL_ATTR_CTIME ) ) tmpattrlist[attrlen++] = FATTR4_TIME_METADATA ;
                                   ...
                                   fsal_handle_t    * pfsal_handle ) 
{
   ...
   memcpy( pfsal_handle->srv_handle_val, pnfs4_handle->nfs_fh4_val, pnfs4_handle->nfs_fh4_len ) ;  
...
if( type == FSAL_TYPE_FILE )
{
  ...
  DisplayLog( "fsal_internal_proxy_create_fh: output nfsv4 server handle= %s fileid=%llu", outstr, fileid );
}

  if( memcmp( pfsal_handle->srv_handle_val,  pnfs4_handle->nfs_fh4_val,  pnfs4_handle->nfs_fh4_len ) )
       DisplayLog( "CRITICAL ERROR: ==========> Filehandle mismatch n ifsal_internal_proxy_create" ) ;
  ...
  for( i = 0 ; i < attrmasklen; i++ )
    {
      ...
      switch( attribute_to_set )
        ...
        case FATTR4_FILEHANDLE:
           ...
           fsal_internal_proxy_create_fh( &nfshandle, pFSAL_attr->type, pFSAL_attr->fileid, phandle ) ;
...
fsal_status_t      fsal_status ;
  ...
  if( ( p_thr_context->rpc_client->cl_auth = authgss_create_default( p_thr_context->rpc_client, 
  ...
  if( !strcmp( p_thr_context->srv_proto, "udp" ) )
   {  
    ...
    if( ( p_thr_context->rpc_client = clntudp_bufcreate( &addr_rpc,
                                                         ...
                                                         p_thr_context->srv_recvsize ) ) == NULL )
       {
          ...
          DisplayLog( "FSAL RECONNECT : Cannot contact server addr=%u.%u.%u.%u port=%u prognum=%u using NFSv4 protocol",
   ...
   {  
    ...
    if( ( p_thr_context->rpc_client = clnttcp_create( &addr_rpc,
                                                      ...
                                                      p_thr_context->srv_recvsize ) ) == NULL )
       {
          DisplayLog( "FSAL RECONNECT : Cannot contact server addr=%x.%x.%x.%x port=%u prognum=%u using NFSv4 protocol",
                      ( ntohl( p_thr_context->srv_addr ) & 0xFF000000 ) >> 24,
                      ( ntohl( p_thr_context->srv_addr ) & 0x00FF0000 ) >> 16,
                      ( ntohl( p_thr_context->srv_addr ) & 0x0000FF00 ) >> 8,
                      ( ntohl( p_thr_context->srv_addr ) & 0x000000FF ),
 ...
 *
 */
int FSAL_proxy_set_hldir(  fsal_op_context_t * p_thr_context, 
                           char              * hl_path ) 
{
  ...
  fsal_status = FSAL_lookupPath( &fsal_path, 
                                 p_thr_context,
                                 &(p_thr_context->openfh_wd_handle),
                                 NULL ) ;
...
#define FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM 1
  ...
  argnfs4.argarray.argarray_val[FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM].argop = NFS4_OP_OPEN_CONFIRM ;
  argnfs4.argarray.argarray_val[FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM].nfs_argop4_u.opopen_confirm.open_stateid.seqid = pfd->stateid.seqid;
  memcpy( (char *)argnfs4.argarray.argarray_val[FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM].nfs_argop4_u.opopen_confirm.open_stateid.other,
          (char *)pfd->stateid.other, 
          12 ) ;
  argnfs4.argarray.argarray_val[FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM].nfs_argop4_u.opopen_confirm.seqid = pfd->stateid.seqid +1;
  ...
  pfd->stateid.seqid = resnfs4.resarray.resarray_val[FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM].nfs_resop4_u.opopen_confirm.OPEN_CONFIRM4res_u.resok4.open_stateid.seqid ;
  memcpy( (char *)pfd->stateid.other, 
          (char *)resnfs4.resarray.resarray_val[FSAL_PROXY_OPEN_CONFIRM_IDX_OP_OPEN_CONFIRM].nfs_resop4_u.opopen_confirm.OPEN_CONFIRM4res_u.resok4.open_stateid.other, 12 ) ;
