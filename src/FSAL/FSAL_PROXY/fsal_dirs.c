...
#define FSAL_READDIR_IDX_OP_READDIR    1
  ...
  memset( (char *)tabentry4attr, 0, FSAL_READDIR_SIZE * sizeof( fsal_proxy_internal_fattr_readdir_t ) ) ;
  for( i = 0 ; i <  nbreaddir ; i++ ) 
   {
      ...
      tabentry4[i].attrs.attr_vals.attrlist4_len =  sizeof( fsal_proxy_internal_fattr_readdir_t ) ;

      tabentry4[i].attrs.attrmask.bitmap4_val    = tabentry4bitmap[i] ; 
      tabentry4[i].attrs.attrmask.bitmap4_len    = 2 ;
   }
  resnfs4.resarray.resarray_val[FSAL_READDIR_IDX_OP_READDIR].nfs_resop4_u.opreaddir.READDIR4res_u.resok4.reply.entries = (entry4 *)tabentry4 ;
  ...
  COMPOUNDV4_ARG_ADD_OP_READDIR( argnfs4, start_position, nbreaddir, dir_descriptor->verifier, bitmap  )  ;  
  ...
  if( resnfs4.resarray.resarray_val[FSAL_READDIR_IDX_OP_READDIR].nfs_resop4_u.opreaddir.READDIR4res_u.resok4.reply.eof == TRUE )
    {
      *end_of_dir = TRUE ;
    }
  ...
  for( piter_entry = resnfs4.resarray.resarray_val[FSAL_READDIR_IDX_OP_READDIR].nfs_resop4_u.opreaddir.READDIR4res_u.resok4.reply.entries ;
       piter_entry != NULL ;
