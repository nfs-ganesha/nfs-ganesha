...
hpss_fileattr_t     *AttrOut)       /* OUT - attributes after change */
{
   ...
   if ((AttrIn == (hpss_fileattr_t *)NULL)
            ||
       (AttrOut == (hpss_fileattr_t *)NULL))
   {
      return(-EFAULT);
   }
   ...
   if (chkbit64m(SelFlagsIn, CORE_ATTR_ACCOUNT)
       && !chkbit64m(SelFlagsIn, CORE_ATTR_UID))
   {
      return(-EPERM);
   }
#endif
   ...
   else
   {
	 ...
	 strcpy(path_object,".");
      ...
      if ( chkbit64m(SelFlagsIn,CORE_ATTR_COS_ID)
	   ...
	   ( attr_parent.COSId != 0 )
	   &&
	   ( AttrIn->Attrs.COSId != attr_parent.COSId ) )
      {
	 error = -EPERM;
	 API_DEBUG_FPRINTF(DebugFile, &RequestID,
	 ...
	 if (chkbit64m(SelFlagsIn,CORE_ATTR_TIME_LAST_READ))
	 {
	    dmg_attr_bits = bld64m(high32m(dmg_attr_bits),
				   (low32m(dmg_attr_bits)
				    | CHANGE_UTIME));
	    dmg_attr_in.Attrs.Attrs.TimeLastRead = 
	       AttrIn->Attrs.TimeLastRead;
	 }
	 ...
	 if (chkbit64m(SelFlagsIn,CORE_ATTR_TIME_LAST_WRITTEN))
	 {
	    dmg_attr_bits = bld64m(high32m(dmg_attr_bits),
				   (low32m(dmg_attr_bits)
				    | CHANGE_UTIME));
	    dmg_attr_in.Attrs.Attrs.TimeLastWritten = 
	       AttrIn->Attrs.TimeLastWritten;
	 }
	 ...
	 if (chkbit64m(SelFlagsIn,CORE_ATTR_DATA_LENGTH))
	 {
	    dmg_attr_bits = bld64m(high32m(dmg_attr_bits),
				   (low32m(dmg_attr_bits)
				    | CHANGE_FILESIZE));
	    dmg_attr_in.Attrs.Attrs.DataLength = 
	       AttrIn->Attrs.DataLength;
	 }
