  ...
  switch(output_type){   
    ...
    case  FSAL_DIGEST_NFSV2:
      ...
      if ( memcmp( &in_fsal_handle->ns_handle.CoreServerUUID,
                   &p_expcontext->fileset_root_handle.CoreServerUUID,
                   sizeof(TYPE_UUIDT)) ){
        char buffer[128];
        snprintmem(buffer,128,(caddr_t)&(in_fsal_handle->ns_handle.CoreServerUUID),sizeof(TYPE_UUIDT));
    ...
    )
{
  ...
  for ( var_index = 0; var_index < var_max ; var_index ++ )
  {
    ...
    if ( !STRCMP( key_name, "PrincipalName" ) )
      {
        ...
        out_parameter->fs_specific_info.behaviors.PrincipalName
            = FSAL_INIT_FORCE_VALUE;
        strcpy( out_parameter->fs_specific_info.hpss_config.PrincipalName,
            key_value );
    ...
    else if ( !STRCMP( key_name, "KeytabPath" ) )
      {
        ...
        out_parameter->fs_specific_info.behaviors.KeytabPath
            = FSAL_INIT_FORCE_VALUE;
        strcpy( out_parameter->fs_specific_info.hpss_config.KeytabPath,
            key_value );
    ...
    if ( !STRCMP( key_name, "PrincipalName" ) )
      {        
        out_parameter->fs_specific_info.behaviors.Principal
            = FSAL_INIT_FORCE_VALUE;
        strcpy( out_parameter->fs_specific_info.Principal,
            key_value );
    ...
    else if ( !STRCMP( key_name, "KeytabPath" ) )
      {
        out_parameter->fs_specific_info.behaviors.KeytabPath
            = FSAL_INIT_FORCE_VALUE;
        strcpy( out_parameter->fs_specific_info.KeytabPath,
            key_value );            
      }
    else if ( !STRCMP( key_name, "AuthMech" ) )
      {
        ...
        out_parameter->fs_specific_info.behaviors.AuthnMech
            = FSAL_INIT_FORCE_VALUE;
        
        error = hpss_AuthnMechTypeFromString( key_value,
                  &out_parameter->fs_specific_info.hpss_config.AuthnMech );
    ...
    else if ( !STRCMP( key_name, "BusyDelay" ) )
      {
        ...
        out_parameter->fs_specific_info.behaviors.BusyDelay
            = FSAL_INIT_FORCE_VALUE;
    ...
    else if ( !STRCMP( key_name, "BusyRetries" ) )
      {
        ...
        if ( key_value[0] == '-' )
        {
          ...
          if (busyretries < 0)
          {
            DisplayLog("FSAL LOAD PARAMETER: ERROR: Unexpected value for %s: integer expected.",key_name);
            ReturnCode( ERR_FSAL_INVAL, 0 );
          }
        ...
        out_parameter->fs_specific_info.behaviors.BusyRetries
            = FSAL_INIT_FORCE_VALUE;
    ...
    else if ( !STRCMP( key_name, "NumRetries" ) )
      {
        ...
        out_parameter->fs_specific_info.behaviors.NumRetries
            = FSAL_INIT_FORCE_VALUE;
      ...
      {
        out_parameter->fs_specific_info.behaviors.DebugPath
            = FSAL_INIT_FORCE_VALUE;
        strcpy( out_parameter->fs_specific_info.hpss_config.DebugPath,
            key_value );
      }
