...
static int HPSSFSAL_Init( fs_specific_initinfo_t * hpss_init_info ){
  ...
  if (!hpss_init_info) return ERR_FSAL_FAULT;
...
#define API_DEBUG_TRACE    (4)
  ...
  DisplayLogJdLevel( fsal_log, NIV_DEBUG, "  LimitedRetries: %d", hpss_config.LimitedRetries);
  DisplayLogJdLevel( fsal_log, NIV_DEBUG, "  MaxConnections: %d", hpss_config.MaxConnections);
  DisplayLogJdLevel( fsal_log, NIV_DEBUG, "  ReuseDataConnections: %d", hpss_config.ReuseDataConnections);
  DisplayLogJdLevel( fsal_log, NIV_DEBUG, "  UsePortRange: %d", hpss_config.UsePortRange);
  DisplayLogJdLevel( fsal_log, NIV_DEBUG, "  RetryStageInp: %d", hpss_config.RetryStageInp);
...
fsal_status_t  FSAL_Init(
  ...
  if ( init_info->fs_specific_info.behaviors.KeytabPath == FSAL_INIT_FS_DEFAULT )
  {
    ...
    DisplayLog("FSAL INIT: *** WARNING: No keytab file specified for HPSS, default client keytab will be used...");
    DisplayLog("FSAL INIT: ***          Set %s::KeytabPath into config file to use another keytab", CONF_LABEL_FS_SPECIFIC);
  }
  
  if ( init_info->fs_specific_info.behaviors.PrincipalName == FSAL_INIT_FS_DEFAULT )
  {
    /* issue a warning on stderr if no credential is specified*/
    DisplayLog("FSAL INIT: *** WARNING: No principal name specified for HPSS, default will be used...");
  ...
  {
    /* issue a warning on stderr if no credential is specified*/
    DisplayLog("FSAL INIT: *** WARNING: No authentication mechanism specified for HPSS, default authentication mechanism will be used...");
    DisplayLog("FSAL INIT: ***          Set %s::AuthMech into config file to use another Authentication Mechanism", CONF_LABEL_FS_SPECIFIC);
  }  
  ...
  {
    /* issue a warning on stderr if no credential is specified*/
    DisplayLog("FSAL INIT: *** WARNING: No principal name specified for HPSS, default principal name will be used...");
    DisplayLog("FSAL INIT: ***          Set %s::PrincipalName into config file to use another principal", CONF_LABEL_FS_SPECIFIC);
  }
  ...
  switch( init_info->fs_specific_info.behaviors.ReturnInconsistentDirent )
  {
    case FSAL_INIT_FORCE_VALUE :
        /* force the value in any case */
        fsal_internal_SetReturnInconsistentDirent(  init_info->fs_specific_info.ReturnInconsistentDirent );
        break;
    /* In the other cases, we keep the default value. */
  }
