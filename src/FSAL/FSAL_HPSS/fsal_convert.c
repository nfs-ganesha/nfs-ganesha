...
int  fsal2hpss_testperm(fsal_accessflags_t testperm){
  ...
  if (testperm & FSAL_R_OK) hpss_testperm |= R_OK;
  if (testperm & FSAL_W_OK) hpss_testperm |= W_OK;
  if (testperm & FSAL_X_OK) hpss_testperm |= X_OK;
  if (testperm & FSAL_F_OK) hpss_testperm |= F_OK;
...
{ 
  ...
  if (!p_hpss_flags) return ERR_FSAL_FAULT;
  ...
  cpt = 0;
  if ( fsal_flags & FSAL_O_RDONLY ) cpt ++;
  if ( fsal_flags & FSAL_O_RDWR )   cpt ++;
  if ( fsal_flags & FSAL_O_WRONLY ) cpt ++;
  
  if ( cpt > 1 ) return ERR_FSAL_INVAL;
  ...
  *p_hpss_flags = 0;
  ...
  if ( fsal_flags & FSAL_O_RDONLY )  *p_hpss_flags |= O_RDONLY;
  if ( fsal_flags & FSAL_O_RDWR )    *p_hpss_flags |= O_RDWR;
  if ( fsal_flags & FSAL_O_WRONLY )  *p_hpss_flags |= O_WRONLY;
  if ( fsal_flags & FSAL_O_APPEND )  *p_hpss_flags |= O_APPEND;
  if ( fsal_flags & FSAL_O_TRUNC )   *p_hpss_flags |= O_TRUNC;
  ...
  switch( hpss_type_in ){
    ...
    default:
      DisplayLogJdLevel( fsal_log, NIV_EVENT,
          "Unknown object type: %d",hpss_type_in );
      return -1;
  }    
 ...
 */
fsal_accessmode_t hpss2fsal_mode( 
    unsigned32 uid_bit,
    unsigned32 gid_bit,
  ...
  /* special bits */
  if ( uid_bit ) out_mode |= FSAL_MODE_SUID;
  if ( gid_bit ) out_mode |= FSAL_MODE_SGID;
  if ( sticky_bit ) out_mode |= FSAL_MODE_SVTX;
  
  /* user perms */
  if ( user_perms & NS_PERMS_RD )  out_mode |= FSAL_MODE_RUSR ;
  if ( user_perms & NS_PERMS_WR )  out_mode |= FSAL_MODE_WUSR ;
  if ( user_perms & NS_PERMS_XS )  out_mode |= FSAL_MODE_XUSR ;
  
  /* group perms */
  if ( group_perms & NS_PERMS_RD )  out_mode |= FSAL_MODE_RGRP ;
  if ( group_perms & NS_PERMS_WR )  out_mode |= FSAL_MODE_WGRP ;
  if ( group_perms & NS_PERMS_XS )  out_mode |= FSAL_MODE_XGRP ;

  /* other perms */
  if ( other_perms & NS_PERMS_RD )  out_mode |= FSAL_MODE_ROTH ;
  if ( other_perms & NS_PERMS_WR )  out_mode |= FSAL_MODE_WOTH ;
  if ( other_perms & NS_PERMS_XS )  out_mode |= FSAL_MODE_XOTH ;
...
void fsal2hpss_mode( 
  ...
  *other_perms = 0;
  ...
  if ( fsal_mode & FSAL_MODE_RUSR  ) *user_perms |= NS_PERMS_RD;
  if ( fsal_mode & FSAL_MODE_WUSR  ) *user_perms |= NS_PERMS_WR;
  if ( fsal_mode & FSAL_MODE_XUSR  ) *user_perms |= NS_PERMS_XS;
  
  /* group perms */
  
  if ( fsal_mode & FSAL_MODE_RGRP  ) *group_perms |= NS_PERMS_RD;
  if ( fsal_mode & FSAL_MODE_WGRP  ) *group_perms |= NS_PERMS_WR;
  if ( fsal_mode & FSAL_MODE_XGRP  ) *group_perms |= NS_PERMS_XS;
  
  /* other perms */
  
  if ( fsal_mode & FSAL_MODE_ROTH  ) *other_perms |= NS_PERMS_RD;
  if ( fsal_mode & FSAL_MODE_WOTH  ) *other_perms |= NS_PERMS_WR;
  if ( fsal_mode & FSAL_MODE_XOTH  ) *other_perms |= NS_PERMS_XS;
...
fsal_status_t hpss2fsal_attributes(  ns_ObjHandle_t * p_hpss_handle_in,
                                     ...
                                     fsal_attrib_list_t * p_fsalattr_out )
 {
  ...
  if( p_fsalattr_out->asked_attributes == 0 )
    {
      ...
      DisplayLog( "Error: p_fsalattr_out->asked_attributes  valait 0 dans hpss2fsal_attributes line %d, fichier %s", 
		  __LINE__, __FILE__ ) ; 
    }
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_FILEID)){
        p_fsalattr_out->fileid = (fsal_u64_t)
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_ATIME )){
        ...
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeLastRead = %d", p_hpss_attr_in->TimeLastRead);
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeCreated = %d", p_hpss_attr_in->TimeCreated);
#endif
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_MTIME )){
        ...
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tType = %d",hpss2fsal_type( p_hpss_handle_in->Type));
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeLastWritten = %d", p_hpss_attr_in->TimeLastWritten);
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeModified = %d", p_hpss_attr_in->TimeModified);
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeCreated = %d", p_hpss_attr_in->TimeCreated);
#endif
          ...
          case FSAL_TYPE_LNK:          
            ...
            if ( p_hpss_attr_in->TimeLastWritten != 0 )
              p_fsalattr_out->mtime
                = hpss2fsal_time(p_hpss_attr_in->TimeLastWritten) ;
            else
              p_fsalattr_out->mtime
                = hpss2fsal_time(p_hpss_attr_in->TimeCreated) ;
            break;
  ...
  if ( FSAL_TEST_MASK(p_fsalattr_out->asked_attributes,
      FSAL_ATTR_CHGTIME )){
        p_fsalattr_out->chgtime
            = hpss2fsal_time( MAX_3(p_hpss_attr_in->TimeModified,p_hpss_attr_in->TimeCreated, p_hpss_attr_in->TimeLastWritten) );
  }
...
fsal_status_t  hpssHandle2fsalAttributes( ns_ObjHandle_t * p_hpsshandle_in,
  ...
  /* sanity check */
  if (!p_hpsshandle_in || !p_fsalattr_out) ReturnCode(ERR_FSAL_FAULT,0);

    
  /* check that asked attributes are available */
  avail_attr = (   
        FSAL_ATTR_SUPPATTR | FSAL_ATTR_TYPE |  FSAL_ATTR_FILEID
      );
                                    ...
                                    hpss_Attrs_t * p_hpss_attrs  )
{
  ...
  fsal_attrib_mask_t  settable_attrs, supp_attrs,
                      unavail_attrs, unsettable_attrs;
  ...
  if ( FSAL_TEST_MASK( p_attrib_set->asked_attributes , FSAL_ATTR_OWNER ) ){
    ...
    (*p_hpss_attrmask) =
        API_AddRegisterValues(*p_hpss_attrmask,CORE_ATTR_UID,-1);
    
     p_hpss_attrs->UID = p_attrib_set->owner;
     
#ifdef _DEBUG_FSAL    
     DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"Setting Owner = : %d ", p_attrib_set->owner );
#endif
  }
  ...
  if ( FSAL_TEST_MASK( p_attrib_set->asked_attributes , FSAL_ATTR_GROUP ) ){
    ...
    (*p_hpss_attrmask) =
        API_AddRegisterValues(*p_hpss_attrmask,CORE_ATTR_GID,-1);
  ...
  if ( FSAL_TEST_MASK( p_attrib_set->asked_attributes , FSAL_ATTR_ATIME ) ){
    ...
    p_hpss_attrs->TimeLastRead = fsal2hpss_time(  p_attrib_set->atime );
        ...
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeLastRead = %d", p_hpss_attrs->TimeLastRead);
#endif
...
#ifdef _DEBUG_FSAL    
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeLastWritten = %d", p_hpss_attrs->TimeLastWritten);
#endif
...
#ifdef _DEBUG_FSAL    
        DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG,"\tTimeModified = %d", p_hpss_attrs->TimeModified);
#endif
