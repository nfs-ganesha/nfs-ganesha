...
#define CEPH_STATX_ATTR_MASK \
	(CEPH_STATX_BASIC_STATS | CEPH_STATX_BTIME | CEPH_STATX_VERSION)

#ifdef USE_FSAL_CEPH_STATX
static inline UserPerm *user_cred2ceph(const struct user_cred *cred)
{
	return ceph_userperm_new(cred->caller_uid, cred->caller_gid,
				 cred->caller_glen, cred->caller_garray);
}

static inline int fsal_ceph_ll_walk(struct ceph_mount_info *cmount,
...
static inline int fsal_ceph_ll_getattr(struct ceph_mount_info *cmount,
				       struct Inode *in, struct ceph_statx *stx,
				       unsigned int want,
				       const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_ll_lookup(struct ceph_mount_info *cmount,
...
static inline int fsal_ceph_ll_mkdir(struct ceph_mount_info *cmount,
				     Inode *parent, const char *name,
				     mode_t mode, Inode **out,
				     struct ceph_statx *stx, bool full,
				     const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_ll_mknod(struct ceph_mount_info *cmount,
				     Inode *parent, const char *name,
				     mode_t mode, dev_t rdev, Inode **out,
				     struct ceph_statx *stx, bool full,
				     const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_ll_symlink(struct ceph_mount_info *cmount,
				       Inode *parent, const char *name,
				       const char *link_path, Inode **out,
				       struct ceph_statx *stx, bool full,
				       const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_ll_readlink(struct ceph_mount_info *cmount,
					Inode *in, char *buf, size_t bufsize,
					const struct user_cred *creds)
{
	int ret;
	UserPerm *perms = user_cred2ceph(creds);
...
static inline int fsal_ceph_ll_create(struct ceph_mount_info *cmount,
				      Inode *parent, const char *name,
				      mode_t mode, int oflags, Inode **outp,
				      Fh **fhp, struct ceph_statx *stx,
				      bool full, const struct user_cred *creds)
{
	int ret;
	UserPerm *perms = user_cred2ceph(creds);
...
static inline int fsal_ceph_ll_setattr(struct ceph_mount_info *cmount, Inode *i,
...
static inline int fsal_ceph_ll_open(struct ceph_mount_info *cmount, Inode *i,
				    int flags, Fh **fh,
				    const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_ll_opendir(struct ceph_mount_info *cmount,
				       struct Inode *in,
...
static inline int fsal_ceph_ll_link(struct ceph_mount_info *cmount, Inode *i,
				    Inode *newparent, const char *name,
				    const struct user_cred *creds)
{
	int ret;
	UserPerm *perms = user_cred2ceph(creds);
...
static inline int fsal_ceph_ll_unlink(struct ceph_mount_info *cmount,
...
static inline int fsal_ceph_ll_rename(struct ceph_mount_info *cmount,
...
static inline int fsal_ceph_ll_rmdir(struct ceph_mount_info *cmount,
				     struct Inode *in, const char *name,
				     const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_readdirplus(struct ceph_mount_info *cmount,
					struct ceph_dir_result *dirp,
					Inode *dir, struct dirent *de,
					struct ceph_statx *stx,
					unsigned int want, unsigned int flags,
					Inode **out, struct user_cred *cred)
{
	return ceph_readdirplus_r(cmount, dirp, de, stx, want, flags, out);
}

static inline int fsal_ceph_ll_getxattr(struct ceph_mount_info *cmount,
					struct Inode *in, const char *name,
...
static inline int fsal_ceph_ll_setxattr(struct ceph_mount_info *cmount,
					struct Inode *in, const char *name,
					char *val, size_t size, int flags,
					const struct user_cred *creds)
{
	int ret;
	UserPerm *perms = user_cred2ceph(creds);
...
static inline int fsal_ceph_ll_removexattr(struct ceph_mount_info *cmount,
					   struct Inode *in, const char *name,
					   const struct user_cred *creds)
{
	int ret;
...
static inline int fsal_ceph_ll_getxattr(struct ceph_mount_info *cmount,
					...
					const struct user_cred *cred)
{
	return ceph_ll_getxattr(cmount, in, name, val, size, cred->caller_uid,
				cred->caller_gid);
}
