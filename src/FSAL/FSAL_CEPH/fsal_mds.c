...
#define BIGGEST_PATTERN 1024 /* Linux supports a stripe pattern with
                                no more than 4096 stripes, but for now
                                we stick to 1024 to keep them da_addrs
                            ...
                            struct fsal_layoutget_res *res)
{
  ...
  /* Since the Linux kernel refuses to work with any layout that
     ...
     Otherwise, make sure the required layout doesn't go beyond what
     can be accessed through pNFS. */
  if (!((res->segment.offset == 0) &&
        (res->segment.length == NFS4_UINT64_MAX)))
    {
      if (FSAL_range_overlaps(res->segment.offset,
                              arg->minlength,
                              last_possible_byte,
                              NFS4_UINT64_MAX))
        {
  ...
  /* For now, just make the low quad of the deviceid be the inode
     ...
     lets us generate the device address on the fly from the deviceid
  ...
  /* We return exactly one filehandle, filling in the necessary
     information for the DS server to speak to the Ceph OSD directly. */
  ...
  /* We are using sparse layouts with commit-through-DS, so our
     utility word contains only the stripe width, our first stripe is
     always at the beginning of the layout, and there is no pattern
     offset. */
  ...
  /* We grant only one segment, and we want it back when the file is
     closed. */
                               ...
                               struct fsal_layoutreturn_res *res)
{
  ...
  /* Since we no longer store DS addresses, we no longer have anything
     to free.  Later on we should unravel the Ceph client a bit more
     and coordinate with the Ceph MDS's notion of read and write pins,
     but that isn't germane until we have LAYOUTRECALL. */

  if (arg->synthetic)
    {
      /* We ALWAYS free the segment for a synthetic return. */
      res->disposed = TRUE;
    }
  else if (FSAL_range_contains(arg->offset,
                               arg->length,
                               res->segment.offset,
                               res->segment.length))
    {
                               ...
                               struct fsal_layoutcommit_res *res)
{
  ...
  if ((ceph_status = ceph_ll_getattr(cmount, VINODE(handle), &stold,
                            uid, gid)) < 0)
    {
                                ...
                                const fsal_deviceid_t *deviceid)
{
  ...
  /* First we encode the count of stripes.  Since our pattern doesn't
     repeat, we have as many indices as we do stripes. */
  ...
  /* Since our index is the OSD number itself, we have only one host
     per multipath_list. */
