     ...
     )
{
   ...
   ***********/
    if ( FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_MODE ) )
    {
        ...
        if ( !S_ISLNK( buffstat.st_mode ) )
        {
            ...
            {
#ifdef _DEBUG_FSAL
            DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Permission denied for CHMOD opeartion: current owner=%d, credential=%d",
   ...
   ***********/
    ...
    if ( FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_OWNER ) )
    {
        ...
        {
#ifdef _DEBUG_FSAL
            DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Permission denied for CHOWN opeartion: current owner=%d, credential=%d, new owner=%d", buffstat.st_uid, p_context->credential.user, attrs.owner );
#endif
            Return( ERR_FSAL_PERM, 0, INDEX_FSAL_setattrs );
        }
    }
    ...
    if ( FSAL_TEST_MASK( attrs.asked_attributes, FSAL_ATTR_GROUP ) )
    {
        ...
        {
#ifdef _DEBUG_FSAL
            DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Permission denied for CHOWN operation: current group=%d, credential=%d, new group=%d", buffstat.st_gid, p_context->credential.group, attrs.group );
#endif
    ...
    {
#ifdef _DEBUG_FSAL
            DisplayLogJdLevel( fsal_log, NIV_FULL_DEBUG, "Performing chown(%s, %d,%d)", fsalpath.path,
