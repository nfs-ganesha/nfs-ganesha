...
static fsal_xattr_def_t xattr_list[] = {
    ...
    {"stripe_size", print_stripe, NULL, XATTR_FOR_FILE | XATTR_FOR_DIR | XATTR_RO, (void*)ARG_STRIPE_SIZE },
    {"stripe_count",  print_stripe, NULL, XATTR_FOR_FILE | XATTR_FOR_DIR | XATTR_RO, (void*)ARG_STRIPE_COUNT },
     ...
     )
{
    ...
    if ( xattr_id < XATTR_COUNT && !do_match_type( xattr_list[xattr_id].flags, file_attrs.type ) )
    {
...
)
{
    ...
    return FSAL_SetXAttrValue( p_objecthandle, &attr_name,
                               p_context, buffer_addr, buffer_size,
                               FALSE );
}
