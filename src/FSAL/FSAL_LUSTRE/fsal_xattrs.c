...
#define ARG_FULL_FID ((long)0)
#define ARG_FID_SEQ  ((long)1)
#define ARG_FID_OID  ((long)2) 
#define ARG_FID_VER  ((long)3)
	       ...
	       void * arg )         
{
    ...
    {
	case ARG_FULL_FID:
    		*p_output_size = snprintf( buffer_addr, buffer_size, "%#llx:%#x:%#x\n", p_objecthandle->seq, p_objecthandle->oid, p_objecthandle->ver );
		break;
	case ARG_FID_SEQ:
    		*p_output_size = snprintf( buffer_addr, buffer_size, "%#llx\n", p_objecthandle->seq );
		break;
	case ARG_FID_OID:
    		*p_output_size = snprintf( buffer_addr, buffer_size, "%#x\n", p_objecthandle->oid );
		break;
	case ARG_FID_VER:
    		*p_output_size = snprintf( buffer_addr, buffer_size, "%#x\n", p_objecthandle->ver );
		break;
	       ...
	       void * arg )
{
    ...
    if ( rc != 0 )
    {
        if ( rc == ENODATA )
	{
            DisplayLogLevel( NIV_DEBUG, "Warning: file %s has no stripe information", entry_path.path );
	...
	switch ( (long)arg )
	{
		case ARG_STRIPE_SIZE:
		        *p_output_size = snprintf( buffer_addr, buffer_size, "%llu\n", p_lum->lmm_stripe_size );
			break;

		case ARG_STRIPE_COUNT:
		        *p_output_size = snprintf( buffer_addr, buffer_size, "%u\n", p_lum->lmm_stripe_count );
			break;
			...
			for ( i = 0; i < p_lum->lmm_stripe_count; i++ )
			  {
				...
				if ( i != p_lum->lmm_stripe_count -1  )
					sz = snprintf( curr, buffer_size - *p_output_size, "%u,", p_lum->lmm_objects[i].l_ost_idx );
				else
...
static fsal_xattr_def_t xattr_list[] = {
    {"fid",   print_fid, NULL, XATTR_FOR_ALL | XATTR_RO, (void*)ARG_FULL_FID},
    {"fid_seq", print_fid, NULL,  XATTR_FOR_ALL | XATTR_RO, (void*)ARG_FID_SEQ},
    {"fid_oid", print_fid, NULL, XATTR_FOR_ALL | XATTR_RO, (void*)ARG_FID_OID},
    {"fid_ver", print_fid, NULL, XATTR_FOR_ALL | XATTR_RO, (void*)ARG_FID_VER},
    {"stripe_size", print_stripe, NULL, XATTR_FOR_FILE | XATTR_FOR_DIR, (void*)ARG_STRIPE_SIZE },
    {"stripe_count",  print_stripe, NULL, XATTR_FOR_FILE | XATTR_FOR_DIR, (void*)ARG_STRIPE_COUNT },
    {"storage_targets", print_stripe, NULL, XATTR_FOR_FILE, (void*)ARG_STORAGE_TGT}
};
     ...
     )
{
    ...
    /* get the value */
    rc = xattr_list[xattr_id].get_func( p_objecthandle, p_context, buffer_addr, buffer_size, p_output_size, xattr_list[xattr_id].arg );
