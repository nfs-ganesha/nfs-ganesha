...
fsal_status_t fsal_internal_handle2fd(fsal_op_context_t * p_context,
				      fsal_handle_t * phandle, int *pfd,
				      int oflags);

fsal_status_t fsal_internal_handle2fd_at(int dirfd, fsal_handle_t * phandle,
					 int *pfd, int oflags);
...
fsal_status_t fsal_readlink_by_handle(fsal_op_context_t * p_context,
				      fsal_handle_t * p_handle, char *__buf,
				      int maxlen);
...
fsal_status_t LUSTREFSAL_load_FS_specific_parameter_from_conf(config_file_t
							      in_config,
							      fsal_parameter_t *
							      out_parameter);
...
fsal_status_t LUSTREFSAL_check_quota(char *path,	/* IN */
				     fsal_quota_type_t quota_type, fsal_uid_t fsal_uid);	/* IN */
...
nfsstat4 LUSTREFSAL_DS_read(fsal_handle_t * handle, fsal_op_context_t * context,
			    const stateid4 * stateid, offset4 offset,
			    count4 requested_length, caddr_t buffer,
			    count4 * supplied_length, bool * end_of_file);

nfsstat4 LUSTREFSAL_DS_write(fsal_handle_t * handle,
			     fsal_op_context_t * context,
			     const stateid4 * stateid, offset4 offset,
			     count4 write_length, caddr_t buffer,
...
nfsstat4 LUSTREFSAL_DS_commit(fsal_handle_t * handle,
			      fsal_op_context_t * context, offset4 offset,
			      count4 count, verifier4 writeverf);
#endif
