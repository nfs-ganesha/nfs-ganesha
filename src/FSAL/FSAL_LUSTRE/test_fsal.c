...
int main(int argc, char **argv)
{
  ...
  lustrefsal_op_context_t op_ctx;
  lustrefsal_handle_t root_handle, handle;
  fsal_name_t name;
  fsal_path_t path;
          ...
          for(i = 0; (!error) && (i < number); i++)
            {
             ...
             LogTest("\t%s : %s (cookie %s)", tracebuff, entries[i].name.name,
                     cookiebuff);
            }
          /* preparing next call */
          from = to;
              ...
              do
                {
                 ...
                 LogTest("\t%s : %s (cookie %s)", tracebuff, curr->name.name,
                         cookiebuff);
                }
  ...
  if(test[0] == '8')
    {
      ...
      lustrefsal_handle_t dir_hdl, subdir_hdl;
  ...
  if ( FSAL_IS_ERROR(st) )                              \
  ...
  else if(test[0] == 'A')
    {
     ...
     LogTest("/cea/prot/S/lama/s8/leibovic: handle = %s", tracebuff);
      ...
      memset(&handle, 0, sizeof(lustrefsal_handle_t));
