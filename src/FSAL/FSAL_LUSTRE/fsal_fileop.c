                                ...
                                fsal_attrib_list_t * file_attributes /* [ IN/OUT ] */ )
{
  ...
  fsal_status = LUSTREFSAL_lookup(dirhandle, filename, p_context, &filehandle, file_attributes);
  if(FSAL_IS_ERROR(fsal_status))
    return fsal_status;

  return LUSTREFSAL_open(&filehandle, p_context, openflags, file_descriptor, file_attributes);
}
...
fsal_status_t LUSTREFSAL_open_by_fileid(fsal_handle_t * filehandle,   /* IN */
...
fsal_status_t LUSTREFSAL_close_by_fileid(fsal_file_t * file_descriptor /* IN */ ,
                                   fsal_u64_t fileid)
{
  Return(ERR_FSAL_NOTSUPP, 0, INDEX_FSAL_open_by_fileid);
}
