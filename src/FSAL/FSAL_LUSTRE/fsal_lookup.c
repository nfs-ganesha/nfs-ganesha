     ...
     )
{
    ...
    if ( !p_parent_directory_handle )
    {
        ...
        FSAL_str2path( p_context->export_context->mount_point,  p_context->export_context->mnt_len, &pathfsal );
        TakeTokenFSCall(  );
    ...
    fprintf( stderr, "lookup of %#llx:%#x:%#x/%s\n", p_parent_directory_handle->seq,
             p_parent_directory_handle->oid, p_parent_directory_handle->ver, p_filename->name );
#endif
