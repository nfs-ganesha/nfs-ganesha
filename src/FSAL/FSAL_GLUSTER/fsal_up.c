			    ...
			    enum glfs_upcall_reason reason)
{
	...
	if (!fs) {
		LogCrit(COMPONENT_FSAL_UP,
			"Invalid fs object of the glusterfs_fs(%p)", gl_fs);
		goto out;
	}
	...
	rc = glfs_get_volumeid(fs, vol_uuid, GLAPI_UUID_LENGTH);
	if (rc < 0) {
		LogDebug(COMPONENT_FSAL_UP, "glfs_get_volumeid failed %p", fs);
		goto out;
	}
	...
	LogDebug(COMPONENT_FSAL_UP, "Received event to process for %p", fs);
...
void *GLUSTERFSAL_UP_Thread(void *Arg)
{
	...
	rcu_register_thread();
	(void)snprintf(thr_name, sizeof(thr_name), "fsal_up_%p", gl_fs->fs);
	...
	LogFullDebug(COMPONENT_FSAL_UP,
		     "Initializing FSAL Callback context for %p.", gl_fs->fs);
	...
	while (!atomic_fetch_int8_t(&gl_fs->destroy_mode)) {
		...
		LogFullDebug(COMPONENT_FSAL_UP,
			     "Received upcall event: reason(%d)", reason);
