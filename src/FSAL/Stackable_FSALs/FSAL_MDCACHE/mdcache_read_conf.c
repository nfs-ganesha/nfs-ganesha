...
static struct config_item mdcache_params[] = {
	CONF_ITEM_UI32("NParts", 1, 32633, 7, mdcache_parameter, nparts),
	CONF_ITEM_UI32("Cache_Size", 1, UINT32_MAX, 32633, mdcache_parameter,
		       cache_size),
	...
	CONF_ITEM_UI32("Chunks_HWMark", 1, UINT32_MAX, 1000, mdcache_parameter,
		       chunks_hwmark),
	CONF_ITEM_UI32("Chunks_LWMark", 1, UINT32_MAX, 1000, mdcache_parameter,
		       chunks_lwmark),
	CONF_ITEM_UI32("LRU_Run_Interval", 1, 24 * 3600, 90, mdcache_parameter,
		       lru_run_interval),
	...
	CONF_ITEM_UI32("FD_HWMark_Percent", 0, 100, 90, mdcache_parameter,
		       fd_hwmark_percent),
	CONF_ITEM_UI32("FD_LWMark_Percent", 0, 100, 50, mdcache_parameter,
		       fd_lwmark_percent),
	...
	CONF_ITEM_UI32("Required_Progress", 1, 50, 5, mdcache_parameter,
		       required_progress),
	CONF_ITEM_UI32("Futility_Count", 1, 50, 8, mdcache_parameter,
		       futility_count),
	CONF_ITEM_UI32("Dirmap_HWMark", 1, UINT32_MAX, 10000, mdcache_parameter,
		       dirmap_hwmark),
				...
				struct config_error_type *err_type)
{
	...
	if (param->chunks_lwmark > param->chunks_hwmark) {
		/* The chunks low water mark must be less than the high water
		 * mark.
		 */
		LogCrit(COMPONENT_CONFIG,
			"Chunks_LWMark (%" PRIi32
			") is larger than Chunks_HWMark (%" PRIi32 ")",
			param->chunks_lwmark, param->chunks_hwmark);
				...
				struct config_error_type *err_type)
{
	...
	mdcache_param.dir.avl_detached_max =
		...
		mdcache_param.dir.avl_detached_mult;

	g_max_files_delegatable = (mdcache_param.files_delegatable_percent *
				   mdcache_param.entries_hwmark) /
				  100;
