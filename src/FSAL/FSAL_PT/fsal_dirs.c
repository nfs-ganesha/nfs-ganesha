		 ...
		 fsal_attrib_list_t * p_dir_attributes /* [ IN/OUT ] */)
{
  ...
  FSI_TRACE(FSI_DEBUG, "FSI - Handle = %s", p_fsi_handle->data.handle.f_handle);

  // FSI will open the dir here (so we don't leave it open forever
  if (fsi_get_name_from_handle(p_context, p_fsi_handle->data.handle.f_handle, fsi_name) < 0) {
  ...
  strncpy(p_dir_descriptor->path.path, fsi_name, FSAL_MAX_PATH_LEN);
		 ...
		 fsal_boolean_t   * p_end_of_dir   /* OUT */)
{
  ...
  strncpy(fsi_parent_dir_path, p_dir_descriptor->path.path, FSAL_MAX_PATH_LEN);
  FSI_TRACE(FSI_DEBUG, "Parent dir path --- %s\n", fsi_parent_dir_path);  //should be full path
  ...
  if(start_position.data.cookie == 0)
    ...
    {
      FSI_TRACE(FSI_DEBUG, "FSI - seekdir called - NOT SUPPORTED RIGHT NOW!!!\n");
      rc = errno;
    }
  ...
  *p_nb_entries = 0;
  while(*p_nb_entries < max_dir_entries)
    ...
    memset(fsi_name, 0, FSAL_MAX_PATH_LEN);
      ...
      for(bpos = 0; bpos < rc;)
        {
          ...
          if(FSAL_IS_ERROR
             (st =
             FSAL_str2name(fsi_dname, FSAL_MAX_NAME_LEN,
          ...
          p_fsi_handle = (ptfsal_handle_t *)(&(p_pdirent[*p_nb_entries].handle));
          ...
          ************************/
          ...
          fsi_cache_name_and_handle(fsi_op_context, p_fsi_handle->data.handle.f_handle, fsi_dname); 
          p_pdirent[*p_nb_entries].attributes.mounted_on_fileid = PTFSAL_FILESYSTEM_NUMBER;
          ...
          ((ptfsal_cookie_t *)(&p_pdirent[*p_nb_entries].cookie))->data.cookie = readdir_record;

          FSI_TRACE(FSI_DEBUG, "readdir [%s] rec %d\n",fsi_dname, readdir_record);
