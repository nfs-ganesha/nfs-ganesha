    ...
    )
{
  ...
  if(p_seek_descriptor)
    {
      ...
      switch (p_seek_descriptor->whence)
        ...
        case FSAL_SEEK_CUR:
          ...
          pcall = FALSE;
          // PTFSAL HACK - NOTE - THIS IS HUGE DESIGN POINT - we have been told we will NEVER get
          // this call.... NEVER ... This means our design is to not cache the current file position
          // if a file is temporarily closed - TO BE CONSISTENT WE SHOULD EITHER
          // REMOVE THIS CASE or RETURN ESTALE if we get this case
          // TakeTokenFSCall();
          // rc = lseek(p_file_descriptor->fd, p_seek_descriptor->offset, SEEK_CUR);
          offset = p_seek_descriptor->offset;
          FSI_TRACE(FSI_DEBUG, "FSI - ERROR ERROR ERROR UNEXPECTED SEEK_CUR req %ld loc %ld offset %ld\n",p_seek_descriptor->offset,location,offset);
          errsv = errno;
          ReleaseTokenFSCall();
          break;
  ...
  // FSI should not require token - we can call multithread in parallel
  nb_read = ptfsal_read(p_file_descriptor, buffer, buffer_size, offset, handle_index);
    ...
    )
{
  ...
  *p_write_amount = 0;
  ...
  /* write operation */
  // FSI should not require token - we can call multithread in parallel
  nb_written = ptfsal_write(file_desc, buffer, buffer_size, offset, handle_index);
