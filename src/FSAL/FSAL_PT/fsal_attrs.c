...
extern fsal_status_t ptfsal_xstat_2_fsal_attributes(ptfsal_xstat_t *p_buffxstat,
                                                      fsal_attrib_list_t *p_fsalattr_out);

#ifdef _USE_NFS4_ACL
extern fsal_status_t fsal_acl_2_ptfs_acl(fsal_acl_t *p_fsalacl, ptfsal_xstat_t *p_buffxstat);
#endif                          /* _USE_NFS4_ACL */
      ...
      if(current_attrs.type != FSAL_TYPE_LNK)
        {
          ...
          if(FSAL_IS_ERROR(status))
            ...
            FSI_TRACE(FSI_INFO, "current mode = %o, new mode = %o",
                      fsal2unix_mode(current_attrs.mode), buffxstat.buffstat.st_mode);
            if (fsi_get_name_from_handle(p_context, p_filehandle->data.handle.f_handle, fsi_name) < 0)
            {
              FSI_TRACE(FSI_ERR, "Failed to convert file handle back to filename" );
              FSI_TRACE(FSI_DEBUG, "Handle to name failed for hanlde %s", p_filehandle->data.handle.f_handle);
              Return (ERR_FSAL_BADHANDLE, 0, INDEX_FSAL_setattrs);
            }
            FSI_TRACE(FSI_DEBUG, "Handle to name: %s for handle %s", fsi_name, p_filehandle->data.handle.f_handle);

            if (ptfsal_chmod(p_context, fsi_name, buffxstat.buffstat.st_mode) == -1)
            {
  ...
  if(FSAL_TEST_MASK(wanted_attrs.asked_attributes, FSAL_ATTR_OWNER | FSAL_ATTR_GROUP))
    {
      ...
      FSI_TRACE(FSI_DEBUG, "handle to name: %s for handle %s", fsi_name, p_filehandle->data.handle.f_handle);
      if (ptfsal_chown(p_context, fsi_name, buffxstat.buffstat.st_uid, buffxstat.buffstat.st_gid) == -1)
      {
  ...
  if(FSAL_TEST_MASK(wanted_attrs.asked_attributes, FSAL_ATTR_ATIME | FSAL_ATTR_MTIME))
    {
      ...
      if (fsi_get_name_from_handle(p_context, p_filehandle->data.handle.f_handle, fsi_name) < 0)
        {
          FSI_TRACE(FSI_ERR, "Failed to convert file handle back to filename from cache" );
          FSI_TRACE(FSI_DEBUG, "Handle to name failed for hanlde %s", p_filehandle->data.handle.f_handle);
          Return (ERR_FSAL_BADHANDLE, 0, INDEX_FSAL_setattrs);
        }

      FSI_TRACE(FSI_DEBUG, "Handle to name: %s for handle %s", fsi_name, p_filehandle->data.handle.f_handle); 
      if (ptfsal_ntimes(p_context, fsi_name, buffxstat.buffstat.st_atime, buffxstat.buffstat.st_mtime) == -1)
        {
