...
static unsigned long hash_peer_idx( hash_parameter_t * p_conf, hash_buffer_t * p_key )
{
    ...
    for (name = p_peer->name; *name != '\0'; name++)
        hash = ((hash*p_conf->alphabet_length) + (unsigned long)(*name)) % p_conf->index_size;
...
static int cmp_peers(hash_buffer_t * p_key1, hash_buffer_t * p_key2)
{
    ...
    if (( p_peer1->parent.inum > p_peer2->parent.inum ) || ( p_peer1->parent.dev > p_peer2->parent.dev ))
            return 1;
    else if (( p_peer1->parent.inum < p_peer2->parent.inum )||( p_peer1->parent.dev < p_peer2->parent.dev ))
            return -1;
    else /* same parent */
...
static unsigned long hash_ino_idx( hash_parameter_t * p_conf, hash_buffer_t * p_key )
{
    ...
    hash = ( p_conf->alphabet_length + ((unsigned long)p_ino->inum ^ (unsigned int)p_ino->dev) );
...
static int print_lookup_peer( hash_buffer_t * p_val, char * outbuff )
{
    ...
    return sprintf(outbuff, "parent:%lX.%lu, name:%s", 
                   (unsigned long int)p_peer->parent.dev, (unsigned long int)p_peer->parent.inum, p_peer->name);
}
...
static int print_inode( hash_buffer_t * p_val, char * outbuff )
{
    ...
    return sprintf(outbuff, "device:%lX inode:%lu (gen:%u)", 
		  (unsigned long int)p_ino->dev, (unsigned long int)p_ino->inum, p_ino->generation);
}
...
static int print_fsnode( hash_buffer_t * p_val, char * outbuff )
{
    ...
    if ( p_node->parent_list )
        return sprintf( outbuff, "device:%lX inode:%lu (gen:%u), linkcount:%u, children:%u, first_parent:%lX.%lu, name=%s",
                        (unsigned long int)p_node->inode.dev, (unsigned long int)p_node->inode.inum, p_node->inode.generation, 
                        p_node->n_lookup, p_node->n_children,
                        (unsigned long int)p_node->parent_list->parent.dev, (unsigned long int)p_node->parent_list->parent.inum, 
			p_node->parent_list->name );
    else
        return sprintf( outbuff, "device:%lX inode:%lu (gen:%u), linkcount:%u, children:%u (no parent)",
                        (unsigned long int)p_node->inode.dev,
                        (unsigned long int)p_node->inode.inum,p_node->inode.generation, p_node->n_lookup, p_node->n_children );
...
fsnode_t * h_del_lookup(ino_t parent_inode, dev_t parent_dev, unsigned int parent_gen, char * name, int * p_rc)
{
    ...
    if (rc == HASHTABLE_SUCCESS )
    {
        ...
        for ( p_lpeer = p_node->parent_list ;
              p_lpeer != NULL ;
              p_lpeer = p_lpeer->p_next )
        {
            if ( p_lpeer == (lookup_peer_t*)buffkey_out.pdata )
            {
                ...
                {
                    DisplayLog( "NAMESPACE MANAGER: An incompatible direntry was found. In node: %lu.%lu (gen:%u) ,%s  Deleted:%lu.%lu (gen:%u),%s",
                                p_lpeer->parent.inum, p_lpeer->parent.dev, p_lpeer->parent.generation,
                                p_lpeer->name, parent_dev, parent_inode, parent_gen, name );
                    /* remove it anyway... */
                }
...
int NamespaceInit( ino_t root_inode, dev_t root_dev, unsigned int * p_root_gen  )
{
    ...
    STUFF_PREALLOC( peer_pool,
                    POOL_CHUNK_SIZE,
                    lookup_peer_t,
                    p_next ) ;

    STUFF_PREALLOC( node_pool,
                    POOL_CHUNK_SIZE,
                    fsnode_t,
                    p_next ) ;
...
#ifdef _DEBUG_FSAL        
    printf("namespace: Root=%lX.%ld (gen:%u)\n", root_dev, root_inode, root->inode.generation );
#endif
                  ...
                  ino_t   entry_ino, dev_t entry_dev, unsigned int * p_new_gen )
{
    ...
    switch ( rc )
    {
        case HASHTABLE_SUCCESS:
        {
            ...
            if ( rc == HASHTABLE_ERROR_NO_SUCH_KEY )
            {
                ...
                p_lpeer = h_insert_new_lookup( parent_ino, parent_dev, parent_gen, name, p_node, FALSE );
                if ( !p_lpeer ) return EFAULT;
            ...
            else if ( rc == HASHTABLE_SUCCESS )
            {
                ...
                if ( (p_node_exist->inode.inum == entry_ino) && (p_node_exist->inode.dev == entry_dev) )
                {
                ...
                else
                {
                    ...
                    DisplayLog( "NAMESPACE MANAGER: An incompatible direntry was found. Existing: %lX.%lu,%s->%lX.%lu  New:%lX.%lu,%s->%lX.%lu",
                                parent_dev, parent_ino, name, p_node_exist->inode.dev, p_node_exist->inode.inum,
                                parent_dev, parent_ino, name, entry_ino );
                    return EEXIST;
                }                
            }
            else /* other error */
    ...
    printf("namespace: Entry %lX.%ld (gen:%u)  has now link count = %u\n",
            p_node->inode.dev, p_node->inode.inum, p_node->inode.generation, p_node->n_lookup );
#endif    
                    ...
                    ino_t parent_entry_tgt, dev_t tgt_dev, unsigned int tgtgen, char * name_tgt )
{
    ...
    rc = NamespaceAdd_nl( parent_entry_tgt, tgt_dev, tgtgen, name_tgt, p_node->inode.inum, p_node->inode.dev, &new_gen );
...
int NamespacePath(ino_t entry, dev_t dev, unsigned int gen, char * path)
{
    ...
    do
    {
	    ...
	    {
#ifdef _DEBUG_FSAL        
		printf("namespace: %lX.%ld not found\n", (unsigned long)dev, (unsigned long)entry);
#endif
                return ENOENT;
	    }
            else
        ...
        else
        {
            ...
            printf("lookup peer found: (%lX.%ld,%s)\n",
                    ...
                    p_node->parent_list->parent.inum,
                    p_node->parent_list->name);
#endif            
            ...
            {
                    DisplayLog( "NAMESPACE MANAGER: loop detected in namespace: %lX.%ld/%s = %lX.%ld",
