...
fsal_status_t FSAL_opendir(
  ...
  rc = NamespacePath( dir_handle->inode, dir_handle->device, dir_handle->validator,
          object_path );
  if ( rc ) Return( ERR_FSAL_STALE, rc, INDEX_FSAL_opendir);
                         ...
                         off_t off )
{
	...
	if ( stbuf->st_ino == 0 )
	{
		DisplayLogJdLevel(fsal_log, NIV_DEBUG,"WARNING in fill_dirent: Filesystem doesn't provide inode numbers !!!");
	}

        to_be_filled->handle.inode = stbuf->st_ino;
        to_be_filled->handle.device = stbuf->st_dev;
        FSAL_str2name(name,strlen(name)+1,&(to_be_filled->name) );
        to_be_filled->cookie = off;
...
#ifdef _DEBUG_FSAL
        printf("getattr_mask = %X, recupere = %X, status=%d, inode=%llX.%llu, type=%d, posixmode=%#o, mode=%#o\n",
                getattr_mask, to_be_filled->attributes.asked_attributes,
                status.major, to_be_filled->attributes.fsid.major, to_be_filled->attributes.fileid,
                to_be_filled->attributes.type, tmp_statbuff.st_mode, to_be_filled->attributes.mode );
#endif                
        if ( FSAL_IS_ERROR(status) )
        {
            FSAL_CLEAR_MASK( to_be_filled->attributes.asked_attributes );
            /* set getattr error bit in attr mask */
            FSAL_SET_MASK( to_be_filled->attributes.asked_attributes,
                FSAL_ATTR_RDATTR_ERR );
            err = TRUE;
        }
    }
    ...
    if ( !stbuf
         ...
         || to_be_filled->attributes.mode == 0
         || to_be_filled->attributes.numlinks == 0 )
    {
        FSAL_CLEAR_MASK( to_be_filled->attributes.asked_attributes );
        /* we only known entry name, we tag it for a later lookup.
         */
        to_be_filled->handle.inode = INODE_TO_BE_COMPLETED;
        FSAL_str2name(name,strlen(name)+1,&(to_be_filled->name) );
        to_be_filled->cookie = off;
    }
...
fsal_status_t FSAL_readdir(
  ...
  if ( rc ) Return( ERR_FSAL_STALE, rc, INDEX_FSAL_readdir);
  ...
  reqbuff.begin_off    = start_position;
  reqbuff.curr_off     = FSAL_READDIR_FROM_BEGINNING;
  ...
  for ( i = 0; i < reqbuff.nb_entries ; i++ )
  {
...
#endif
          st = FSAL_lookup( &dir_descriptor->dir_handle,
                            &pdirent[i].name,
                            &dir_descriptor->context,
                            &pdirent[i].handle,
                            &pdirent[i].attributes );
      ...
      else
      {
          ...
          if ( strcmp( pdirent[i].name.name, "." )
                && strcmp( pdirent[i].name.name, ".." ) )
          {
#ifdef _DEBUG_FSAL              
               printf("adding entry to namespace: %lX.%ld %s\n",
                       pdirent[i].handle.device,
                       pdirent[i].handle.inode,  pdirent[i].name.name);
#endif
               pdirent[i].handle.validator = pdirent[i].attributes.ctime.seconds;
               
               NamespaceAdd( dir_descriptor->dir_handle.inode,
                             dir_descriptor->dir_handle.device,
                             dir_descriptor->dir_handle.validator,
                             pdirent[i].name.name,
                             pdirent[i].handle.inode,
                             pdirent[i].handle.device,
                             &(pdirent[i].handle.validator) );
          }
      }
...
fsal_status_t FSAL_closedir(
  ...
  rc = NamespacePath( dir_descriptor->dir_handle.inode,
                      dir_descriptor->dir_handle.device,
                      dir_descriptor->dir_handle.validator, dir_path );
  if ( rc ) Return( ERR_FSAL_STALE, rc, INDEX_FSAL_closedir);
