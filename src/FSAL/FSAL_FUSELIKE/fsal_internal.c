                                     ...
                                     fs_common_initinfo_t  *  fs_common_info )
{
  ...
  /* sanity check */
  if (!fsal_info || !fs_common_info) ReturnCode(ERR_FSAL_FAULT,0);
  ...
  DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  lease_time  = %u.%u     ",global_fs_info.lease_time.seconds, global_fs_info.lease_time.nseconds);
  ...
  DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  auth_exportpath_xdev  = %d  ",global_fs_info.auth_exportpath_xdev);
  DisplayLogJdLevel( fsal_log, NIV_DEBUG,"  xattr_access_rights = %#o ",global_fs_info.xattr_access_rights);
...
int fsal_set_thread_context( fsal_op_context_t * p_ctx )
{
  ...
  {
    int rc = errno;
    DisplayErrorJd( fsal_log, ERR_SYS, ERR_PTHREAD_ONCE, rc );
    return rc;
  }
