...
fsal_status_t FSAL_create(
    ...
    rc = NamespacePath( parent_directory_handle->inode,
                        ...
                        parent_path );
    if ( rc ) Return( ERR_FSAL_STALE, rc, INDEX_FSAL_create);
    ...
    if ( p_fs_ops->create )
    {
     ...
     printf("Call to create( %s, %#o, %#X )\n", child_path, mode, dummy.flags );
        ...
        if ( rc ) Return( fuse2fsal_error(rc,TRUE), rc, INDEX_FSAL_create );
    ...
    {
        TakeTokenFSCall();
        rc = p_fs_ops->chown( child_path, p_context->credential.user , p_context->credential.group );
        ReleaseTokenFSCall();
...
fsal_status_t FSAL_mkdir(
    ...
    */
    ...
    rc = NamespacePath( parent_directory_handle->inode,
                        ...
                        parent_directory_handle->validator,
                        parent_path );
    if ( rc ) Return( ERR_FSAL_STALE, rc, INDEX_FSAL_mkdir);
    ...
    if ( rc ) Return( fuse2fsal_error(rc,TRUE), rc, INDEX_FSAL_mkdir );
...
fsal_status_t FSAL_link(
  ...
  rc = NamespacePath( target_handle->inode,
                      ...
                      target_handle->validator,
                      target_path );
  if ( rc ) Return( ERR_FSAL_STALE, rc, INDEX_FSAL_link);
  ...
  if ( rc ) Return( fuse2fsal_error(rc,TRUE), rc, INDEX_FSAL_link );
  ...
  {
      DisplayLogJdLevel(fsal_log, NIV_MAJ, "A wrong behaviour has been detected is FSAL_link: An object and its hardlink don't have the same generation id");
  }
