...
int main( int argc, char ** argv )
{
    ...
    if ( rc ) { printf("NamespaceInit rc=%d\n", rc); exit(1); }
    ...
    for ( i=0; i< 2; i++ )
    {
        ...
        {
            rc = NamespaceAdd( p_test->parent_inode, DEV, gen, p_test->name, p_test->entry_inode, DEV, &gen );
            printf("NamespaceAdd(%lu,%s->%lu) = %d\n", p_test->parent_inode,
                    p_test->name, p_test->entry_inode, rc );
            if (rc) exit(1); /* This is an error */
        }
        ...
        for ( p_test = testset; p_test->name != NULL; p_test++ )
        {
            rc = NamespacePath( p_test->entry_inode, DEV, gen, path );
            if ( rc == 0 )
                { printf( "NamespacePath(%lu) => \"%s\"\n",p_test->entry_inode, path  ); exit(1); }
            else if ( rc != ENOENT )
                { printf( "NamespacePath(%lu) rc=%d\n",p_test->entry_inode, rc  ); exit(1); }
            else
    ...
    if (rc) { printf("NamespaceAdd error %d line %d\n", rc, __LINE__ - 1 ); exit(1);}
    ...
    for ( i=0; i < 3; i++ )
    {
        ...
        printf("NamespaceAdd(%lu,%s->%lu) = %d\n", ROOT_INODE+2, name, ROOT_INODE+3, rc );
        if ( rc ) exit(1);
        
        rc = NamespacePath( ROOT_INODE+3, DEV, gen, path );
        if ( rc )
            { printf( "NamespacePath(%lu) rc=%d\n", ROOT_INODE+3, rc ); exit(1); }
        else
            printf( "NamespacePath(%lu) => \"%s\"\n",ROOT_INODE+3, path );        
