                 ...
                 const fsal_name_t *p_name2)
{
  ...
  return (strncmp(p_name1->name, p_name2->name, sizeof(p_name1->name)));
...
int FSAL_pathcmp(fsal_path_t * p_path1, fsal_path_t * p_path2)
{
  ...
  return (strncmp(p_path1->path, p_path2->path, sizeof(p_path1->path)));
...
fsal_status_t FSAL_buffdesc2name(fsal_buffdesc_t * in_buf, fsal_name_t * out_name)
{
  ...
  if(in_buf->len >= sizeof(out_name->name))
    ReturnCode(ERR_FSAL_NAMETOOLONG, 0);

  out_name->len = in_buf->len;

  /* copies the string */
  memcpy(out_name->name, in_buf->pointer, in_buf->len);

  /* sets the terminating \0 */
  out_name->name[in_buf->len] = '\0';
...
fsal_status_t FSAL_buffdesc2path(fsal_buffdesc_t * in_buf, fsal_path_t * out_path)
{
  ...
  if(in_buf->len >= sizeof(out_path->path))
    ReturnCode(ERR_FSAL_NAMETOOLONG, 0);

  out_path->len = in_buf->len;

  /* copies the string */
  memcpy(out_path->path, in_buf->pointer, in_buf->len);

  /* sets the terminating \0 */
  out_path->path[in_buf->len] = '\0';
