...
int nodelist_common_string_get_token(char* string,char* separators_list,int token_id,char** p_token){
  int fstatus=-1;
  int token_nb=0;
  fstatus=_nodelist_common_string_get_token_common(string,separators_list,&token_nb,token_id,p_token);
  if(*p_token!=NULL)
    fstatus=0;
  else
    fstatus=-1;
...
int nodelist_common_string_get_tokens_quantity(char* string,char* separators_list,int* p_token_nb){
  int fstatus=-1;
  fstatus=_nodelist_common_string_get_token_common(string,separators_list,p_token_nb,0,NULL);
  return fstatus;
}
/*
  -------------------------------------
  ...
  if(string!=NULL && separators_list!=NULL){
	...
	*/
	for(i=0;i<separators_list_length;i++){
	  current_pointer=get_next_token(working_string,*(separators_list+i));
