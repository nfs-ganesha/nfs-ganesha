...
int
_nodelist_range_compare(const void* a1,const void* a2){
...
int
nodelist_range_intersection(nodelist_range_t* r1,nodelist_range_t* r2,nodelist_range_t* rout)
{
  int fstatus=-1;
  if(!nodelist_range_check(r1) || !nodelist_range_check(r2))
    return fstatus;
...
int
nodelist_rangelist_init(nodelist_rangelist_t* array)
{
  int fstatus=-1;
  array->pre_allocated_ranges=DEFAULT_RANGELIST_SIZE;
  array->ranges_nb=0;
  array->array=(nodelist_range_t*)malloc(array->pre_allocated_ranges*sizeof(nodelist_range_t));
  if(array->array!=NULL)
    {
      fstatus=0;
    }
...
int
nodelist_rangelist_free_contents(nodelist_rangelist_t* array)
{
  int fstatus=0;
  array->pre_allocated_ranges=0;
  array->ranges_nb=0;
...
int
nodelist_rangelist_incremente_size(nodelist_rangelist_t* array)
{
  
  int fstatus=-1;
  array->pre_allocated_ranges+=DEFAULT_RANGELIST_INC_SIZE;
  array->array=(nodelist_range_t*)realloc(array->array,array->pre_allocated_ranges*sizeof(nodelist_range_t));
  if(array->array!=NULL)
    fstatus=0;
  return fstatus;
}
...
int
nodelist_rangelist_add_range(nodelist_rangelist_t* array,nodelist_range_t* rin)
{
  int fstatus=-1;
	  ...
	  for(id=0;id<array->ranges_nb;id++)
	    {
	      ...
	      /* to the next range */
	      if(!nodelist_range_intersects(&(array->array[id]),&r) && !nodelist_range_contiguous(&(array->array[id]),&r))
		{
		  if(work_array.ranges_nb==work_array.pre_allocated_ranges)
		    nodelist_rangelist_incremente_size(&work_array);
		  memcpy(work_array.array+work_array.ranges_nb,&(array->array[id]),sizeof(nodelist_range_t));
...
int
nodelist_rangelist_add_rangelist(nodelist_rangelist_t* array,nodelist_rangelist_t* rlin)
{
  int fstatus=0;
...
int
nodelist_rangelist_remove_range(nodelist_rangelist_t* array,nodelist_range_t* rin)
{
  int fstatus=-1;
...
int
nodelist_rangelist_remove_rangelist(nodelist_rangelist_t* array,nodelist_rangelist_t* rlin)
{
  int fstatus=0;
...
int
nodelist_rangelist_add_list(nodelist_rangelist_t* array,char* list)
{
  int fstatus=0;
  char* in_list;
  size_t in_list_size;
...
int
nodelist_rangelist_sort(nodelist_rangelist_t* array)
{
  int fstatus=-1;
  qsort(array->array,array->ranges_nb,sizeof(nodelist_range_t),_nodelist_range_compare);
  return fstatus;
}
...
int
nodelist_rangelist_includes(nodelist_rangelist_t* a1,nodelist_rangelist_t* a2)
{
  int fstatus=0;
  int i,j;
...
int
nodelist_rangelist_show(nodelist_rangelist_t* array)
{
  long int id;
  fprintf(stdout,"----------------------------------\n");
