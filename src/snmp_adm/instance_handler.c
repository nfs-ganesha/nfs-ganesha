			    ...
			    netsnmp_request_info *requests)
{
	...
	case MODE_GET:
		 ...
		 */
		/* call the function */
		err=info->function_info.get_set->getter(&var, info->function_info.get_set->opt_arg );
		if(err) {
			snmp_set_var_typed_value(requests->requestvb, ASN_OCTET_STR,
	...
	case MODE_SET_ACTION:
		...
		case SNMP_ADM_INTEGER:
			...
			break;
		case SNMP_ADM_STRING:	
			strncpy(var.string,(char*)(requests->requestvb->val.string),SNMP_ADM_MAX_STR);
			break;
		case SNMP_ADM_REAL:
		...
		if(!err) {
			/* call the function */
			err_fct=info->function_info.get_set->setter(&var, info->function_info.get_set->opt_arg);
...
static void* launch_proc(void *arg)
{
	...
	err=pinfo->myproc((const snmp_adm_type_union **)pinfo->inputs,pinfo->outputs,pinfo->opt_arg);
			  ...
			  netsnmp_request_info *requests)
{
	...
	for(info=register_info_list;info;info=info->next)
		if(info->type==PROC && 
		   info->function_info.proc->num==num_proc)
			break;
	...
	case MODE_GET:
		snmp_set_var_typed_value(requests->requestvb, ASN_INTEGER,
					 (u_char *) &(pinfo->trigger), 
					 sizeof(pinfo->trigger));
		break;
	case MODE_SET_ACTION:
