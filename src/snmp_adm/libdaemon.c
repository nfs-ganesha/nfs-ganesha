 ...
 */
static int register_meta(oid *myoid, int len,char *name, char *desc,netsnmp_handler_registration** tab_reg )
{
	int err1,err2;
...
static int register_proc_instance(const register_proc *instance)
{
	...
	*/
	...
	for(i=0;i<instance->nb_in;i++) {
		...
		myoid[len-1]=i;
		reg_scal(myoid,len,p_info->inputs[i]->string,instance->type_in[i], SNMP_ADM_ACCESS_RW,info->reg+tab_reg_offset);
		tab_reg_offset+=2;
	}
	     /* output */
	for(i=0;i<instance->nb_out;i++) {
		...
		myoid[len-1]=i;
		reg_scal(myoid,len,p_info->outputs[i]->string,instance->type_out[i], SNMP_ADM_ACCESS_RO,info->reg+tab_reg_offset);
		tab_reg_offset+=2;
	}
	return err1 || err2;
}
...
int snmp_adm_register_scalars(int branch,register_scal *tab,int len)
{
	...
	for(i=0;i<len;i++){
		...
		if(err) {
			snmp_adm_log("ERROR registering %s %s",((branch==CONF_OID)?"conf ":"stat "),tab[i].label);
			return 1;
		}else			
			snmp_adm_log("register %s %s",((branch==CONF_OID)?"conf ":"stat "),tab[i].label);
	}
	registered=1;
	return 0;
}
...
int snmp_adm_register_get_set_function(int branch, register_get_set *tab,int len)
{
	...
	for(i=0;i<len;i++) {
		...
		if(err) {
			snmp_adm_log("ERROR registering getset %s %s",((branch==CONF_OID)?"conf ":"stat "),tab[i].label);
			return 1;
		}else
			snmp_adm_log("register getset %s %s",((branch==CONF_OID)?"conf ":"stat "),tab[i].label);		
...
{	
	...
	pthread_create(&polling_threads[polling_list_size],NULL,polling_fct,&polling_args[polling_list_size]);
	polling_list_size++;
