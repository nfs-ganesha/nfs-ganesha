...
bool_t svcauth_wrap_dummy( XDR       * xdrs,
                           xdrproc_t   xdr_func,
                           caddr_t     xdr_ptr) ;
...
static bool_t Rendezvous_request( register SVCXPRT * xprt)
{
	...
	xprt->xp_addrlen = len;
        ...
        if( ( rc = pthread_create( &sockmgr_thrid, &attr_thr, rpc_tcp_socket_manager_thread, (void *)((unsigned long)xprt->xp_fd) ) ) != 0 )
           return FALSE ;
#else
        ...
        if( ( rc = pthread_create( &sockmgr_thrid, &attr_thr, rpc_tcp_socket_manager_thread, (void *)((unsigned long)xprt->xp_sock) ) ) != 0 )
           return FALSE ;
  ...
  if (msg->rm_reply.rp_stat == MSG_ACCEPTED &&
      msg->rm_reply.rp_acpt.ar_stat == SUCCESS)
    {
      ...
      if (!xdr_replymsg(xdrs, msg) ||
          !SVCAUTH_WRAP( NULL, xdrs, xdr_proc, xdr_where))
        return (FALSE);
    }
