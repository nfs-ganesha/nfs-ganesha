...
unsigned long gss_ctx_hash_func( hash_parameter_t * p_hparam, hash_buffer_t * buffclef ) 
{
  ...
  hash_func = (unsigned long)pgss_ctx->mech_type + (unsigned long)pgss_ctx->internal_ctx_id ;
...
int compare_gss_ctx(  hash_buffer_t * buff1, hash_buffer_t * buff2 )
{
  ...
  return ( (pgss_ctx1->internal_ctx_id == pgss_ctx2->internal_ctx_id ) && (pgss_ctx1->mech_type == pgss_ctx2->mech_type ) )?0:1 ;
} /* compare_gss_ctx */
...
int display_gss_ctx( hash_buffer_t * pbuff, char * str ) 
{
  ...
  return sprintf( str, "0x%lx%lx", (unsigned long)pgss_ctx->internal_ctx_id, (unsigned long)pgss_ctx->mech_type );
} /* display_gss_ctx */
...
int display_gss_svc_data( hash_buffer_t * pbuff, char * str ) 
{
  ...
  return sprintf( str, "established=%u ctx=(%u) sec=(mech=%p,qop=%u,svc=%u,cred=%p,flags=%u) cname=(%u|%s) seq=%u win=%u seqlast=%u seqmask=%u",
                  gd->established,
                  gd->ctx_exported.length,
		  gd->sec.mech,
		  gd->sec.qop,
		  gd->sec.svc,
		  gd->sec.cred,
...
int Gss_ctx_Hash_Set( gss_union_ctx_id_desc * pgss_ctx, struct svc_rpc_gss_data * gd ) 
{
  ...
  if( ( buffval.pdata = (caddr_t)Mem_Alloc( sizeof( struct svc_rpc_gss_data_stored ) ) ) == NULL )
    return 0 ;
  ...
  if( HashTable_Test_And_Set( ht_gss_ctx, &buffkey, &buffval, HASHTABLE_SET_HOW_SET_NO_OVERWRITE ) != HASHTABLE_SUCCESS )
    return 0 ;
