...
void nfs_rpc_cb_pkginit(void)
{
	...
	rpc_call_pool =
	    pool_init("RPC Call Pool", sizeof(rpc_call_t), pool_basic_substrate,
		      NULL, nfs_rpc_init_call, NULL);
	if (!(rpc_call_pool)) {
		LogCrit(COMPONENT_INIT, "Error while allocating rpc call pool");
...
{
	if (!strncmp
	    (netid, netid_nc_table[_NC_TCP].netid,
	     netid_nc_table[_NC_TCP].netid_len))
		return (_NC_TCP);

	if (!strncmp
	    (netid, netid_nc_table[_NC_TCP6].netid,
	     netid_nc_table[_NC_TCP6].netid_len))
		return (_NC_TCP6);

	if (!strncmp
	    (netid, netid_nc_table[_NC_UDP].netid,
	     netid_nc_table[_NC_UDP].netid_len))
		return _NC_UDP;

	if (!strncmp
	    (netid, netid_nc_table[_NC_UDP6].netid,
	     netid_nc_table[_NC_UDP6].netid_len))
		return _NC_UDP6;

	if (!strncmp
	    (netid, netid_nc_table[_NC_RDMA].netid,
	     netid_nc_table[_NC_RDMA].netid_len))
		return _NC_RDMA;

	if (!strncmp
	    (netid, netid_nc_table[_NC_RDMA6].netid,
	     netid_nc_table[_NC_RDMA6].netid_len))
		return _NC_RDMA6;

	if (!strncmp
	    (netid, netid_nc_table[_NC_SCTP].netid,
	     netid_nc_table[_NC_SCTP].netid_len))
		return _NC_SCTP;

	if (!strncmp
	    (netid, netid_nc_table[_NC_SCTP6].netid,
				      ...
				      const char *uaddr)
{
	...
	case _NC_UDP:
		/* IPv4 (ws inspired) */
		if (sscanf
		    (uaddr, "%u.%u.%u.%u.%u.%u", &bytes[1], &bytes[2],
	...
	case _NC_UDP6:
		/* IPv6 (ws inspired) */
		if (sscanf
		    (uaddr, "%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x.%u.%u", &bytes[1],
		     &bytes[2], &bytes[3], &bytes[4], &bytes[5], &bytes[6],
		     &bytes[7], &bytes[8], &bytes[9], &bytes[10]) == 10) {
						...
						int *fd, int *proto)
{
	...
	*proto = -1;
	...
	case AF_INET:
		...
		code =
		    connect(nfd, (struct sockaddr *)sin,
	...
	case AF_INET6:
		...
		code =
		    connect(nfd, (struct sockaddr *)sin6,
					      ...
					      nfs_client_cred_t * cred)
{
	...
	if (chan->gss_sec.svc != RPCSEC_GSS_SVC_NONE) {
		...
		auth =
		    authgss_create_default(chan->clnt, hprinc, &chan->gss_sec);
...
int nfs_rpc_create_chan_v40(nfs_client_id_t * clientid, uint32_t flags)
{
	...
	case IPPROTO_TCP:
		...
		chan->clnt =
		    clnt_vc_create(fd, &raddr, clientid->cid_cb.v40.cb_program,
	...
	case IPPROTO_UDP:
		...
		chan->clnt =
		    clnt_dg_create(fd, &raddr, clientid->cid_cb.v40.cb_program,
				   NFS_CB /* Errata ID: 2291 */ ,
				   0, 0);
		break;
	default:
			    ...
			    callback_sec_parms4 * sec_parms)
{
	 ...
	 */
	chan->clnt =
	    clnt_vc_create_svc(session->xprt, session->cb_program,
			       NFS_CB /* Errata ID: 2291 */ ,
...
rpc_call_channel_t *nfs_rpc_get_chan(nfs_client_id_t * clientid, uint32_t flags)
{
	...
	if (clientid->cid_minorversion == 0) {
		...
		if (!chan->clnt) {
			if (nfs_rpc_create_chan_v40(clientid, flags) == 0) {
			}
		}
	} else {		/* 1 and higher */
			   ...
			   bool locked)
{
	...
	stat =
	    clnt_call(chan->clnt, chan->auth, CB_NULL, (xdrproc_t) xdr_void,
...
int32_t nfs_rpc_dispatch_call(rpc_call_t * call, uint32_t flags)
{
	...
	call->stat =
	    clnt_call(call->chan->clnt, call->chan->auth, CB_COMPOUND,
