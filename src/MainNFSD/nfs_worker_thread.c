...
void free_args(nfs_request_t *reqdata)
{
	...
	GSH_AUTO_TRACEPOINT(nfs_rpc, end, TRACE_INFO, "Rpc end. request: {}",
			    reqdata);
}
...
{
	GSH_AUTO_TRACEPOINT(nfs_rpc, op_end, TRACE_INFO, "Op end. request: {}",
			    reqdata);
}
					      ...
					      bool retry)
{
	...
	if (xprt->xp_tls.tls_pending) {
		...
		if (auth_rc == AUTH_OK) {
			...
			LogDebug(COMPONENT_DISPATCH,
				 "Processed AUTH_TLS probe on fd %" PRId32,
				 xprt->xp_fd);
			return SVC_STAT(xprt);
		} else {
			/* Invalid AUTH_TLS probe */
			LogDebug(COMPONENT_DISPATCH,
				 "Invalid AUTH_TLS probe, auth_result=%" PRId32,
				 auth_rc);
...
retry_after_drc_suspend:
	...
	if (op_ctx->ctx_export != NULL) {
		...
		if (((op_ctx->export_perms.options & EXPORT_OPTION_TLS) != 0) &&
		    (!(xprt->xp_tls.tls_established))) {
			LogInfoAlt(COMPONENT_DISPATCH, COMPONENT_EXPORT,
				   "[TLS] Auth not allowed on ExportId %" PRId32
		...
		if (((op_ctx->export_perms.options & EXPORT_OPTION_MTLS) !=
		     0) &&
		    (!(xprt->xp_tls.tls_established) || !(xprt->xp_tls.mtls))) {
			LogInfoAlt(
				COMPONENT_DISPATCH, COMPONENT_EXPORT,
				"[TLS] NFSAUTH not allowed on ExportId %" PRId32
...
enum xprt_stat nfs_rpc_valid_NFS(struct svc_req *req)
{
	...
	GSH_AUTO_TRACEPOINT(
		nfs_rpc, valid, TRACE_INFO,
		"Valid nfs req. xprt_ptr: {}, cb_prog: {}, cb_vers: {} cb_proc: {}",
		reqdata->svc.rq_xprt, req->rq_msg.cb_prog, req->rq_msg.cb_vers,
		reqdata->svc.rq_msg.cb_proc);
