...
static void queue_invalidate(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->invalidate(args->export,
						  &args->obj,
						  args->flags);
...
static void queue_update(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->update(args->export,
					      &args->obj,
					      &args->attr,
					      args->flags);
...
static void queue_lock_grant(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->lock_grant(args->export,
						  &args->file,
						  args->owner,
						  &args->lock_param);
...
static void queue_lock_avail(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->lock_avail(args->export,
						  &args->file,
						  args->owner,
						  &args->lock_param);
...
static void queue_link(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->link(args->export,
					    &args->dir,
					    args->name,
					    (args->target.addr ?
					     &args->target :
...
int up_async_link(
	struct fridgethr *fr,
	struct fsal_export *export,
	const struct gsh_buffdesc *dir,
	const char *name,
	const struct gsh_buffdesc *target,
	void (*cb)(void *, cache_inode_status_t),
	void *cb_arg)
{
	...
	args = gsh_malloc(sizeof(struct link_args) + dir->len +
			  namelen + (target ? target->len : 0));
	if (!args) {
		rc = ENOMEM;
		goto out;
	}
...
static void queue_unlink(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->unlink(args->export,
					      &args->dir,
					      args->name);
...
static void queue_move_from(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->move_from(args->export,
						 &args->dir,
						 args->name);
...
static void queue_move_to(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->move_to(args->export,
					       &args->dir,
					       args->name,
					       (args->target.addr ?
						&args->target :
	...
	void *cb_arg)
{
	...
	args = gsh_malloc(sizeof(struct move_to_args) + dir->len
			  + namelen + (target ? target->len : 0));
	if (!args) {
		rc = ENOMEM;
		goto out;
...
static void queue_rename(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->rename(args->export,
					      &args->dir,
					      args->old,
					      args->new);
	...
	void *cb_arg)
{
	...
	args = gsh_malloc(sizeof(struct rename_args) + dir->len
			  + oldlen + newlen);
	if (!args) {
		rc = ENOMEM;
		goto out;
...
static void queue_layoutrecall(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->layoutrecall(
		args->export,
		&args->handle,
		args->layout_type,
		args->changed,
...
static void queue_notify_device(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->notify_device(args->export,
						     args->notify_type,
						     args->layout_type,
						     args->devid,
						     args->immediate);
...
static void queue_delegrecall(struct fridgethr_context *ctx)
{
	...
	status = args->export->up_ops->delegrecall(args->export,
						   &args->handle);
