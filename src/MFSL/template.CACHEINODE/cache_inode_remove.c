                                            ...
                                            int                    use_mutex )
{
  ...
  if( status == CACHE_INODE_SUCCESS )
    {
      ...
      if( FSAL_IS_ERROR( fsal_status ) ) 
		...
		DisplayLog( "cache_inode_remove: Stale FSAL FH detected for pentry %p", pentry ) ;

		if( cache_inode_kill_entry( pentry, ht, pclient, &kill_status ) != CACHE_INODE_SUCCESS ) 
		  DisplayLog( "cache_inode_remove: Could not kill entry %p, status = %u", pentry, kill_status ) ;
  ...
  if( pentry->internal_md.type == DIR_BEGINNING )
  ...
  else if( pentry->internal_md.type == DIR_CONTINUE )
    {
      ...
      pentry->object.dir_cont.pdir_begin->object.dir_begin.attributes.mtime.seconds  = pasyncopdesc->op_time.tv_sec ;
      pentry->object.dir_cont.pdir_begin->object.dir_begin.attributes.mtime.nseconds = pasyncopdesc->op_time.tv_usec ;
      pentry->object.dir_cont.pdir_begin->object.dir_begin.attributes.ctime.seconds  = pasyncopdesc->op_time.tv_sec ;
      pentry->object.dir_cont.pdir_begin->object.dir_begin.attributes.ctime.nseconds = pasyncopdesc->op_time.tv_usec ;
      ...
      if( remove_attr.numlinks > 1 )
        {
          switch( to_remove_entry->internal_md.type )
            ...
            case SYMBOLIC_LINK:
              ...
              to_remove_entry->object.symlink.attributes.ctime.seconds  = pasyncopdesc->op_time.tv_sec ;
              to_remove_entry->object.symlink.attributes.ctime.nseconds = pasyncopdesc->op_time.tv_usec ;
              to_remove_numlinks = to_remove_entry->object.symlink.attributes.numlinks ;
              break ;

            case REGULAR_FILE:
              to_remove_entry->object.file.attributes.numlinks -= 1 ;
              to_remove_entry->object.file.attributes.ctime.seconds  = pasyncopdesc->op_time.tv_sec ;
              to_remove_entry->object.file.attributes.ctime.nseconds = pasyncopdesc->op_time.tv_usec ; 
            ...
            case FIFO_FILE:
              ...
              to_remove_numlinks = to_remove_entry->object.special_obj.attributes.numlinks ;
              break ;
  ...
  if( to_remove_numlinks == 0 )
	...
	if( to_remove_entry->object.file.pentry_content != NULL )
	    ...
	    /* Something is to be deleted, release the cache data entry */
            if( cache_content_release_entry( (cache_content_entry_t  *)to_remove_entry->object.file.pentry_content ,
	 	...
	 	DisplayLogJdLevel( pclient->log_outputs, NIV_EVENT, 
				   "pentry %p, named %s could not be released from data cache, status=%d",
				    to_remove_entry, pnode_name->name, cache_content_status ) ;  
              }
         }
      }

      if( ( pfsal_handle_remove =  cache_inode_get_fsal_handle( to_remove_entry, pstatus ) ) == NULL )
        {
          if( use_mutex )
            V( to_remove_entry->lock ) ;
      ...
      if( to_remove_entry->gc_lru_entry != NULL )
        {
          if( LRU_invalidate( to_remove_entry->gc_lru, to_remove_entry->gc_lru_entry ) != LRU_LIST_SUCCESS )
            {
      ...
      if( to_remove_entry->internal_md.type == DIR_BEGINNING )
        {
          ...
          RELEASE_PREALLOC( to_remove_entry->object.dir_begin.pdir_data, pclient->pool_dir_data, next_alloc ) ;
        }
      
      if( to_remove_entry->internal_md.type == DIR_CONTINUE )
        {
          /* Put the pentry back to the pool */
          RELEASE_PREALLOC( to_remove_entry->object.dir_cont.pdir_data, pclient->pool_dir_data, next_alloc ) ;
        }
