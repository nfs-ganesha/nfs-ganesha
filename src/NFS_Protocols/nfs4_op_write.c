...
int nfs4_op_write(struct nfs_argop4 *op, compound_data_t * data, struct nfs_resop4 *resp)
{
  ...
  LogFullDebug(COMPONENT_NFS_V4, "   NFS4_OP_WRITE: offset = %"PRIu64"  length = %llu   stable = %d", offset, size,
         stable_how);
  ...
  LogFullDebug(COMPONENT_NFS_V4, "             NFS4_OP_WRITE: offset = %"PRIu64"  length = %llu", offset, size);
