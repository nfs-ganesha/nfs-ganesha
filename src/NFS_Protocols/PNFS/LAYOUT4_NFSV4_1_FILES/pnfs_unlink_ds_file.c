...
static int pnfs_unlink_ds_partfile(pnfs_ds_client_t * pnfsdsclient,
                         component4 name, fattr4_fileid fileid, pnfs_part_file_t * ppartfile)
{
                        ...
                        fattr4_fileid fileid, pnfs_ds_file_t * pfile)
{
  ...
  for( i = 0 ; i < pnfsclient->nb_ds ; i++ )
   {
      if( ( rc = pnfs_unlink_ds_partfile( &(pnfsclient->ds_client[i]), name, fileid, &(pfile->filepart[i]) ) ) != NFS4_OK )
        return rc ;
   }
