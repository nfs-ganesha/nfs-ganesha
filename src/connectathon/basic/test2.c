...
int main( int argc, char *argv[] )
{
  ...
  if (b == NULL) {
	...
	if (b->levels == -1) {
		fprintf(stderr, "Missing 'levels' parameter in the config file '%s' for the basic test number 2\n", config_file);
    free_testparam(param);
		exit(1);
	}
	if (b->files == -1) {
		fprintf(stderr, "Missing 'files' parameter in the config file '%s' for the basic test number 2\n", config_file);
    free_testparam(param);
		exit(1);
	}
	if (b->dirs == -1) {
		fprintf(stderr, "Missing 'dirs' parameter in the config file '%s' for the basic test number 2\n", config_file);
    free_testparam(param);
		exit(1);
	}
	levels = b->levels;
