...
int main( int argc, char * argv[] )
{
  ...
  if (b == NULL) {
	...
	if (b->files == -1) {
		fprintf(stderr,"Missing 'files' parameter in the config file '%s' for the basic test number 6\n", config_file);
    free_testparam(param);
		exit(1);
	}
	if (b->count == -1) {
		fprintf(stderr,"Missing 'count' parameter in the config file '%s' for the basic test number 6\n", config_file);
    free_testparam(param);
		exit(1);
	}
	count = b->count;
  ...
  free_testparam(param);
	...
	if (count > files) {
		fprintf(stderr,"count (%d) can't be greater than files (%d)",
			count, files);
		exit(1);
	}
...
didn't read expected '%s' dir entry, pass %d", str, ct);
  ...
  fprintf(log, "b6\t%d\t%d\t%ld.%02ld\n", entries, files, (long)time.tv_sec, (long)time.tv_usec / 10000);
  fclose(log);
