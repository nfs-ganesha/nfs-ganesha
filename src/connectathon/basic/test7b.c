...
int main( int argc, char * argv[] )
{
  ...
  if (b == NULL) {
	...
	if (b->files == -1) {
		fprintf(stderr,"Missing 'files' parameter in the config file '%s' for the basic test number 7\n", config_file);
    free_testparam(param);
		exit(1);
	}
	if (b->count == -1) {
		fprintf(stderr,"Missing 'count' parameter in the config file '%s' for the basic test number 7\n", config_file);
    free_testparam(param);
		exit(1);
	}
	count = b->count;
  ...
  fprintf(log, "b7b\t%d\t%d\t%ld.%02ld\n", files * count, files, (long)time.tv_sec, (long)time.tv_usec / 10000);
  fclose(log);
