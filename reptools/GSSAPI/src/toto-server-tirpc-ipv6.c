...
main( int argc, char * argv[] )
{
  ...
  adresse_rpc.sin6_addr        = in6addr_any ;
  ...
  if(!__rpc_fd2sockinfo( sock, &si) )
   {
      fprintf( stderr, "__rpc_fd2sockinfo impossible\n" ) ;
      exit( 1 ) ;
   }
  ...
  if( bind( sock,
   ...
   {
      fprintf( stderr, "bind impossible\n" ) ;
      exit( 1 ) ;
   }

  listen( sock, (int)bindaddr.qlen);

  /* Creation du handler SVC */
  if( ( ptr_svc = svc_vc_create( sock, SEND_SIZE , RECV_SIZE ) ) == NULL )
    {
      fprintf( stderr, "svctcp_create impossible\n" ) ;
      exit( 1 ) ;
    }
  
  ptr_svc->xp_netid = strdup(nconf->nc_netid);
  ptr_svc->xp_tp = strdup(nconf->nc_device);
