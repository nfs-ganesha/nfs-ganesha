                          ...
                          unsigned short port, int sockfd )
{
  ...
  if( ( client = rpcsecgss_clnttcp_create( &adresse_rpc, programme, version,  
...
main( int argc, char * argv[] ) 
{
          ...
          if( isalpha( *optarg ) )
            {
              ...
              if( ( hp = gethostbyname( optarg ) ) == NULL )
                {
                  fprintf( stderr, "Erreur gethostbyname impossible errnp=%u|%s\n", errno, strerror( errno ) ) ;
                  exit( 1 ) ; 
                }
  ...
  if( ( client = Creer_RPCClient( adresse_serveur, rpc_service_num, V1, 0 , RPC_ANYSOCK ) ) == NULL )
    {
      char erreur[100] ;
  ...
  if( ( rc = clnt_call( client, PROC_PLUS1, 
